...
if ToClient_isPS4() then
  isConsoleRegionKorea = CppEnums.GameCountryRegionCodeType.KR == getGameCountryRegionCode()
end
...
function Panel_Logo_Init()
  ...
  elseif isGameTypeTaiwan() or isGameTypeTR() or isGameTypeTH() or isGameTypeID() or isGameTypeGT() or isGameServiceTypeConsole() or isGameTypeRussia() then
...
elseif isGameTypeEnglish() then
  ...
  aniInfo2:SetEndColor(Defines.Color.C_FFFFFFFF)
elseif isGameTypeTaiwan() or isGameTypeGT() or isGameServiceTypeConsole() then
  local aniInfo11 = static_Daum:addColorAnimation(startAniTime + 0, startAniTime + 3, CppEnums.PAUI_ANIM_ADVANCE_TYPE.PAUI_ANIM_ADVANCE_COS_HALF_PI)
  aniInfo11:SetStartColor(Defines.Color.C_00FFFFFF)
  aniInfo11:SetEndColor(Defines.Color.C_FFFFFFFF)
...
function Panel_Logo_Update()
  static_Movie = UI.createControl(__ePAUIControl_WebControl, Panel_Logo, "WebControl_Movie")
