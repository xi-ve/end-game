...
function UpdateHairSlider(sliderIndex)
  ...
  setParam(paramType[luaSliderIndex], paramIndex[luaSliderIndex], value)
...
function OpenHairShapeUi(uiId)
  ...
  for sliderIndex = 0, sliderNum - 1 do
    local luaSliderIndex = sliderIndex + 1
    paramType[luaSliderIndex] = getUiSliderParamType(uiId, defaultContentsIndex, sliderIndex)
    paramIndex[luaSliderIndex] = getUiSliderParamIndex(uiId, defaultContentsIndex, sliderIndex)
    local sliderText = getUiSliderDescName(uiId, defaultContentsIndex, sliderIndex)
    local param = getParam(paramType[luaSliderIndex], paramIndex[luaSliderIndex])
    if 0 <= sliderIndex and sliderIndex < 3 then
      paramMin[luaSliderIndex] = getHairMinLength(sliderIndex)
      paramMax[luaSliderIndex] = getParamMax(paramType[luaSliderIndex], paramIndex[luaSliderIndex])
      paramDefault[luaSliderIndex] = getParamDefault(paramType[luaSliderIndex], paramIndex[luaSliderIndex])
    ...
    elseif sliderIndex == 4 then
      paramMin[luaSliderIndex] = getParamMin(paramType[luaSliderIndex], paramIndex[luaSliderIndex])
      paramMax[luaSliderIndex] = getParamMax(paramType[luaSliderIndex], paramIndex[luaSliderIndex])
      paramDefault[luaSliderIndex] = getParamDefault(paramType[luaSliderIndex], paramIndex[luaSliderIndex])
      if curlRange == 0 then
        EnableSlide(StaticTextArr[luaSliderIndex], StaticText_CurrentValue[luaSliderIndex], SliderArr[luaSliderIndex], false)
...
function OpenHairShapeUiWithoutBoneControl(uiId)
  ...
  local defaultContentsIndex = 0
  sliderCount = getUiSliderCount(uiId, defaultContentsIndex)
  local meshParamType = 1
  local curlRange = getCurlLengthRange()
