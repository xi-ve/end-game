...
function OpenCommonDecorationUi(classType, uiId, checkType)
  ...
  if 1 < contentsCount then
    for contentsIndex = 0, contentsCount - 1 do
      ...
      local tempRadioButton = UI.createControl(__ePAUIControl_RadioButton, Panel_Customization_Common_Decoration, "RadioButton_" .. luaContentsIndex)
      ...
      tempRadioButton:SetTextMode(__eTextMode_AutoWrap)
...
function UpdateDecorationContents(contentsIndex, currentclassType, currentuiId)
  ...
  if listCount == 1 then
    ...
    for itemIndex = 0, paramMax do
      ...
      local tempContentImage = UI.createControl(__ePAUIControl_Static, Frame_Content, "Frame_Image_" .. itemIndex .. "_" .. selectedUiId)
      ...
      if NoCashType.eCustomizationNoCashType_Deco == listParamType and (NoCashDeco.eCustomizationNoCashDeco_FaceTattoo == listParamIndex or NoCashDeco.eCustomizationNoCashDeco_BodyTattoo == listParamIndex) then
        staticPayMark = UI.createControl(__ePAUIControl_Static, tempContentImage, "Static_PayMark_" .. itemIndex .. "_" .. selectedUiId)
      ...
      elseif not isNormalCustomizingIndex(selectedClassType, listParamType, listParamIndex, itemIndex) and not FGlobal_IsInGameMode() then
        tempContentImage:SetShow(false)
      else
  ...
  if paletteCount == 1 then
    ...
    CreateCommonPalette(FrameTemplateColor, Static_Collision, selectedClassType, paletteParamType, paletteParamIndex, paletteIndex, false)
...
function OpenEyeDecorationUi(classType, uiId)
  ...
  CheckControlArr[1]:SetIgnore(isShy)
  CheckControlArr[2]:SetIgnore(isShy)
  CheckControlArr[1]:SetMonoTone(isShy)
  CheckControlArr[2]:SetMonoTone(isShy)
...
function UpdateEyeDecorationContents(contentsIndex, addHistory, currentclassType, currentuiId)
  ...
  if listCount == 1 then
    ...
    for itemIndex = 0, paramMax do
      local luaShapeIdx = itemIndex + 1
      local tempContentImage = UI.createControl(__ePAUIControl_Static, Frame_Content, "Frame_Image_" .. itemIndex)
      CopyBaseProperty(Frame_ContentImage, tempContentImage)
