...
function CustomizationStatic_UpdatePerFrame(deltaTime)
  ...
  if doScreenShotInFrame then
    if 0.1 < timerForSS and doScreenCapture == true then
      ToClient_CaptureFace()
      doScreenCapture = false
      EndScreenCapture = true
    elseif 0.2 < timerForSS and EndScreenCapture == true then
      _takeFaceScreenShot()
      EndScreenCapture = false
    elseif 0.3 < timerForSS then
      timerForSS = 0
  ...
  if initialized == true then
    for index = 1, mainButtonNum do
      if index ~= 3 then
        ...
        if true == mainButtonInfo[index].isOpen and 170 < distance then
...
function chracterCraeation_SeasonSetting()
  ...
  if true == canMakeSeason then
    ...
    txt_SeasonFamilyTitle:SetSpanSize(txt_SeasonFamilyTitle:GetSpanSize().x, originFamTitleSpan)
    txt_SeasonFamilyName:SetSpanSize(txt_SeasonFamilyName:GetSpanSize().x, originFamNameSpan)
    txt_SeasonCharNameTitle:SetSpanSize(txt_SeasonCharNameTitle:GetSpanSize().x, originCharTitleSpan)
    edit_SeasonCharName:SetSpanSize(edit_SeasonCharName:GetSpanSize().x, originEditSpan)
    ...
    btn_CreateSeason:SetSpanSize(btn_CreateSeason:GetSpanSize().x, originBtnSpan + 13)
  end
