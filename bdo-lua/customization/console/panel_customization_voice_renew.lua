...
function Customization_VoiceInfo:UpdateVoice()
  ...
  if true == _ContentsGroup_isContentsCustomizationVoice then
    applyVoice(self._selectedVoiceIndex + 1, false)
  else
    applyVoice(self._selectedVoiceIndex + 1, true)
  end
end
...
function PaGlobalFunc_Customization_Voice_CreateVoiceList(inGameMode, classType)
  ...
  for itemIdx = 0, self._voiceCount - 1 do
    local tempContentImage = UI.createControl(__ePAUIControl_RadioButton, self._ui._static_TypeGroup, "Frame_Image_" .. itemIdx)
  ...
  for itemIdx = 0, count - 1 do
    if MotionTable[itemIdx] == social then
      local tempContentImage = UI.createControl(__ePAUIControl_RadioButton, self._ui._static_motionGroup, "Frame_MotionImage_" .. createIndex)
