...
local _needCollectTool = UI.getChildControl(Instance_Interaction, "Static_Cant")
local _background = UI.getChildControl(Instance_Interaction, "Static_Background")
_background:setGlassBackground(true)
local _globalGuide = UI.getChildControl(Instance_Interaction, "StaticText_Purpose")
local _circularProgressBarInteraction = UI.getChildControl(Instance_Interaction, "CircularProgress_Press")
local _circularProgressBarStaticbgInteraction = UI.getChildControl(Instance_Interaction, "Static_CircleProgress_PressBG")
Instance_Interaction:RegisterShowEventFunc(true, "Panel_Interaction_ShowAni()")
Instance_Interaction:RegisterShowEventFunc(false, "Panel_Interaction_HideAni()")
...
for ii = 0, #BUTTON_ID do
  UI_BUTTON[ii] = UI.getChildControl(Instance_Interaction, BUTTON_ID[ii])
...
local otherUIList = {
  [CppEnums.InteractionType.InteractionType_HavestLop] = UI.getChildControl(Instance_Interaction, "Button_KillBugs"),
  [CppEnums.InteractionType.InteractionType_HavestKillBug] = UI.getChildControl(Instance_Interaction, "Button_Feeding"),
  [CppEnums.InteractionType.InteractionType_HarvestInformation] = UI.getChildControl(Instance_Interaction, "Button_DomesticAnimalInfo")
}
...
UI.getChildControl(Instance_Interaction, "Button_Hunting"):SetShow(false)
UI.getChildControl(Instance_Interaction, "Button_VehicleTraining"):SetShow(false)
local UI_NAME = UI.getChildControl(Instance_Interaction, "Static_Text_CharacterName")
local UI_DESC = UI.getChildControl(Instance_Interaction, "Static_Icon_Font")
...
function Interaction_Update(deltaTime)
  ...
  if selfPlayer == nil or selfPlayer:isDead() or Instance_Looting:GetShow() or Defines.UIMode.eUIMode_Default ~= GetUIMode() or isDeadPlayer then
    Interaction_Close()
    return
  end
  ...
  if nil ~= actor then
    ...
    if Instance_Interaction:GetShow() and ANIMATING_BUTTON then
...
function Interaction_Show(actor)
  ...
  if ToClient_isConsole() and actor:get():isHouseHold() then
    return
  end
  if true == ToClient_isConsole() then
    for ii = 0, #interactionTargetUIList do
      ...
      if isShow and (ii == CppEnums.InteractionType.InteractionType_OpenDoor or ii == CppEnums.InteractionType.InteractionType_Observer or ii == CppEnums.InteractionType.InteractionType_PvPBattle or ii == CppEnums.InteractionType.InteractionType_RankerHouseList) then
        Instance_Interaction:SetShow(false)
        return
      end
    end
  end
  ...
  Instance_Interaction:SetShow(true)
  local actor = interaction_getInteractable()
  local actorKey = 0
  local interactableFrag = 0
  ...
  _globalGuide:SetPosY(Instance_Interaction:GetSizeY() * 0.5 - _globalGuide:GetSizeY() * 0.5)
...
function Panel_Interaction_GetGlobalGuidePosX()
  return Instance_Interaction:GetPosX() + _globalGuide:GetPosX()
end
function Panel_Interaction_GetGlobalGuidePosY()
  return Instance_Interaction:GetPosY() + _globalGuide:GetPosY()
end
function Interaction_Close()
  if Instance_Interaction:IsShow() then
    Instance_Interaction:SetShow(false)
...
function Interaction_PositionUpdate(actor)
  ...
  if pos2d.x < 0 and 0 > pos2d.y then
    Instance_Interaction:SetPosX(-1000)
    Instance_Interaction:SetPosY(-1000)
  else
    Instance_Interaction:SetPosX(pos2d.x + 0)
    Instance_Interaction:SetPosY(pos2d.y - 100)
  end
end
...
function Interaction_ButtonPushed(interactionType)
  if CppEnums.InteractionType.InteractionType_Ride ~= interactionType and CppEnums.InteractionType.InteractionType_Looting ~= interactionType and CppEnums.InteractionType.InteractionType_BlackSpiritPossession ~= interactionType then
    return
  end
  preUIMode = GetUIMode()
...
registerEvent("EventSelfPlayerDead", "FromClient_EventSelfPlayerDead_Interaction")
...
function PaGlobal_Interaction_GetShow()
  return Instance_Interaction:GetShow()
end
...
function Interaction_ExecuteByKeyMapping(keycode)
  ...
  elseif keycode == CppEnums.VirtualKeyCode.KeyCode_F10 and 7 <= buttonCount then
...
function FGlobal_Interaction_CheckAndGetPressedKeyCode_Xbox(deltaTime)
  ...
  elseif keyCustom_IsPressed_Action(CppEnums.ActionInputType.ActionInputType_Interaction) then
    ...
    if 0.5 < currentInteractionKeyPressedTime then
  ...
  elseif keyCustom_IsUp_Action(CppEnums.ActionInputType.ActionInputType_Interaction) then
    if currentInteractionKeyPressedTime < 0.5 and 1 < SHOW_BUTTON_COUNT then
...
function FromClient_EventSelfPlayerDead_Interaction()
  isDeadPlayer = true
end
houseInit()
