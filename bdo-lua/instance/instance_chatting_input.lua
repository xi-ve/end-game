...
function chatInput:init()
  self.buttonIds = {}
  if nil == Panel_SA_Widget_Party or 0 >= PaGlobalFunc_SA_Widget_Party_PartyMemberCount() then
    self.buttonIds = {
      [0] = nil,
      [1] = "Button_Anounce",
      [3] = "Button_Normal"
  ...
  if true == ToClient_IsInstanceFieldPlayerbyContentsType(__eInstanceContentsType_BattleRoyale) then
    self.control.arabicCheckButton:SetShow(false)
  end
...
function ChatInput_TypeButtonOn(chatType)
  ...
  local self = chatInput
  local button = self.control.buttons[chatType]
  local count = RequestParty_getPartyMemberCount()
  local isTeamMode = false
  if 1 < count then
    isTeamMode = true
  end
  if true == isTeamMode then
...
function ChatInput_CheckReservedKey()
  ...
  elseif isKeyPressed(VCK.KeyCode_MENU) and isKeyDown_Once(VCK.KeyCode_4) or isKeyDown_Once(VCK.KeyCode_1) then
...
function ChatInput_CheckInstantCommand()
  ...
  if ChatInput_IsInstantCommand_Normal(chatMessage) then
    ...
    isProcess = true
  elseif ChatInput_IsInstantCommand_Party(chatMessage) and nil ~= Panel_SA_Widget_Party and 0 >= PaGlobalFunc_SA_Widget_Party_PartyMemberCount() then
