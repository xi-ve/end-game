...
function quickSlot:createSlot()
  for ii = 1, self.config.slotCount do
    local slot = {
      ...
      item = nil,
      skill = nil,
      background = UI.createControl(__ePAUIControl_StaticText, Instance_QuickSlot, "Static_Slot_BG_" .. ii),
...
function quickSlot:setSpecIcon(skillNo, slotNo)
  ...
  self:setSpecIconEmpty(slotNo)
  self:updateSpecIconPos(slotNo, 0 < count)
  if count <= 0 then
    return
  end
...
function quickSlot:setSkillDesc(skillNo, slotNo)
  local index = slotNo + 1
  self._staticText_SkillDescList[index]:SetTextMode(__eTextMode_AutoWrap)
