...
function HandleMLUp_SkillWindow_UpdateData(tabIndex, isLearnMode, doForce)
  ...
  if not doForce and not Instance_Window_Skill:GetShow() then
    return
  end
...
function HandleMLUp_SkillWindow_OpenForLearn()
  ...
  if not Instance_Window_Skill:IsShow() then
    UIAni.fadeInSCRDialog_Down(Instance_Window_Skill)
    Instance_Window_Skill:SetShow(true, true)
    if screenSizeX <= 1400 then
      Instance_Window_Skill:SetPosX(screenSizeX / 2 - Instance_Window_Skill:GetSizeX() / 2 - 100)
    else
      Instance_Window_Skill:SetPosX(screenSizeX / 2 - Instance_Window_Skill:GetSizeX() / 2 - 150)
    end
    Instance_Window_Skill:SetPosY(screenSizeY / 2 - Instance_Window_Skill:GetSizeY() / 2)
    self:OpenLearnAni1()
    self:OpenLearnAni2()
  end
...
function HandleMLUp_SkillWindow_Close(isManualClick)
  if Instance_Window_Skill:IsShow() then
    PaGlobal_Skill.isPartsSkillReset = false
    local self = PaGlobal_Skill
    self.lastLearnMode = true
    self.saved_isLearnMode = true
    Panel_SkillTooltip_Hide()
    Instance_Window_Skill:SetShow(false, true)
  end
...
function HandleMOver_SkillWindow_ToolTipHide(skillNo, SlotType, isFusion)
  ...
  if not Instance_Tooltip_SimpleText:GetShow() then
    local skillStaticWrapper = getSkillStaticStatus(skillNo, 1)
    local skillTypeStaticStatusWrapper = skillStaticWrapper:getSkillTypeStaticStatusWrapper()
    local skillLevelInfo = getSkillLevelInfo(skillNo)
...
function HandleMLUp_SkillWindow_StartDrag(skillNo)
  ...
  if nil ~= skillLevelInfo and nil ~= skillTypeStaticWrapper then
    DragManager:setDragInfo(Instance_Window_Skill, nil, skillLevelInfo._skillKey:get(), "Icon/" .. skillTypeStaticWrapper:getIconPath(), Skill_GroundClick, nil)
  end
end
function HandleMScroll_SkillWindow_ScrollEvent(isShow)
  ...
  if isShow then
    ...
    UIScrollButton.ScrollButtonEvent(false, Instance_Window_Skill, PaGlobal_Skill.frames[0], vScroll)
  end
end
...
function Skill_RegistEventHandler()
  ...
  Instance_Window_Skill:RegisterShowEventFunc(true, "Skill_ShowAni()")
  Instance_Window_Skill:RegisterShowEventFunc(false, "Skill_HideAni()")
