...
Instance_MessageHistory:SetShow(false)
Instance_MessageHistory:RegisterShowEventFunc(true, "MessageHistory_ShowAni()")
Instance_MessageHistory:RegisterShowEventFunc(false, "MessageHistory_HideAni()")
Instance_MessageHistory:ActiveMouseEventEffect(true)
Instance_MessageHistory_BTN:SetShow(false)
local thisWidgetComplete = false
local MessageHistory = {
  content = UI.getChildControl(Instance_MessageHistory, "StaticText_Content"),
  autoPop = UI.getChildControl(Instance_MessageHistory, "CheckButton_AutoPopUp"),
  BTN_Next = UI.getChildControl(Instance_MessageHistory, "Button_Next"),
  BTN_Confirm = UI.getChildControl(Instance_MessageHistory, "Button_Confirm"),
  contentCount = UI.getChildControl(Instance_MessageHistory, "StaticText_Page"),
  closeBTN = UI.getChildControl(Instance_MessageHistory, "Button_Close"),
  badges = UI.getChildControl(Instance_MessageHistory_BTN, "StaticText_badges"),
  badgesBTN = UI.getChildControl(Instance_MessageHistory_BTN, "Static_BTN"),
  thisPage = 1,
  msgCount = 0,
  elapsedTime = 0,
  hideAniPlay = false
...
local panelSizeX = Instance_MessageHistory:GetSizeX()
local panelSizeY = Instance_MessageHistory:GetSizeY()
local Acquire_Enum = {
  LevelUp = 0,
...
function MessageHistory_ShowAni()
  local MoveAni1 = Instance_MessageHistory:addMoveAnimation(0, 0.3, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
  ...
  local MoveAni2 = Instance_MessageHistory:addMoveAnimation(0.3, 0.4, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
...
function MessageHistory:init()
  ...
  self.content:SetTextMode(__eTextMode_AutoWrap)
...
function FGlobal_MessageHistory_InputMSG(type, msg, func)
  ...
  if not Instance_MessageHistory:GetShow() then
    self:PrintMsg()
  else
    self:Update()
  end
...
function MessageHistory:PrintMsg()
  ...
  if MessageHistory.autoPop:IsCheck() and Defines.UIMode.eUIMode_Default == GetUIMode() then
    Instance_MessageHistory:SetShow(true, true)
    Instance_MessageHistory_BTN:SetShow(false)
  end
end
...
function MessageHistory:BadgesBTN_ToggleLogic(length)
  ...
  if 0 == length then
    ...
    Instance_MessageHistory_BTN:SetShow(true)
  end
end
function MessageHistory:Close()
  self.msgCount = massagePool:length()
  Instance_MessageHistory:SetShow(false, false)
  MessageHistory.badges:SetText(self.msgCount)
...
function FromClient_MessageHistoryBTN_BounceLoop()
  if Instance_MessageHistory_BTN:GetShow() then
    local panelSizeX = Instance_MessageHistory_BTN:GetSizeX()
    local panelSizeY = Instance_MessageHistory_BTN:GetSizeY()
    local MoveAni1 = Instance_MessageHistory_BTN:addMoveAnimation(0, 0.2, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
    MoveAni1:SetStartPosition(screenX - panelSizeX - Instance_MessageHistory_BTN:GetSpanSize().x, screenY - panelSizeY)
    MoveAni1:SetEndPosition(screenX - panelSizeX - Instance_MessageHistory_BTN:GetSpanSize().x, screenY - panelSizeY - 15)
    MoveAni1.IsChangeChild = true
    local MoveAni2 = Instance_MessageHistory_BTN:addMoveAnimation(0.2, 0.3, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
    MoveAni2:SetStartPosition(screenX - panelSizeX - Instance_MessageHistory_BTN:GetSpanSize().x, screenY - panelSizeY - 15)
    MoveAni2:SetEndPosition(screenX - panelSizeX - Instance_MessageHistory_BTN:GetSpanSize().x, screenY - panelSizeY)
    MoveAni2.IsChangeChild = true
  else
    return
  end
...
function MessageHistory:registMessageHandler()
  ...
  Instance_MessageHistory:RegisterUpdateFunc("MessageHistory_AutoShowOff")
end
function MessageHistory_AutoShowOff(deltaTime)
  local self = MessageHistory
  self.elapsedTime = self.elapsedTime + deltaTime
  if self.elapsedTime > 5 and false == self.hideAniPlay then
    local MoveAni3 = Instance_MessageHistory:addMoveAnimation(0, 0.1, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
    ...
    local MoveAni4 = Instance_MessageHistory:addMoveAnimation(0.1, 0.4, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
