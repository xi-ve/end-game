...
local Pin_FindWay = UI.getChildControl(Panel_SA_Widget_Minimap, "icon_pin_Findway")
local Pin_FindWay_Arrow = UI.getChildControl(Panel_SA_Widget_Minimap, "pin_area_Findway")
local RadarSizeX = 0
local RadarSizeY = 0
local SelfPosX = 0
local SelfPosZ = 0
...
local function CalcRadarPostionToWorldPostion()
  local posX = getMousePosX() - Panel_SA_Widget_Minimap:GetPosX()
  local posY = getMousePosY() - Panel_SA_Widget_Minimap:GetPosY()
...
function InitPinInRadar()
  ...
  Ping_Effect = UI.getChildControl(Panel_SA_Widget_Minimap, "Static_GetthePoint")
  local selfPlayerWrapper = getSelfPlayer()
  if nil == selfPlayerWrapper then
    return
  end
  for i = 0, _listPingMax do
    _listPing[i] = UI.createControl(__ePAUIControl_Static, Panel_SA_Widget_Minimap, "Ping_Effect" .. tostring(i))
    CopyBaseProperty(Ping_Effect, _listPing[i])
  end
  for i = 0, _listGuildMax do
    _listGuild[i] = UI.createControl(__ePAUIControl_Static, Panel_SA_Widget_Minimap, "Radar_Guild_" .. tostring(i))
    CopyBaseProperty(Pin_Guild, _listGuild[i])
  end
  for i = 0, _listPartyMax do
    _listParty[i] = UI.createControl(__ePAUIControl_Static, Panel_SA_Widget_Minimap, "Radar_Party_" .. tostring(i))
    CopyBaseProperty(Pin_Party, _listParty[i])
  end
  for i = 0, _listGuideMax do
    _listGuide[i] = UI.createControl(__ePAUIControl_Static, Panel_SA_Widget_Minimap, "Radar_Guide_" .. tostring(i))
...
function FromClient_ResponseSendPositionGuideToRadar(PinInfo)
  ...
  if PinInfo._isPermanent == false then
    ...
    local RadarSizeX = Panel_SA_Widget_Minimap:GetSizeX()
    local RadarSizeY = Panel_SA_Widget_Minimap:GetSizeY()
...
function FromeClient_RadarPINCreate(PinInfo, control, pinGuideControl, isMine)
  ...
  local posX = Panel_SA_Widget_Minimap:GetSizeX() - pinGuideControl:GetSizeX()
  local posY = Panel_SA_Widget_Minimap:GetSizeY() - pinGuideControl:GetSizeY()
  posX = posX / 2
  posY = posY / 2
...
function FGlobal_UpdateRadarPin()
  ...
  RadarSizeX = Panel_SA_Widget_Minimap:GetSizeX()
  RadarSizeY = Panel_SA_Widget_Minimap:GetSizeY()
