function PaGlobal_ModeBranch:initialize()
  ...
  PaGlobal_ModeBranch._ui._txt_branchDesc:SetTextMode(__eTextMode_AutoWrap)
  for index = 1, 3 do
...
function PaGlobal_ModeBranch:checkBettingKey()
  ...
  if nil == PaGlobal_ModeBranch._selectedMode or false == (__eBattleRoyaleMode_Solo == PaGlobal_ModeBranch._selectedMode or __eBattleRoyaleMode_Team == PaGlobal_ModeBranch._selectedMode) then
    return result
  end
  local nowSeasonScore = ToClient_GetBattleRoyaleCurrentSeasonScore(PaGlobal_ModeBranch._selectedMode)
  if nil == nowSeasonScore then
