...
function PaGlobal_TutorialPhase_BasicMove:startPhaseXXX(stepNo)
  ...
  _PA_LOG("\237\138\156\237\134\160\235\166\172\236\150\188", "PaGlobal_TutorialPhase_BasicMove:startPhase() stepNo : " .. tostring(stepNo))
  self._currentStep = 1
...
function PaGlobal_TutorialPhase_BasicMove:Step02_Prepare()
  ...
  local message = PAGetString(Defines.StringSheet_GAME, "LUA_TUTORIAL_OBSIDIAN_TEXT") .. " " .. PAGetString(Defines.StringSheet_GAME, "LUA_TUTORIALXB_TEXT_1")
...
function PaGlobal_TutorialPhase_BasicMove:Step02_DoStep(deltaTime)
  if 1 == self._currentProgress then
    if self._updateTime < 1.5 * timeRatio then
      ...
      local message = PAGetString(Defines.StringSheet_GAME, "LUA_TUTORIAL_OBSIDIAN_TEXT_3") .. " " .. PAGetString(Defines.StringSheet_GAME, "LUA_TUTORIALXB_TEXT_2")
      PaGlobal_TutorialUiManager:getUiBlackSpirit():setSpiritUiForTutorial_Desc(message)
      self._updateTime = 0
      self._currentProgress = self._currentProgress + 1
    end
  elseif 2 == self._currentProgress then
    if self._updateTime < 2 * timeRatio then
      ...
      local message = PAGetString(Defines.StringSheet_GAME, "TUTORIAL_MOVE_STEP2_DARKSPIRIT") .. " " .. PAGetString(Defines.StringSheet_GAME, "LUA_TUTORIALXB_TEXT_3")
      PaGlobal_TutorialUiManager:getUiBlackSpirit():setSpiritUiForTutorial_Desc(message)
      self._updateTime = 0
      self._currentProgress = self._currentProgress + 1
    end
