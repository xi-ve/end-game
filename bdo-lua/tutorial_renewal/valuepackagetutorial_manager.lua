...
function PaGlobal_ValuePackageTutorial_Manager:updateDeltaTime_ValuePackage_Step1(deltaTime)
  ...
  local invenOpenCheck = false
  if true == _ContentsGroup_NewUI_Inventory_All then
    invenOpenCheck = Panel_Window_Inventory_All:GetShow()
  else
    invenOpenCheck = Panel_Window_Inventory:GetShow()
  end
  self._updateTime = self._updateTime + deltaTime
  if invenOpenCheck or self._updateTime > 5 then
    if not invenOpenCheck then
...
function PaGlobal_ValuePackageTutorial_Manager:updateDeltaTime_ValuePackage_Step2(deltaTime)
  ...
  if true == _ContentsGroup_NewUI_Inventory_All then
    isValuePackageItemUse = PaGlobalFunc_Inventory_All_IsValuePackageItemUse()
  else
    isValuePackageItemUse = FGlobal_ValuePackagerItemUse()
  end
  if true == isValuePackageItemUse then
    self._updateTime = 0
    self._stepNo = 3
  end
