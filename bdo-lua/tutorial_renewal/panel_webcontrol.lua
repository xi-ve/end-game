...
local html_WebHelper_Control = UI.createControl(__ePAUIControl_WebControl, Panel_WebControl, "WebControl_Help_CharInfo")
local ui = {
  _btn_Close = nil,
  _txt_Title = UI.getChildControl(Panel_WebControl, "StaticText_Title"),
...
function Panel_WebHelper_ShowToggle(helpType)
  ...
  if Panel_WebControl:IsShow() then
    ...
    if isGameTypeKR2() then
      local messageBoxData = {
        title = PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_ALERT_NOTIFICATIONS"),
        content = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_GUILD_ALLIANCEINFO_BOARD_READY"),
        functionApply = MessageBox_Empty_function,
        priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW
      }
      MessageBox.showMessageBox(messageBoxData)
...
function Panel_WebControl_Initialize()
  ...
  ui._btn_Close = UI.getChildControl(ui._txt_Title, "Button_Close")
