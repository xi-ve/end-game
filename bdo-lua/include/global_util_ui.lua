...
function UI.checkResolution4KForXBox()
  if true == isGameServiceTypeConsole() then
    ...
    if 3840 <= screenWidth or 2160 <= screenHeight then
...
function PaGlobalFunc_ConsoleKeyGuide_SetAlign(listKeyGuid, parentControl, alignType, keySize, keySpace)
  ...
  if CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_RIGHTAUTOWRAP == alignType then
    ...
    local length = parantSizeX
    local height = 0
    local prevControlName = ""
    for name, value in pairs(reversedTable) do
      if true == value:GetShow() then
        local spaceFromRight = value:GetTextSizeX() + space
        local keySizeLT = 0
        if nil ~= prevControlName and nil ~= string.find(prevControlName, "LT") then
          keySizeLT = keySize + 10
          spaceFromRight = spaceFromRight + keySizeLT
        end
        length = length - spaceFromRight
        prevControlName = value:GetID()
        if length - 20 - keySizeLT < 0 then
          length = parantSizeX - spaceFromRight + keySizeLT
          height = height + value:GetSizeY()
        end
        value:SetPosX(length)
        value:SetPosY(height)
      end
    end
    local combineKeyBgSizeY = 50
    parentControl:SetSize(parantSizeX, combineKeyBgSizeY + height)
    parentControl:SetSpanSize(parentControl:GetSpanSize().x, (combineKeyBgSizeY + height) * -1)
