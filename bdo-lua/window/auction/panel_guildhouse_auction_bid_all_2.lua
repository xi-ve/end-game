...
function HandleEventLUp_GuildHouse_Auction_All_clickedConfirm()
  ...
  if nil == houseAuctionNowPrice then
    return
  end
...
function FromClient_GuildHouse_Auction_Bid_All_ReSizePanel()
  if false == PaGlobal_GuildHouse_Auction_Bid_All._isConsole then
    ...
    local keyguide = {
      PaGlobal_GuildHouse_Auction_Bid_All._ui_console.stc_keyGuideX,
      PaGlobal_GuildHouse_Auction_Bid_All._ui_console.stc_selectA,
      PaGlobal_GuildHouse_Auction_Bid_All._ui_console.stc_cancelB
    }
    PaGlobalFunc_ConsoleKeyGuide_SetAlign(keyguide, PaGlobal_GuildHouse_Auction_Bid_All._ui_console.stc_bottomBg, CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_RIGHT)
  end
