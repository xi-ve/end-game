...
function initializeNaviBtn()
  ...
  Panel_NaviButton:SetSize(naviBtn:GetSizeX() + 10, naviBtn:GetSizeY() + 10)
...
function WorldMap_NaviButton_RePos()
  ...
  if 1 < playerModelPositionPosZ or playerModelPositionPosZ < 0 then
    Panel_NaviButton:SetShow(false)
...
function WorldMap_ShortcutButton_RePos()
  ...
  local playerModelPositionPosZ
  if true == ToClient_WorldMapNaviIsLoopPath() then
    Panel_NaviButton:SetPosX(ToClient_getNaviEndPathPostionForOnlyLoopPath().x * getScreenSizeX() - 60)
    Panel_NaviButton:SetPosY(ToClient_getNaviEndPathPostionForOnlyLoopPath().y * getScreenSizeY() + 60)
    playerModelPositionPosZ = ToClient_getNaviEndPathPostionForOnlyLoopPath().z
  else
    ...
    playerModelPositionPosZ = ToClient_getNaviEndPathPostion().z
  end
  if 1 < playerModelPositionPosZ or playerModelPositionPosZ < 0 then
    Panel_NaviButton:SetShow(false)
  elseif isCullingNaviBtn == false then
...
function FGlobal_OpenOtherPanelWithNodeMenu(node, isShow)
  ...
  local wayPlant = ToClient_getPlant(plantKey)
  local exploreLevel = node:getLevel()
  if 0 < exploreLevel and wayPlant ~= nil and wayPlant:getType() == CppEnums.PlantType.ePlantType_Zone then
