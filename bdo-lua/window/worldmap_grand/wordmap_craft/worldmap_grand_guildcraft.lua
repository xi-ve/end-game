...
GuildCraft_Right._check_IgnoreWorkingCount:SetTextMode(__eTextMode_AutoWrap)
local WorkDataRow = {}
...
function GuildCraft_Right:Update_Reciepe()
  if nil ~= self._itemExchangeKeyRaw then
    ...
    GuildCraft_Right._workingItem_Name:SetTextMode(__eTextMode_Limit_AutoWrap)
...
function GuildCraft_Right:Update_Detail()
  ...
  if nil ~= exchangeItemKeyRaw then
    ...
    if itemExchangeSSW:isSet() then
      ...
      self._partsDetail_Item_Name:SetTextMode(__eTextMode_LimitText)
...
function FGlobal_GuildCraft_Open(houseKeyRaw, reciepeKeyRaw)
  ...
  Panel_Info._useTypeDesc:SetTextMode(__eTextMode_AutoWrap)
