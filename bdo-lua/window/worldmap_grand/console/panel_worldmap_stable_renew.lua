...
function Window_WorldMap_StableInfo:SetStable()
  ...
  for servantIdx = 0, nodeServantCount - 1 do
    ...
    if nil ~= servantInfo then
      ...
      if vehicleType == CppEnums.VehicleType.Type_Camel or vehicleType == CppEnums.VehicleType.Type_Donkey or vehicleType == CppEnums.VehicleType.Type_Elephant or vehicleType == CppEnums.VehicleType.Type_Carriage or vehicleType == CppEnums.VehicleType.Type_RepairableCarriage then
...
function Window_WorldMap_StableInfo:InitControl()
  self._ui._list2_Stable:registEvent(__ePAUIList2EventType_LuaChangeContent, "PaGlobalFunc_WorldMap_Stable_List2EventControlCreate")
  self._ui._list2_Stable:createChildContent(__ePAUIList2ElementManagerType_List)
end
...
function PaGlobalFunc_WorldMap_Stable_List2EventControlCreate(list_content, key)
  ...
  if true == ToClient_IsContentsGroupOpen("243") and nil ~= info._tier and 9 ~= info._tier then
  ...
  static_Sex:SetShow(nil ~= info._isMale)
  if nil ~= info._isMale then
