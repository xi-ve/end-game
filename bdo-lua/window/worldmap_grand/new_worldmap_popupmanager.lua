...
function WorldMapPopupManager:pop()
  ...
  self._currentMode = self._currentMode - 1
  local contentOpen_Worldmap1 = ToClient_IsGrowStepOpen(__eGrowStep_worldmap1)
  local contentOpen_Worldmap2 = ToClient_IsGrowStepOpen(__eGrowStep_worldmap2)
  local contentOpen_Worldmap3 = ToClient_IsGrowStepOpen(__eGrowStep_worldmap3)
  if 0 <= self._currentMode then
    ...
    for key, value in pairs(popArray) do
      ...
      elseif value._panel == Panel_NodeMenu then
        if true == contentOpen_Worldmap3 then
