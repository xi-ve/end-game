...
function FromClient_UI_CharacterInfo_Basic_WeightChanged()
  ...
  self._ui._staticStatus_Value[self._status._weight]:SetText(str_AllWeight .. " / " .. str_MaxWeight .. " " .. PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_WEIGHT"))
end
function FromClient_UI_CharacterInfo_Basic_AttackChanged()
  ...
  self._ui._staticTextStatExtra_Param1_Title:SetShow(false)
  self._ui._staticTextStatExtra_Param2_Title:SetShow(false)
  self._ui._staticTextStatExtra_Param3_Title:SetShow(false)
  ...
  if nil ~= isSetAwakenWeapon then
    ...
    if false == isShyWomen then
      ...
      self._ui._staticTextStatExtra_Param1_Title:SetShow(isShow)
      self._ui._staticTextStatExtra_Param1_Value:SetText(tostring(chaAwakenAttack))
    end
...
function FromClient_UI_CharacterInfo_Basic_SkillPointChanged()
  LUA_SCOPED_PROFILE("FromClient_UI_CharacterInfo_Basic_SkillPointChanged")
  if nil == Panel_Window_CharInfo_BasicStatus then
    return
  end
  local self = PaGlobal_CharacterInfoBasic
...
function FromClient_UI_CharacterInfo_Basic_CraftLevelChanged()
  ...
  for index = 1, 10 do
    ...
    if nil ~= self._ui._lifeInfo[index]._level then
      self._ui._lifeInfo[index]._level:SetText(PaGlobalFunc_Util_CraftLevelColorReplace(self._playerGet:getLifeExperienceLevel(craftType)))
...
function FromClient_UI_CharacterInfo_Basic_RankChanged()
  ...
  if 0 < bestRank and bestRank < 31 then
...
function FromClient_UI_CharacterInfo_Basic_LifeLevelChangeNew()
  ...
  for key, value in pairs(PaGlobal_Char_LifeInfo._lifeType_Panel_Name) do
    ...
    PaGlobal_Char_LifeInfo._lifeInfo[key]._ui._levelText:SetText(PaGlobalFunc_Util_CraftLevelColorReplace(currentLevel))
