...
function CharacterChallengeInfo:init()
  ...
  self._ui.list2:registEvent(__ePAUIList2EventType_LuaChangeContent, "PaGlobalFunc_CharacterChallengeInfo_ListContent")
...
function PaGlobalFunc_CharacterChallengeInfo_ListContent(content, key)
  ...
  txt_title:SetTextMode(__eTextMode_None)
  ...
  if nil ~= challengeWrapper then
    ...
    if txt_title:GetSizeX() < txt_title:GetTextSizeX() then
      txt_title:SetTextMode(__eTextMode_Limit_AutoWrap)
    ...
    for ii = 1, #stc_baseSlotBG do
      stc_baseSlot[ii]:addInputEvent("Mouse_On", "InputMOn_CharacterChallengeInfo_SelectItem(" .. key32 .. "," .. ii .. ")")
      stc_baseSlot[ii]:registerPadEvent(__eConsoleUIPadEvent_Up_X, "")
      stc_baseSlot[ii]:registerPadEvent(__eConsoleUIPadEvent_Up_A, "")
      ...
      if ii <= baseRewardCount then
        ...
        if nil ~= baseReward then
          ...
          stc_baseSlot[ii]:registerPadEvent(__eConsoleUIPadEvent_Up_X, "InputPadX_CharacterChallengeInfo_ShowTooltip(" .. key32 .. "," .. ii .. ")")
    ...
    for ii = 1, #stc_selectSlotBG do
      ...
      stc_selectSlot[ii]:addInputEvent("Mouse_On", "InputMOn_CharacterChallengeInfo_SelectItem(" .. key32 .. "," .. alterii .. ")")
      stc_selectSlot[ii]:registerPadEvent(__eConsoleUIPadEvent_Up_X, "")
      stc_selectSlot[ii]:registerPadEvent(__eConsoleUIPadEvent_Up_A, "")
      ...
      if ii <= selectRewardCount then
        ...
        if nil ~= selectReward then
          ...
          stc_selectSlot[ii]:registerPadEvent(__eConsoleUIPadEvent_Up_X, "InputPadX_CharacterChallengeInfo_ShowTooltip(" .. key32 .. "," .. alterii .. ")")
          stc_selectSlot[ii]:registerPadEvent(__eConsoleUIPadEvent_Up_A, "InputPadA_CharacterChallengeInfo_SelectItem(" .. key32 .. "," .. alterii .. ")")
...
function InputMOn_CharacterChallengeInfo_SelectItem(key32, itemIndex)
  ...
  if itemIndex > self._baseRewardSlotCount and nil ~= self._rewardTable[key32] and nil ~= self._rewardTable[key32][itemIndex] then
...
function InputMLUp_CharacterChallengeInfo_ReceiveBtn(key32)
  ...
  if _ContentsGroup_ChallengeReward then
    if 1 < existRewardCount and stackable then
      Panel_NumberPad_Show(true, toInt64(0, existRewardCount), challengeKey, PaGlobal_CharacterChallengeInfo_GetChallengeReward, false, key32)
    else
      ToClient_requestChallengeReward(challengeKey, choosenRewardIndex, 1)
      self._ui.list2:requestUpdateByKey(toInt64(0, key32))
    end
  else
...
function PaGlobal_CharacterChallengeInfo_GetChallengeReward(inputNumber, challengeKey, key32)
  local self = CharacterChallengeInfo
  local choosenRewardIndex
  if nil ~= self._currentRewardItem then
    choosenRewardIndex = self._currentRewardItem - self._baseRewardSlotCount - 1
  ...
  ToClient_requestChallengeReward(challengeKey, choosenRewardIndex, receiveCount)
  self._ui.list2:requestUpdateByKey(toInt64(0, key32))
end
...
function PaGlobalFunc_CharacterChallengeInfoTab_PadControl(index)
  ...
  local firstContent = UI.getChildControl(self._ui.list2, "List2_Challenges_Content_0")
  if nil ~= firstContent then
    local stc_BG = UI.getChildControl(firstContent, "Static_Reward_BG")
    local stc_rewardArea = UI.getChildControl(stc_BG, "Static_RewardArea")
    local stc_baseSlotBG = UI.getChildControl(stc_rewardArea, "Static_Basic_Item0")
    local stc_baseSlot = UI.getChildControl(stc_baseSlotBG, "Static_Slot")
    ToClient_padSnapChangeToTarget(stc_baseSlot)
  end
end
