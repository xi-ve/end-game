...
function CashBuff:init()
  self._ui.list2:registEvent(__ePAUIList2EventType_LuaChangeContent, "PaGlobalFunc_CashBuff_ListContent")
...
function CashBuff:update()
  ...
  if true == _contentsGroup_BookOfCombat then
    flags[BUFF_TYPE.BookOfCombat] = bookOfCombat
  end
  self._buffData = {}
  self:updateBuffData()
  self._listShowBuff = {}
  ...
  for buffType = 1, self._buffTypeCount do
    ...
    elseif buffType == BUFF_TYPE.Kamasylvia then
      ...
      if isGameTypeRussia() then
        ...
        desc = leftTime .. "\n" .. PAGetString(Defines.StringSheet_GAME, "LUA_SELFPLAYEREXPGAGE_YAZBUFF_DESC_CONSOLE")
      end
    ...
    elseif buffType == BUFF_TYPE.ValuePack then
      ...
      elseif isGameTypeID() then
        ...
        name = PAGetString(Defines.StringSheet_GAME, "LUA_SELFPLAYEREXPGAGE_EILEENBUFF_TITLE")
        desc = leftTime .. "\n" .. PAGetString(Defines.StringSheet_GAME, "LUA_CHARACTERINFO_CASHBUFF_VALUEPACKAGE_DESC")
      end
    ...
    elseif buffType == BUFF_TYPE.BookOfCombat then
      ...
      uv = _iconUV[BUFF_TYPE.BookOfCombat]
    end
