...
function Alchemy_Show(_isCook)
  ...
  if getScreenSizeX() / 2 < invenSizeX + Panel_Alchemy:GetSizeX() / 2 then
    Panel_Alchemy:SetPosX(getScreenSizeX() - invenSizeX - Panel_Alchemy:GetSizeX() + 20)
  end
  if true == _ContentsGroup_NewUI_Inventory_All then
  ...
  _uiAlchemyDesc:SetTextMode(__eTextMode_AutoWrap)
  if _isCook == true then
    ...
    PaGlobal_Util_RegistHelpTooltipEvent(_buttonQuestion, "\"PanelCook\"")
    ...
    PaGlobal_Util_RegistHelpTooltipEvent(_buttonQuestion, "\"PanelAlchemy\"")
    audioPostEvent_SystemUi(12, 11)
  end
  RecnetCookSlotClear()
...
function Alchemy_Close()
  if Panel_Alchemy:IsShow() then
    ...
    if true == _ContentsGroup_NewUI_Inventory_All then
      Panel_Window_Inventory_All:SetShow(false, true)
    else
...
function RecentCookInit()
  ...
  _alchemyDesc:SetTextMode(__eTextMode_AutoWrap)
...
function RecentCookSetup(recipeCountIndex)
  ...
  if nil ~= AlchemyRecordWrapper and 0 < materialCount then
    for itemIndex = 0, materialCount - 1 do
      ...
      local slotNo
      if true == _ContentsGroup_NewUI_Inventory_All then
        slotNo = PaGlobalFunc_Inventory_All_RecentCookItemCheck(itemKey, itemCount)
      else
        slotNo = FGlobal_RecentCookItemCheck(itemKey, itemCount)
      end
      if nil == slotNo then
    ...
    if isProcessingCheck then
      for itemIndex = 0, materialCount - 1 do
        ...
        if nil ~= slotNo then
          RequestAlchemy_PushItemFromInventory(slotNo, itemCount)
          Alchemy_UpdateSlot()
        end
      end
    end
...
function RecentCook_registMessageHandler()
  ...
  list2:registEvent(__ePAUIList2EventType_LuaChangeContent, "Alchemy_ListControlCreate")
