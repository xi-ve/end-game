...
function Material:clearXXX()
  self.slotNo = __eTInventorySlotNoUndefined
  self.itemWhereType = CppEnums.ItemWhereType.eCount
  self.itemKey = 0
end
function Material:checkEmpty()
  return __eTInventorySlotNoUndefined == self.slotNo or CppEnums.ItemWhereType.eCount == self.itemWhereType or 0 == self.itemKey
end
...
function manufacture:checkToDo(showMessage)
  ...
  if true == IsSelfPlayerSwimmingAction() then
    ...
    manufacture:close()
    if nil ~= Panel_Manufacture_Notify and true == ManufactureNotifier:checkShow() then
      ManufactureNotifier:close()
      return
    end
  end
...
function manufacture:checkValidAction(actionIndex)
  return 0 <= actionIndex and actionIndex < self._actionListSize
end
...
function manufacture:createListItem(content, key)
  ...
  buttonControl:SetTextMode(__eTextMode_LimitText)
...
function manufacture:initialize()
  ...
  self._ui.list2:registEvent(__ePAUIList2EventType_LuaChangeContent, "PaGlobalFunc_ManufactureCreateListItem")
  ...
  self._ui.actionDescControl:SetTextMode(__eTextMode_AutoWrap)
  ...
  self._ui.materialDescControl:SetTextMode(__eTextMode_AutoWrap)
...
function manufacture:checkMassActionEnabled(actionIndex)
  ...
  if actionIndex < 0 or 5 < actionIndex then
    return false
  end
  return true
end
...
function manufacture:selectMultipleMaterials()
  ...
  if true == self._isFromWarehouse then
    if _ContentsGroup_NewUI_WareHouse_All then
      PaGlobal_Warehouse_All_SetFunctor(PaGlobalFunc_ManufactureFilterWarehouse, PaGlobalFunc_ManufactureRightClickWarehouse)
    else
...
function manufacture:open(installationType, initFlag)
  ...
  if true == ToClient_SniperGame_IsPlaying() or true == Panel_Global_Manual:GetShow() or true == Panel_Fishing:GetShow() then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_MENU_PVPBATTLEGROUND_CONDITION_WAIT"))
    return
  end
  if true == IsSelfPlayerSwimmingAction() then
...
function manufacture:checkActionVisible(actionIndex)
  ...
  if 0 <= actionIndexUsingInstallation and actionIndex ~= actionIndexUsingInstallation then
    return false
  end
  return true
end
...
function manufacture:updateMaterial()
  ...
  if true == _ContentsGroup_NewUI_WareHouse_All then
    if _ContentsGroup_RenewUI_Inventory and not PaGlobal_WareHouse_All_GetShow() then
      PaGlobalFunc_InventoryInfo_InitMultipleSelect()
    end
  elseif _ContentsGroup_RenewUI_Inventory and not PaGlobalFunc_Warehouse_GetShow() then
    PaGlobalFunc_InventoryInfo_InitMultipleSelect()
  end
  for i = 1, table.getn(self._materialTable) do
    local material = self._materialTable[i]
...
function manufacture:updateButton()
  ...
  if true == _ContentsGroup_NewUI_WareHouse_All then
    showTabNavButton = showTabNavButton and not PaGlobal_WareHouse_All_GetShow()
  else
    showTabNavButton = showTabNavButton and not PaGlobalFunc_Warehouse_GetShow()
  end
