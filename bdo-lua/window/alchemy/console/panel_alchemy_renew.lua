...
function alchemy:initialize()
  ...
  self._ui.tipControl:SetTextMode(__eTextMode_AutoWrap)
  ...
  self._ui.list2:registEvent(__ePAUIList2EventType_LuaChangeContent, "PaGlobalFunc_AlchemyUpdateListItem")
...
function alchemy:initKnowledge()
  ...
  ToClient_AlchemySetKnowledgeFilter(self._knownKnowledgeOnlyFlag, isOnlyName)
...
function alchemy:open(isCook, installationType)
  ...
  if true == _ContentsGroup_NewUI_Inventory_All then
    PaGlobalFunc_Inventory_All_SetInventoryDragNoUse(self._panel)
  else
...
function alchemy:updateMaterial()
  ...
  for i = 1, table.getn(self._materialSlotItemTable) do
    ...
    local showMaterialFlag = __eTInventorySlotNoUndefined ~= slotNo
