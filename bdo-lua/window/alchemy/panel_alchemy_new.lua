...
function PaGlobal_Alchemy:initialize()
  ...
  ui._staticTextAlchemyTips:SetTextMode(__eTextMode_AutoWrap)
  ui._staticTextAlchemyDesc:SetTextMode(__eTextMode_AutoWrap)
  ...
  ui._listKnowledge:registEvent(__ePAUIList2EventType_LuaChangeContent, "Panel_Alchemy_CreateKnowledgeListContent")
  ui._listKnowledge:createChildContent(__ePAUIList2ElementManagerType_List)
...
function PaGlobal_Alchemy:showPanel(isCook, installationType)
  ...
  if true == isCook then
    ...
    PaGlobal_Util_RegistHelpTooltipEvent(ui._buttonQuestion, "\"PanelCook\"")
    ...
    PaGlobal_Util_RegistHelpTooltipEvent(ui._buttonQuestion, "\"PanelAlchemy\"")
  ...
  if true == _ContentsGroup_NewUI_Inventory_All then
    PaGlobalFunc_Inventory_All_SetInventoryDragNoUse(Panel_Alchemy)
  else
...
function PaGlobal_Alchemy:updateKnowledgeList()
  ...
  ToClient_AlchemySetKnowledgeFilter(ui._checkButtonLearntOnly:IsCheck(), false, self._filterText)
...
function PaGlobal_Alchemy:selectKnowledge(knowledgeIndex)
  ...
  if nil ~= mentalCardStaticWrapper then
    ...
    if true == isLearn then
      ...
      if invenSizeX > (getScreenSizeX() - (Panel_Alchemy:GetSizeX() + Panel_RecentCook:GetSizeX())) * 0.5 then
...
function PaGlobal_Alchemy:handleMouseLUp_SearchButton()
  ...
  local isOnlyName = false
  ToClient_AlchemySetKnowledgeFilter(true, isOnlyName, self._filterText)
  self:updateKnowledgeList()
end
