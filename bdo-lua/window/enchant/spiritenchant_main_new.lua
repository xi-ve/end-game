...
function PaGlobal_Enchant:initialize()
  ...
  self._ui._txt_EatDesc:SetTextMode(__eTextMode_AutoWrap)
  ...
  self._ui._statictext_EnchantInfoDesc:SetTextMode(__eTextMode_AutoWrap)
  for dIndex = 0, 10 do
  ...
  self._ui._blablaText:SetTextMode(__eTextMode_AutoWrap)
  local slotConfig = {
    createIcon = false,
    createBorder = true,
    createCount = true,
  ...
  self._ui._statictext_BounsStats:SetTextMode(__eTextMode_AutoWrap)
  self._ui._statictext_StackForNext:SetTextMode(__eTextMode_AutoWrap)
  ...
  self._ui._statictext_noticeApplyButton:SetTextMode(__eTextMode_AutoWrap)
  self._ui._statictext_noticeApplyButton:SetAutoResize(true)
  self._ui._statictext_EnchantResult:SetTextMode(__eTextMode_AutoWrap)
  self._ui._button_SecretExtract:SetTextMode(__eTextMode_AutoWrap)
  self._ui._button_SecretExtract:SetText(self._ui._button_SecretExtract:GetText())
  self._ui._statictext_SafeEnchant:SetTextMode(__eTextMode_AutoWrap)
  ...
  PaGlobal_Util_RegistHelpTooltipEvent(self._ui._button_Question, "\"SpiritEnchant\"")
  ...
  self._ui._statictext_AddStat[0]:SetTextMode(__eTextMode_LimitText)
  self._ui._statictext_AddStat[1]:SetTextMode(__eTextMode_LimitText)
  self._ui._statictext_AddStat[2]:SetTextMode(__eTextMode_LimitText)
  self._ui._statictext_AddStat[3]:SetTextMode(__eTextMode_LimitText)
...
function PaGlobal_Enchant:init_EnchantFrame()
  ...
  self._ui._checkbox_UseCron:SetTextMode(__eTextMode_AutoWrap)
...
function PaGlobal_Enchant:showPanel()
  ...
  if false == _ContentsGroup_RenewUI_InventoryEquip then
    if true == _ContentsGroup_NewUI_Equipment_All then
      ...
      Panel_Window_Equipment_All:SetPosX(self._const._gapX_PanelEquip)
...
function PaGlobal_Enchant:initCronLevelAndCountText(maxLevel)
  ...
  self._ui._topDesc2:SetTextMode(__eTextMode_AutoWrap)
...
function PaGlobal_Enchant:hideAllCronCountText()
  ...
  self._ui._topDesc:SetTextMode(__eTextMode_AutoWrap)
...
function PaGlobal_Enchant:showDifficulty(inventoryType, slotNo)
  ...
  local rate = ToClient_getEnchantSuccessRate(self._ui._checkbox_ForcedEnchant:IsCheck(), self:isDifficultEnchant(), __eEnchantFailCountSlotNo_Undefined)
...
function PaGlobal_Enchant:showNoticeEnchantApply(enchantType)
  if enchantType == self._enum_EnchantType._safe then
    ...
    if (true == isWeapon or true == isCollectTool) and 8 <= enchantLevel or 6 <= enchantLevel then
      return
    end
...
function PaGlobal_Enchant:setEnchantTarget(slotNo, itemWrapper, inventoryType, resultType, isMonotone)
  ...
  if CppEnums.ItemClassifyType.eItemClassify_Accessory == equipType then
    if 3 <= currentEnchantLevel then
      blablaShow = true
    end
  elseif 18 <= currentEnchantLevel then
    blablaShow = true
  end
...
function PaGlobal_Enchant:didsetEnchantTarget(isMonotone, isRadioClick)
  ...
  if (16 < enchantLevel or 4 == enchantItemClassify) and false == isStackLessBlackStone and toInt64(0, 0) < self._enchantInfo:ToClient_getNeedCountForProtect_s64() then
...
function PaGlobal_Enchant:cancelEnchant()
  ...
  self:setAsEnchantButton()
  audioPostEvent_SystemUi(5, 99)
  PaGlobal_Enchant._ui._checkbox_skipEffect:SetIgnore(false)
  ToClient_BlackspiritEnchantCancel()
end
...
function PaGlobal_Enchant:handleMOnEnchantMaterialTooltip()
  ...
  elseif __eTInventorySlotNoUndefined == self._ui._slot_EnchantMaterial.inventoryType then
    local needSSW
...
function OnScreenEvent()
  ...
  Panel_Window_Enchant:SetPosY(invenPosY)
end
...
function PaGlobal_Enchant_IsAnimating()
  return PaGlobal_Enchant._isAnimating
end
...
function UpdateFunc_checkAnimation(deltaTime)
  ...
  if true == self._isAnimating then
    ...
    if self._const._effectTime_Enchant <= self._animationTimeStamp then
      self._isAnimating = false
      self._ui._checkbox_skipEffect:SetIgnore(false)
      self._ui._radiobutton_EnchantTab:SetIgnore(false)
      self._ui._radiobutton_CronEnchantTab:SetIgnore(false)
      self._ui._radiobutton_EatTab:SetIgnore(false)
      if true == self:isEnchantTab() then
    ...
    self._ui._checkbox_skipEffect:SetIgnore(true)
