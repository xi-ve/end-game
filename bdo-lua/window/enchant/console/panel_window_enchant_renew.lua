...
function EnchantInfo:initialize()
  ...
  self._ui.txt_descSub:SetTextMode(__eTextMode_AutoWrap)
  ...
  self._ui.txt_normalEnchantDesc:SetTextMode(__eTextMode_AutoWrap)
  ...
  self._ui.txt_forceEnchantDesc:SetTextMode(__eTextMode_AutoWrap)
  ...
  self._ui.txt_result:SetTextMode(__eTextMode_AutoWrap)
  ...
  if false == _ContentsGroup_EatEnchant then
    UI.getChildControl(self._ui.stc_topMenu, "Static_RB"):SetShow(false)
    UI.getChildControl(self._ui.stc_topMenu, "Static_LB"):SetShow(false)
    self._ui.rdo_tabButtons[2]:SetShow(false)
    self._ui.rdo_tabButtons[1]:SetShow(false)
  end
...
function EnchantInfo:setText_EnchantInfo()
  if true == self._isSetNewPerfectItemMaterial then
    ...
    local enchantType = self._enchantInfo:ToClient_getEnchantType()
    local rate = ToClient_getEnchantSuccessRate(false, self:isDifficultEnchant(), __eEnchantFailCountSlotNo_Undefined)
...
function FromClient_EnchantInfo_ResultShow(resultType, mainWhereType, mainSlotNo, subWhereType, subSlotNo)
  ...
  EnchantInfo:showEffectByResult(resultType, mainSlotNo, mainWhereType)
...
function EnchantInfo:repositionDesc()
  ...
  if 0 < tempSizeY or 0 < tempSizeY2 then
...
function EnchantInfo:setItemToSlotMonoTone(slot, itemStaticWrapper)
  ...
  slot.empty = false
  slot.slotNo = 0
  slot.inventoryType = __eTInventorySlotNoUndefined
