...
function socketInfo:initialize()
  ...
  for ii = 1, self.config.socketSlotCount do
    self._ui.staticSocketName[ii]:SetTextMode(__eTextMode_AutoWrap)
    self._ui.staticSocketDesc[ii]:SetTextMode(__eTextMode_AutoWrap)
  end
...
function socketInfo:createControl()
  ...
  for ii = 1, self.config.socketSlotCount do
    ...
    slotSocket.desc:SetTextMode(__eTextMode_AutoWrap)
...
local function Socket_Pop_Confirm()
  ...
  Socket_PopJewelFromSocket(self._currentSocket - 1, CppEnums.ItemWhereType.eCount, __eTInventorySlotNoUndefined)
  self._ui.txt_keyGuideDestroy:SetMonoTone(false ~= self.slotSocket[self._currentSocket].empty or not _snappedOnThisPanel)
end
