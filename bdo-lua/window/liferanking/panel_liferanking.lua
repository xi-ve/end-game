...
PaGlobal_Util_RegistHelpTooltipEvent(LifeRanking._btnHelp, "\"LifeRanking\"")
local tooltip = {
...
function LifeRanking:Update()
  ...
  for listIdx = LifeRanking._startIndex, LifeRanking._listCount - 1 do
    ...
    if 0 == listIdx then
      ...
      if LifeRanking._selectedTabIdx <= 9 then
        LifeRanking._topGrade[1]:SetText(PaGlobalFunc_Util_CraftLevelReplace(lifeRankerLv))
      elseif 10 == LifeRanking._selectedTabIdx then
        LifeRanking._topGrade[1]:SetShow(false)
      elseif 11 == LifeRanking._selectedTabIdx then
    ...
    elseif 1 == listIdx then
      ...
      if LifeRanking._selectedTabIdx <= 9 then
        LifeRanking._topGrade[2]:SetText(PaGlobalFunc_Util_CraftLevelReplace(lifeRankerLv))
      elseif 10 == LifeRanking._selectedTabIdx then
        LifeRanking._topGrade[2]:SetShow(false)
      elseif 11 == LifeRanking._selectedTabIdx then
        local levelFlot = levelFlotFunc(lifeRanker)
    ...
    elseif 2 == listIdx then
      ...
      if LifeRanking._selectedTabIdx <= 9 then
        LifeRanking._topGrade[3]:SetText(PaGlobalFunc_Util_CraftLevelReplace(lifeRankerLv))
      elseif 10 == LifeRanking._selectedTabIdx then
        LifeRanking._topGrade[3]:SetShow(false)
      elseif 11 == LifeRanking._selectedTabIdx then
        local levelFlot = levelFlotFunc(lifeRanker)
      ...
      if LifeRanking._selectedTabIdx <= 9 then
        list.grade:SetText(PaGlobalFunc_Util_CraftLevelReplace(lifeRankerLv))
      elseif 10 == LifeRanking._selectedTabIdx then
        list.grade:SetShow(false)
      elseif 11 == LifeRanking._selectedTabIdx then
        local levelFlot = levelFlotFunc(lifeRanker)
...
function FromClient_ShowLifeRank()
  if _ContentsGroup_isUsedNewCharacterInfo == true and 0 < requestCount then
...
function FromClient_ShowContentsRank(contentsRankType)
  if ToClient_IsInstanceFieldPlayerbyContentsType(__eInstanceContentsType_SavageDefence) then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_BLOODALTAR_DONTOPENLIFERANKING"))
    return
  end
