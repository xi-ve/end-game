function Panel_FixEquip_InteractortionFromInventory(slotNo, itemWrapper, count, inventoryType)
  ...
  elseif self._slotSub.empty then
    ...
    local repairAmount = ToClient_getRepairMaxEnduranceCount(self._slotMain.itemKey, itemWrapper:get():getKey())
    local isSameItem = self._slotMain.itemKey:getItemKey() == self._slotSub.itemKey
    if 44195 == self._slotSub.itemKey then
      repairAmount = self._memoryFlagRecoveryCount
    elseif true == isSameItem then
      repairAmount = 10
    end
    self._enduranceText:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_FIXEQUIP_RECOVERYCOUNT", "count", tostring(repairAmount)))
...
function PaGlobal_FixEquip:fixEquip_ApplyButton(isHelpRepair)
  ...
  if nil ~= self._slotMain.slotNo and nil ~= self._slotSub.slotNo then
    ...
    local subItemWrapper = getInventoryItemByType(self._slotSub.whereType, self._slotSub.slotNo)
    local repairAmount = ToClient_getRepairMaxEnduranceCount(self._slotMain.itemKey, subItemWrapper:get():getKey())
    if self._onlyItemCheck then
      local count = 10
      if isMemoryFlag then
        count = memoryFlagRecoveryCount
      elseif false == isSameItem then
        count = repairAmount
      end
      if true == isHelpRepair then
        count = count * 4
      end
      contentString = PAGetString(Defines.StringSheet_GAME, "LUA_FIXEQUIP_ONLYITEMCHECK_CONTENTSTRING") .. PAGetStringParam1(Defines.StringSheet_GAME, "LUA_FIXEQUIP_CONTROL_MAXENDURANCERECOVERYCOUNT", "count", count)
...
function FixEquip_InvenFiler_MainItem(slotNo, itemWrapper)
  ...
  if nil == itemWrapper:getStaticStatus():get()._repairEnduranceCount then
    return true
  end
...
function PaGlobal_FixEquip:fixEquip_Show()
  ...
  if Panel_FixEquip:IsShow() == false then
    ...
    if true == _ContentsGroup_NewUI_Inventory_All then
      Panel_Window_Inventory_All:SetPosX(getScreenSizeX() - Panel_Window_Inventory_All:GetSizeX())
    else
...
function FixEquip_Resize()
  ...
  if repairPanel:GetShow() then
    local dialogY = repairPanel:GetSizeY()
