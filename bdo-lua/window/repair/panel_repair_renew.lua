...
function RepairInfo:open()
  self._isCamping = PaGlobalFunc_Camp_All_GetIsCamping()
  ...
  self._ui.btn_repairs[REPAIR_TYPE.MAX_ENDURANCE]:SetShow(not self._isCamping)
  self._ui.btn_repairs[REPAIR_TYPE.SERVANT]:SetShow(not self._isCamping)
  self._ui.btn_repairs[REPAIR_TYPE.SHIP]:SetShow(not self._isCamping)
...
function PaGlobalFunc_RepairInfo_InvenRClick(slotNo, itemWrapper, s64_itemCount, itemWhereType)
  ...
  local repairPrice = repair_getRepairPrice_s64(itemWhereType, slotNo, CppEnums.ServantType.Type_Count, self._isCamping)
...
function PaGlobalFunc_RepairInfo_EquipRClick(equipSlotNo, itemWrapper)
  ...
  local repairPrice = repair_getRepairPrice_s64(CppEnums.ItemWhereType.eEquip, equipSlotNo, CppEnums.ServantType.Type_Count, self._isCamping)
  if repairPrice > Defines.s64_const.s64_0 then
    local strPrice = string.format("%d", Int64toInt32(repairPrice))
    local messageboxMemo = PAGetStringParam1(Defines.StringSheet_GAME, "REPAIR_MESSAGEBOX_MEMO", "price", makeDotMoney(strPrice))
    local messageboxData = {
...
function RepairInfo:onClickEquipRepair()
  local s64_totalPrice = repair_RepairAllPrice_s64(CppEnums.ItemWhereType.eEquip, true, CppEnums.ServantType.Type_Count, false, self._isCamping)
  if s64_totalPrice > Defines.s64_const.s64_0 then
    local strPrice = string.format("%d", Int64toInt32(s64_totalPrice))
    local messageboxMemo = PAGetStringParam1(Defines.StringSheet_GAME, "REPAIR_EQUIP_MESSAGEBOX_MEMO", "price", makeDotMoney(strPrice))
    local messageboxData = {
...
function RepairInfo:onClickInvenRepair()
  local inventory_s64 = repair_RepairAllPrice_s64(CppEnums.ItemWhereType.eInventory, true, CppEnums.ServantType.Type_Count, false, self._isCamping)
  local totalPrices_64 = repair_RepairAllPrice_s64(CppEnums.ItemWhereType.eCashInventory, false, CppEnums.ServantType.Type_Count, false, self._isCamping)
