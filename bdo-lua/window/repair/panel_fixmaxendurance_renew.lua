...
function PaGlobalFunc_CloseFixMaxEnduranceInfo()
...
function PaGlobalFunc_FixMaxEnduranceInfo_PickTargetOrSubject(slotNo, itemWrapper, count, inventoryType)
  ...
  elseif nil == self._ui.slot_subjectItem.slotNo then
    ...
    self._subjectItemKey = itemWrapper:get():getKey()
    local repairAmount = ToClient_getRepairMaxEnduranceCount(self._ui.slot_targetItem.itemKey, self._subjectItemKey)
    local isSameItem = self._ui.slot_targetItem.itemKey:getItemKey() == self._ui.slot_subjectItem.itemKey
    ...
    elseif false == isSameItem then
      if nil == repairAmount or 0 == repairAmount then
        repairAmount = 10
      end
      self._ui.txt_expectedVal:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_FIXEQUIP_RENEW_RECOVERY") .. PAGetStringParam1(Defines.StringSheet_GAME, "LUA_FIXEQUIP_RECOVERYCOUNT", "count", tostring(repairAmount)))
    else
...
function FixMaxEnduranceInfo:fixEquip_ApplyButton(isHelpRepair)
  ...
  local function funcYesExe()
    ...
    local cashInvenSize = getSelfPlayer():get():getInventorySlotCount(true)
    ...
    for invenIdx = 0, cashInvenSize - 1 do
      ...
      if nil ~= itemWrapper then
        ...
        if nil ~= item then
          ...
          if nil ~= itemkey and (17598 == itemkey or 17729 == itemkey) then
            cashItemCount = Int64toInt32(itemWrapper:getCount())
          end
        end
      end
    end
    if self._isAutoRepeat then
      ...
      local maxRecoverEnduarance = targetItemWrapper:getStaticStatus():get():getMaxEndurance() - targetItemWrapper:get():getMaxEndurance()
      local maxRepeatCount = 0
      if self._useCash then
        targetmemoryFlagRecoveryCount = 4 * targetmemoryFlagRecoveryCount
      end
      if 44195 == subjectItemKey then
        local memoryFragmentItemWrapper = getInventoryItemByType(self._ui.slot_subjectItem.whereType, self._ui.slot_subjectItem.slotNo)
        local memoryFragmentCount = Int64toInt32(memoryFragmentItemWrapper:getCount())
        maxRepeatCount = math.ceil(maxRecoverEnduarance / targetmemoryFlagRecoveryCount)
        maxRepeatCount = math.min(maxRepeatCount, memoryFragmentCount)
        if self._useCash then
          maxRepeatCount = math.min(maxRepeatCount, cashItemCount)
        end
        ...
        for ii = __eTInventorySlotNoUseStart, invenMaxSize - 1 do
          ...
          if false == bool then
            ...
            if nil ~= item then
              local itemkey = item:getKey():getItemKey()
              if nil ~= itemkey and 44195 ~= itemkey then
                slotNoTable[#slotNoTable + 1] = ii
              end
            end
          end
        end
        maxRepeatCount = math.ceil(maxRecoverEnduarance / 10)
        maxRepeatCount = math.min(#slotNoTable, maxRepeatCount)
        if self._useCash then
          maxRepeatCount = math.min(maxRepeatCount, cashItemCount)
        end
        for jj = 1, maxRepeatCount do
  ...
  if nil ~= self._ui.slot_targetItem.slotNo and nil ~= self._ui.slot_subjectItem.slotNo then
    ...
    elseif isDriganFlag and false == isHelpRepair then
      ...
      local repairAmount = ToClient_getRepairMaxEnduranceCount(self._ui.slot_targetItem.itemKey, self._subjectItemKey)
      local isSameItem = self._ui.slot_targetItem.itemKey:getItemKey() == self._ui.slot_subjectItem.itemKey
      if nil == repairAmount or 0 == repairAmount or isSameItem then
        repairAmount = 10
      end
      if isHelpRepair then
        contentString = PAGetString(Defines.StringSheet_GAME, "LUA_FIXEQUIP_ONLYITEMCHECK_CONTENTSTRING") .. "\n" .. PAGetStringParam1(Defines.StringSheet_GAME, "LUA_FIXEQUIP_CONTROL_MAXENDURANCERECOVERYCOUNT", "count", repairAmount * 4)
      else
        contentString = PAGetString(Defines.StringSheet_GAME, "LUA_FIXEQUIP_ONLYITEMCHECK_CONTENTSTRING") .. "\n" .. PAGetStringParam1(Defines.StringSheet_GAME, "LUA_FIXEQUIP_CONTROL_MAXENDURANCERECOVERYCOUNT", "count", repairAmount)
      end
    end
