...
function CombatRoutine.new()
    ...
    self.Dash:forceReuseCycle(1000)
    self.SkillActions = {
        self.Transquility,
        self.Dragonize,
        self.Sea_Burial,
...
function CombatRoutine:onMoveTo(dest, isNode)
    if Infinity.BDO.getLocalPlayer():getCombatStance() == 2 then
        ...
        if isNode then
            if distanceToDest < 900 and distanceToDest > 300 and self.Dash:canUse(nil) then
...
function CombatRoutine:combat(targetKey)
    ...
    if target then
        ...
        if distToTarget < 900 and distToTarget > 500 and self.Dash:canUse(target) then
            self.Dash:use(target)
        elseif self.Transquility:canUse(target) and Player.getWpPercent() < 50 then
