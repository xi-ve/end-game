MagLib.BDO.Navigation.Graph.NavGraphHandler = function(name)
	local handler = {}
	local Vec = MagLib.Utils.Vector
	handler.graph = {nodes = {}, edges = {}}
	handler.Node = MagLib.BDO.Navigation.Graph.Node
	...
	handler.getClosestNode = function(pos, use3dDistance, okDistance, filterFunc)
		...
		for _, node in pairs(handler.graph.nodes) do
			...
			if distance <= okDistance and (not filterFunc or filterFunc(node)) then
				return node, distance
			end

			if distance < closestNodeDistance and (not filterFunc or filterFunc(node)) then
				closestNode = node
	...
	handler.getClosestElement_2DOnScreen = function(pos, maxDistance, okDistance)
		...
		for _, node in pairs(nodes) do
			...
			if nodePos then
				local dist = Vec.Distance2(pos, nodePos)
		...
		for _, edge in pairs(edges) do
			...
			local to = edge.to
			local toMiddleOfEdge = Vec.divide(Vec.sub(to.pos, from.pos), 2)
			-- Giving 3 points on the edge
			local edgePos3D_1 = Vec.add(from.pos, Vec.multiply(toMiddleOfEdge, 1.25))
			local edgePos3D_2 = Vec.add(from.pos, Vec.multiply(toMiddleOfEdge, 0.75))
			local edgePos3D_3 = Vec.add(from.pos, toMiddleOfEdge)
			...
			if edgePos_1 then
				dist = Vec.Distance2(pos, edgePos_1)
				if dist < closestDist and dist < maxDistance then
					closest = edge
					closestDist = dist
					isNode = false
			...
			if not set then
				...
				if edgePos_2 then
					dist = Vec.Distance2(pos, edgePos_2)
					if dist < closestDist and dist < maxDistance then
						closest = edge
						closestDist = dist
						isNode = false
			...
			if not set then
				...
				if edgePos_3 then
					dist = Vec.Distance2(pos, edgePos_3)
					if dist < closestDist and dist < maxDistance then
						closest = edge
						closestDist = dist
						isNode = false
	...
	handler.find_edge = function(a, b)
		...
		for _, edge in ipairs(edges_list) do
			if (edge.from == a and edge.to == b) or (edge.from == b and edge.to == a) then
				edges_list = nil
				return edge
			end
	...
	handler.addEdge = function(from, to, weight, log)
        ...
        if log then
            print("[" .. handler.graphName .. "] Adding Edge from " .. from.index .. " to " .. to.index)
        end]]
		handler.edge_size = handler.edge_size + 1
		table.insert(handler.graph.edges, {from = from, to = to, weight = weight or 0})
	end
		...
		for k, node in pairs(handler.graph.nodes) do
			if rootNode ~= node then
				...
				if distance > minDist and distance < maxDist then
					local ok = true
					if raycast and MagLib.BDO.Raycast.safeTwoPos(rootNodePos, nodePos, 150, 30) then
						ok = false
					end
	...
	handler.removeNode = function(node)
		...
		table.sort(edgeIndexesToRemove, function(a, b)
			return a > b
		end)
	...
	handler.addNode = function(position, log)
		...
		handler.node_size = handler.node_size + 1
		local newIndex = MagLib.Utils.Table.firstBorder(handler.graph.nodes) + 1
		handler.graph.nodes[newIndex] = handler.Node(newIndex)
		handler.graph.nodes[newIndex].pos = position
		if log then
			print("[" .. handler.graphName .. "] Added node #" .. tostring(newIndex))
		end
		return newIndex
	end
