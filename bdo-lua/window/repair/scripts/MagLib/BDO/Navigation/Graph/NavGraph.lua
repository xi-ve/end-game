MagLib.BDO.Navigation.Graph.NavGraph = function()
	local _n = {}
	_n.Node = MagLib.BDO.Navigation.Graph.Node
	...
	_n.updateMinDistanceBetweenNodes(300)
	...
	_n.resetGraph = function()
		_n.graph_handler = MagLib.BDO.Navigation.Graph.NavGraphHandler("Main Graph")
		_n.key_graph_handler = MagLib.BDO.Navigation.Graph.NavGraphHandler("Key Graph")
	end
	...
	-- Pathfinders +
	_n.pathfinder = {}
	_n.pathfinder.graph = nil
	_n.pathfinder.key_graph = nil

	_n.pathfinder.resetPathfinders = function()
		_n.pathfinder.graph = MagLib.BDO.Navigation.Graph.Pathfinding.AStarPathfinder()
		_n.pathfinder.key_graph = MagLib.BDO.Navigation.Graph.Pathfinding.AStarPathfinder()
	...
	_n.recalculateT2Edges = function()
		...
		for k, _node in pairs(key_graph_nodes) do
	...
	_n.processAutoAdd_higherNodes = function()
		local pos = MagLib.BDO.Actors.LocalPlayer.getPos()
		...
		if not _n.pseudoMeshCreation or _n.forceAddT1Node then
			...
			while done == false do
				...
				for _, extraPos in ipairs(possiblePos) do
					if not MagLib.BDO.Raycast.safeTwoPos(pos, extraPos, 50, 30) then
						local extraPos_lowered = Vector3(extraPos.X, extraPos.Y - 25, extraPos.Z)
						if not MagLib.BDO.Raycast.twoPos(pos, extraPos_lowered, 50) then
