...
function Panel_Window_StroyInstanceDungeon_info:initLoadMovie()
  ...
  if _ContentsGroup_RenewUI then
    Panel_MovieStroyIndun:SetVerticalMiddle()
    Panel_MovieStroyIndun:SetHorizonCenter()
  else
  ...
  if nil == self._ui.cutSceneMovie then
    self._ui.cutSceneMovie = UI.createControl(__ePAUIControl_WebControl, Panel_MovieStroyIndun, "Static_StroyMovie")
  end
  if nil ~= self._ui.cutSceneMovie then
