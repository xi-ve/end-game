...
function PaGlobal_ExtractionCrystal:createControl_uiSlotSocket()
  ...
  for ii = 1, self._config.socketSlotCount do
    slotSocket = {
      ...
      descParent = self._uiControl.stc_SocketDescGroup[ii],
      descControl = self._uiControl.staticSocketDesc[ii],
...
function PaGlobal_ExtractionCrystal:removeCrystal(indexSocket)
  ...
  local function remove_crystal_do()
    ...
    Socket_PopJewelFromSocket(indexSocket, CppEnums.ItemWhereType.eCount, __eTInventorySlotNoUndefined)
  end
...
function PaGlobal_ExtractionCrystal:updateSocket()
  ...
  for ii = 1, maxCount do
    ...
    if nil == itemStaticWrapper then
      ...
      PaGlobalFunc_CrystalExtraction_DescSetting(socketSlot, self._desc)
      ...
      if nil ~= jewelSkillStaticWrapper then
        ...
        PaGlobalFunc_CrystalExtraction_DescSetting(socketSlot, descTable)
      end
    end
  end
...
function PaGlobalFunc_CrystalExtraction_DescSetting(socketSlot, descTable)
  ...
  for i = 1, buffDescCount do
    if 1 == buffDescCount then
      descControl[1]:SetTextMode(__eTextMode_Limit_AutoWrap)
      descControl[1]:SetLineCount(3)
    else
      descControl[i]:SetTextMode(__eTextMode_LimitText)
    end
    ...
    descControl[i]:SetIgnore(not descControl[i]:IsLimitText())
    if descControl[i]:IsLimitText() then
      tempTable[i] = {
        control = descControl[i],
        desc = descControl[i]:GetText()
      }
      descControl[i]:addInputEvent("Mouse_On", "PaGlobalFunc_Crystal_Control_TooltipLimitedText(" .. i .. ",true)")
      descControl[i]:addInputEvent("Mouse_Out", "PaGlobalFunc_Crystal_Control_TooltipLimitedText(" .. i .. ",false)")
    end
  end
