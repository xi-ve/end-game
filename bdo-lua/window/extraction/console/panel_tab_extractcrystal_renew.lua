...
function ExtractCrystal:initialize()
  ...
  for ii = 1, self.config.socketSlotCount do
    ...
    self.slotSocket[ii].name:SetTextMode(__eTextMode_Limit_AutoWrap)
    self.slotSocket[ii].desc:SetTextMode(__eTextMode_AutoWrap)
...
function PaGlobalFunc_ExtractCrystal_ApplyRemove()
  ...
  local function remove_crystal_do()
    ...
    Socket_PopJewelFromSocket(self._currentIndex - 1, CppEnums.ItemWhereType.eCount, __eTInventorySlotNoUndefined)
  end
