...
function inGameShopBuy:init()
  ...
  self._radio_PayInven:SetCheck(false == self._bool_payInSilverFromWarehouse)
  self._radio_PayWarehouse:SetCheck(self._bool_payInSilverFromWarehouse)
end
      ...
      if false == isDiscountPearl then
        ...
        if couponMaxDiscount > toInt64(0, 0) then
          if couponMaxDiscount < couponDiscountValue then
            ...
            self._static_DiscountPrice:SetText(tostring(cashProduct:getPrice() - couponDiscountValue))
...
function InGameShopBuy_ResetCount(isCouponApply)
  ...
  local couponIndex = FGlobal_IngameCashShopCoupon_ReturnValue()
  if nil ~= couponIndex and 0 < couponIndex then
    local count = 1
...
function HandleClicked_InGameShopBuy_CheckPayWarehouse(bool)
  ...
  local silver = InGameShopBuy_GetSliver()
  local price = cashProduct:getProductPrice()
  local maxCountPerPrice = Defines.s64_const.s64_0
  if Defines.s64_const.s64_0 ~= silver then
    maxCountPerPrice = silver / price
  end
  local maxBuyCount = InGameShopBuy_GetAndCheck_CashProductMaxCountByPrice(cashProduct)
  local limitCount = cashProduct:getCashPurchaseCount()
  ...
  if Int64toInt32(maxCountPerPrice) < Int64toInt32(maxBuyCount) then
    lastMaxCount = Int64toInt32(maxCountPerPrice)
  else
    lastMaxCount = Int64toInt32(maxBuyCount)
  end
  if count > lastMaxCount then
    count = lastMaxCount
  end
  if count <= 0 then
    count = 1
