...
function CashMileage:registControlEventHandler()
  ...
  for index = 0, #self._ui._list2_mileage do
    self._ui._list2_mileage[index]:registEvent(__ePAUIList2EventType_LuaChangeContent, "PaGlobal_CashMileage_ListCreate")
    self._ui._list2_mileage[index]:createChildContent(__ePAUIList2ElementManagerType_List)
...
function PaGlobal_CashMileage_ConfirmReward()
  ...
  if true == _ContentsGroup_ChallengeReward then
    ToClient_requestChallengeReward(self._mileageInfo[itemKey]:getKey(), 0, 1)
  else
...
function PaGlobal_CashMileage_ListCreate(content, key)
  ...
  local stctxt_rewardedIcon = UI.getChildControl(stc_contentArea, "StaticText_RewardedIcon")
  local gapX = 3
  ...
  stctxt_rewardedIcon:SetShow(false)
  ...
  elseif self._mileageInfo[key32].nowStep == CHALLENGE_STEP.COMPLETED then
    ...
    stc_contentArea:getBaseTexture():setUV(x1, y1, x2, y2)
    stc_contentArea:setRenderTexture(stc_contentArea:getBaseTexture())
    stctxt_rewardedIcon:SetShow(true)
    stctxt_rewarded:SetShow(false)
  end
end
