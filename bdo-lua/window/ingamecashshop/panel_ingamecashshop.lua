...
function inGameShop:setElement(index, productNoRaw, slot)
  ...
  if UI_PLT.None ~= limitType then
    ...
    if 0 < limitCount and mylimitCount == 0 then
  ...
  for ii = 0, self._listCount - 1 do
    ...
    if self:isSelectProductGroup(productNoRaw) then
      ...
      if self._currentPos < pos + slot.static:GetSizeY() + self._goodDescBG:GetSizeY() and pos + slot.static:GetSizeY() < self._currentPos + areaSizeY and 1 < self._goodDescBG:GetSizeY() then
        ...
        self.desc._static_PearOriginalPrice:SetSpanSize(self.desc._static_PearlPrice:GetSpanSize().x + self.desc._static_PearlPrice:GetTextSizeX(), 60)
        self.desc._static_PearlIcon:SetSpanSize(self.desc._static_PearOriginalPrice:GetSpanSize().x + self.desc._static_PearOriginalPrice:GetTextSizeX() + 5, 60)
      end
...
function inGameShop:initTabPos()
  ...
  for ii = 1, self._tabCount do
    ...
    tab.text:SetPosX(40)
    tab.text:SetPosY(5)
    if false == self._isHotDealOpen and ii == self._hotDealTabIndex or false == tabIndexList[ii][8] then
...
function InGameShop_TabEvent(tab)
  ...
  if 0 < tab and tab <= self._tabCount then
...
function makeSubTab(tabIndex)
  ...
  if not (0 < tabIndex) or not (tabIndex <= getCashMainCategorySize()) then
    return
  end
...
function setPosOpenSubTab(tabIndex)
  ...
  for jj = tabIndex + 1, self._tabCount do
    ...
    tab.text:SetPosX(40)
    tab.text:SetPosY(5)
    if false == self._isHotDealOpen and jj == self._hotDealTabIndex or false == tabIndexList[jj][8] then
      tab.static:SetShow(false)
    else
      tab.static:SetShow(true)
      tabPosIndex = tabPosIndex + 1
  ...
  if false == self._isHotDealOpen then
    ...
    self._myCartTab.static:SetPosY(tabConfig._startY + subTabSize + tabConfig._gapY * (tabId.cart - (1 + self._hideTabCount)))
  end
...
function setPosCloseSubTab()
  ...
  if false == self._isHotDealOpen then
    ...
    self._myCartTab.static:SetPosY(tabConfig._startY + tabConfig._gapY * (tabId.cart - (1 + self._hideTabCount)))
  end
...
function _ingameCash_SetTabIconTexture(control, tabIdx, status)
  ...
  if 1 <= tabIdx and tabIdx <= categorySize then
...
function _inGameShop_TabOnOut_ChangeTexture(isOn, tabIdx)
  local self = inGameShop
  local control
  if 0 < tabIdx and tabIdx < self._tabCount + 1 then
...
function FGlobal_InGameShop_OpenByEventAlarm()
  ...
  elseif CppEnums.CountryType.JPN_ALPHA == getGameServiceType() then
    promotionUrl = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_PROMOTIONURL_JP_ALPHA")
    categoryUrl = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_CATEGORYURL_JP_ALPHA")
  elseif CppEnums.CountryType.JPN_REAL == getGameServiceType() then
    promotionUrl = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_PROMOTIONURL_JP")
    categoryUrl = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_CATEGORYURL_JP")
  else
    promotionUrl = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_PROMOTIONURL")
    categoryUrl = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_CATEGORYURL")
  end
...
function IngameCashShop_DescUpdate()
  ...
  if 0 < inGameShop._currentTab and inGameShop._currentTab <= getCashMainCategorySize() and true == FGlobal_IngameCashShopEventCart_IsContentsOpen() then
    ...
    if 0 < eventListKey and true == isOpenEventList and 0 < eventSlotKey and true == isOpenEventSlot then
    ...
    elseif 0 < eventSlotKey and true == isOpenEventSlot then
      self._btn_BigECart:SetShow(false)
      self._btn_BigECartSlot:SetShow(true)
      ...
      self._btn_BigECartSlot:SetEnable(isOnSlotButton)
      self._btn_BigCart:SetShow(false)
    elseif 0 < eventListKey and true == isOpenEventList then
      self._btn_BigECart:SetShow(true)
      self._btn_BigECartSlot:SetShow(false)
      self._btn_BigECart:SetMonoTone(false == isOnListButton)
...
function InGameShop_subItemEvent(index)
  ...
  if 0 < index and index < self._listComboCount + self._skipCount and 0 ~= inGameShop._subItemButton[index].productNo then
...
function ToClient_RequestShowProduct(productNo, price, bValue)
  ...
  if nil ~= cashProduct then
    ...
    IngameCashShop_SelectedItemXXX(productNo, false, bValue)
    for ii = 1, inGameShop._listComboCount do
