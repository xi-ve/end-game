...
function PaGlobal_EasyBuy:Open(uniqueNo, waypointKey, immediatelyUse)
  ...
  if Panel_IngameCashShop_EasyPayment:GetShow() then
    if false == _ContentsGroup_NewUI_Servant_All then
      if false == Panel_Window_StableInfo:GetShow() and nil ~= Panel_Window_StableInfo then
        PaGlobal_EasyBuy:Close()
        return
      end
    elseif false == Panel_Dialog_ServantInfo_All:GetShow() and nil ~= Panel_Dialog_ServantInfo_All then
      PaGlobal_EasyBuy:Close()
      return
    end
  end
