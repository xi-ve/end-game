...
_ItemName:SetTextMode(__eTextMode_Limit_AutoWrap)
_desc:SetTextMode(__eTextMode_AutoWrap)
      ...
      if itemCommerceType == tradeBuyMarket.enCommerceIndex or enCommerceType.enCommerceType_Max == tradeBuyMarket.enCommerceIndex then
        ...
        if _ContentsGroup_isUsedNewTradeEventNotice then
          if true == _ContentsGroup_NewUI_TradeEventNotice_All then
            territorySupplyKey = PaGloabl_TradeEventNotice_Renewal_All_GetTerritorySupplyIndex(tradeBuyMarket.itemEnchantKey[commerceIndex])
          else
...
function tradeBuyMarket:setBuyItemDataInfo(index, itemName, leftCount, tradeItemWrapper, buyableStackCount, sellRate)
  ...
  tradeBuyMarket.itemName[index]:SetTextMode(__eTextMode_Limit_AutoWrap)
  ...
  if true == tradeItemWrapper:isTradableItem() then
    ...
    local conditionTypeName = UI_LifeString[needLifeType]
  ...
  if nil ~= eventControl then
    ...
    if nil ~= parentControl then
      if false == _ContentsGroup_NewUI_NumberPad_All then
        if parentControl:GetKey() == Panel_Window_Exchange_Number:GetKey() then
          return
        end
      elseif parentControl:GetKey() == Panel_Window_NumberPad_All:GetKey() then
        return
      end
    end
  end
...
function createItemList(index, row, col)
  local tempListBody = UI.createControl(__ePAUIControl_Static, Panel_Trade_Market_BuyItemList, "Static_Panel_" .. index)
  ...
  local tempTrendShow = UI.createControl(__ePAUIControl_Button, tempListBody, "Button_TrendShow" .. index)
  ...
  local tempExpiration = UI.createControl(__ePAUIControl_StaticText, tempListBody, "StaticText_Expiration" .. index)
  CopyBaseProperty(_expiration, tempExpiration)
  tradeBuyMarket.expiration[index] = tempExpiration
  local tempBuyingCondition = UI.createControl(__ePAUIControl_StaticText, tempListBody, "StaticText_BuyingCondition" .. index)
  CopyBaseProperty(_buyingCondition, tempBuyingCondition)
  tempBuyingCondition:SetTextMode(__eTextMode_LimitText)
  tradeBuyMarket.buyingCondition[index] = tempBuyingCondition
  local tempSupply = UI.createControl(__ePAUIControl_StaticText, tempListBody, "StaticText_Supply_" .. index)
...
function TradeMarket_BuyList_SimpleTooltip(isShow, index, lifeType, lifeLevel)
  ...
  local name, desc, control
  local conditionLevel = PaGlobalFunc_Util_CraftLevelReplace(lifeLevel + 1)
  local conditionTypeName = UI_LifeString[lifeType]
