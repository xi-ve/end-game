...
local TradeMarketGraph = {
  _ui = {
    ...
    stc_enableCountBG = UI.getChildControl(_panel, "Static_EnableCountBg"),
...
function TradeMarketGraph:init()
  ...
  self._ui.txt_enableTitle = UI.getChildControl(self._ui.stc_enableCountBG, "StaticText_CountTitle")
  self._ui.txt_enableCount = UI.getChildControl(self._ui.stc_enableCountBG, "StaticText_CountValue")
  self._ui.txt_enableTitle:SetTextMode(__eTextMode_AutoWrap)
  self._ui.txt_enableTitle:SetText(self._ui.txt_enableTitle:GetText())
  self._ui.txt_enableTitle:SetSize(self._ui.txt_enableTitle:GetSizeX(), self._ui.txt_enableTitle:GetTextSizeY())
  self._ui.stc_enableCountBG:SetSize(self._ui.stc_enableCountBG:GetSizeX(), self._ui.txt_enableTitle:GetTextSizeY() + 40)
  self._ui.txt_enableTitle:ComputePos()
  self._ui.txt_enableCount:ComputePos()
  ...
  for idx = 1, enCommerceType.enCommerceType_Max - 1 do
    ...
    commerceText:SetTextMode(__eTextMode_LimitText)
...
function TradeMarketGraph:registEvent()
  self._ui.list_TradeItem:registEvent(__ePAUIList2EventType_LuaChangeContent, "PaGlobal_TradeMarketGraph_CreateList")
  self._ui.list_TradeItem:createChildContent(__ePAUIList2ElementManagerType_List)
...
function TradeMarketGraph:update()
  ...
  if nil ~= characterStaticStatus then
    if characterStaticStatus:isSupplyMerchant() then
      ...
      self._ui.stc_enableCountBG:SetShow(true)
      self._ui.txt_enableCount:SetText(leftCount)
      self._ui.stc_enableCountBG:SetPosY(10 + avaliableTabCount * self._commerceBtnGapY + self._ui.stc_LeftBg:GetPosY())
    else
      self._ui.stc_enableCountBG:SetShow(false)
    end
  else
    self._ui.stc_enableCountBG:SetShow(false)
  end
...
function PaGlobal_TradeMarketGraph_ToggleItemDetailDesc(index)
  ...
  self._ui.txt_detailName:SetTextMode(__eTextMode_AutoWrap)
  ...
  self._ui.txt_itemDetailCondition:SetTextMode(__eTextMode_AutoWrap)
...
function PaGlobal_TradeMarketGraph_CreateList(content, key)
  ...
  txt_Condition:SetTextMode(__eTextMode_LimitText)
  if 1 == self._currentNPCType then
  ...
  elseif 3 == self._currentNPCType then
    ...
    if Defines.s64_const.s64_0 == _s64_leftCount then
      ...
      txt_LeftCount:SetTextMode(__eTextMode_AutoWrap)
