...
tradeGraph._staticBaseCommerceName:SetTextMode(__eTextMode_Limit_AutoWrap)
...
function tradeGraph:registUIControl()
  ...
  for count = 1, tradeGraph._itemMaxCount do
    local miniPanel = UI.createControl(__ePAUIControl_StaticText, Panel_Trade_Market_Graph_Window, "static_MiniPanel_" .. count)
    ...
    local staticGraph = UI.createControl(__ePAUIControl_StaticText, miniPanel, "static_Graph" .. count)
    ...
    local staticCommerceName = UI.createControl(__ePAUIControl_StaticText, miniPanel, "static_ItemName" .. count)
    ...
    local staticTextPermission = UI.createControl(__ePAUIControl_StaticText, miniPanel, "StaticText_Permission" .. count)
    ...
    local staticPriceRate = UI.createControl(__ePAUIControl_StaticText, miniPanel, "item_quotationRate" .. count)
    ...
    local static_PriceIcon = UI.createControl(__ePAUIControl_StaticText, miniPanel, "item_sellPrice_goldIcon_" .. count)
    ...
    local static_OriginalPrice = UI.createControl(__ePAUIControl_StaticText, miniPanel, "item_Original_goldIcon_" .. count)
    ...
    local static_SupplyCount = UI.createControl(__ePAUIControl_StaticText, miniPanel, "item_Supply_count_" .. count)
    ...
    local static_ConditionText = UI.createControl(__ePAUIControl_StaticText, miniPanel, "item_Condition_" .. count)
    ...
    local staticCurrentPoint = UI.createControl(__ePAUIControl_Static, miniPanel, "static_CurrentPoint" .. count)
    ...
    local staticHighPoint = UI.createControl(__ePAUIControl_Static, miniPanel, "static_HigePoint" .. count)
    CopyBaseProperty(tradeGraph._staticBaseHighPoint, staticHighPoint)
    tradeGraph._staticHighPoint[count] = staticHighPoint
    local staticLowPoint = UI.createControl(__ePAUIControl_Static, miniPanel, "static_LowPoint" .. count)
    CopyBaseProperty(tradeGraph._staticBaseLowPoint, staticLowPoint)
    tradeGraph._staticLowPoint[count] = staticLowPoint
    local staticBaseLine = UI.createControl(__ePAUIControl_Static, miniPanel, "static_BaseLine" .. count)
...
function tradeGraph.tradeMarket_DrawGraph(commerceIndexForGraph, itemKey, UIIndex, itemOrderIndex)
  ...
  local isFishing = npcSSW:isFishSupplyMerchant()
  if true == tradeItemWrapper:isTradableItem() and (true == tradeGraph._isNodeLinked or true == _byWorldmap) or true == isFishing then
...
function tradeGraph:registTradeGraphEvent()
  ...
  PaGlobal_Util_RegistHelpTooltipEvent(tradeGraph._buttonQuestion, "\"PanelTradeMarketGraph\"")
...
function NpcTradeGraph_ScrollEvent(isUpScroll)
  ...
  if last_Tooltip ~= nil and tooltipPanel:GetShow() == true then
