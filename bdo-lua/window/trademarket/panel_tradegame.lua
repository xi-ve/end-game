...
if nil ~= Panel_Dialog_Trade_MyBasket_All then
  btnSellAll:addInputEvent("Mouse_LUp", "HandleEventLUp_TradeMyBasket_All_SellAll()")
else
...
if (7 == getGameServiceType() or 8 == getGameServiceType()) and getContentsServiceType() == CppEnums.ContentsServiceType.eContentsServiceType_CBT then
  processMsg:SetTextMode(__eTextMode_LimitText)
else
  processMsg:SetTextMode(__eTextMode_AutoWrap)
end
function TradeGame_Restart()
  ...
  if nil ~= Panel_Dialog_Trade_MyBasket_All then
    if 0 >= PaGlobalFunc_TradeMyBasket_All_GetMySellItemCount() then
  ...
  elseif nil ~= Panel_Trade_Market_Sell_ItemList and 0 >= FGlobal_MySellCount() then
    local messageBoxMemo = PAGetString(Defines.StringSheet_GAME, "LUA_TRADEMARKET_TRADEGAME_MSG_2")
    local messageBoxData = {
      title = PAGetString(Defines.StringSheet_GAME, "LUA_TRADEMARKET_TRADEGAME_MSG_1"),
      content = messageBoxMemo,
...
function FromClient_TradeGameResult(isSuccess)
  ...
  if true == isSuccess then
    ...
    elseif nil ~= Panel_Trade_Market_Sell_ItemList then
