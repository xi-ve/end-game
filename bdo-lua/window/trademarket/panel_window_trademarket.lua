...
function npcTradeShop:registUiControl()
  ...
  for countCol = 1, npcTradeShop._TradeBuyListCol do
    for countRow = 1, npcTradeShop._TradeBuyListRow do
      ...
      buyList.static_TextItemName = UI.createControl(__ePAUIControl_StaticText, npcTradeShop.buyListFrameContent, "BuyListBuyItemName_" .. index)
      CopyBaseProperty(preloadUIList._static_TextItemName, buyList.static_TextItemName)
      buyList.static_Multiply = UI.createControl(__ePAUIControl_StaticText, npcTradeShop.buyListFrameContent, "BuyList_multiply_" .. index)
      CopyBaseProperty(preloadUIList._static_Multiply, buyList.static_Multiply)
      buyList.button_Quantity = UI.createControl(__ePAUIControl_Button, npcTradeShop.buyListFrameContent, "BuyList_Quantity" .. index)
      ...
      buyList.static_Equal = UI.createControl(__ePAUIControl_StaticText, npcTradeShop.buyListFrameContent, "BuyList_Equal" .. index)
      CopyBaseProperty(preloadUIList._static_Equal, buyList.static_Equal)
      buyList.static_Cost = UI.createControl(__ePAUIControl_StaticText, npcTradeShop.buyListFrameContent, "BuyList_Cost" .. index)
      CopyBaseProperty(preloadUIList._static_Cost, buyList.static_Cost)
      buyList.static_CostIcon = UI.createControl(__ePAUIControl_StaticText, npcTradeShop.buyListFrameContent, "BuyListIcon_Cost" .. index)
...
function check_Servant()
  ...
  if true == ToClient_IsContentsGroupOpen("576") and true == npcTradeShop._isShip then
...
function npcTradeShop:setSizeBuyListControl(index, posX, posY)
  ...
  resizeControl.static_TextItemName:SetTextMode(__eTextMode_AutoWrap)
...
function InitNpcTradeShopOpen()
  if false == global_IsTrading then
    ...
    elseif false == Panel_Npc_Dialog:IsShow() then
      return
    end
    local npcKey = dialog_getTalkNpcKey()
    if 0 ~= npcKey then
  ...
  if not boolValue and false == isFishingSupply then
