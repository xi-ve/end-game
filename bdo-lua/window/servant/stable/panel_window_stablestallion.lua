...
function StableStallion:init()
  ...
  self._stallionNotify:SetTextMode(__eTextMode_AutoWrap)
  self._stallionNotify:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_STABLESTALLION_TEXT_NOTIFY"))
  self.protect._protectItemDesc:SetTextMode(__eTextMode_AutoWrap)
...
function StableStallion_HandleMOnoutProtectItemToolTip(isShow)
  ...
  if isShow then
    if true == _ContentsGroup_NewUI_Tooltip_All then
      registTooltipControl(self.protect._protectIcon, Panel_Widget_Tooltip_Item_All)
    else
...
function Set_StallionItemSlotClick(index)
  self = StableStallion
  if Panel_NumberPad_IsPopUp() or -1 ~= self._buttonClick then
    return
  end
  self._index = index
...
function HandleMOnoutAwakenItemToolTip(isShow)
  ...
  if isShow then
    if true == _ContentsGroup_NewUI_Tooltip_All then
      registTooltipControl(self.awaken._awakenIcon, Panel_Widget_Tooltip_Item_All)
    else
...
function HandleMOnoutTrainingItemToolTip(index, slotNo)
  ...
  if nil ~= self._selectedItemWhereType then
    ...
    if nil ~= itemWrapper then
      local itemSSW = itemWrapper:getStaticStatus()
      if true == _ContentsGroup_NewUI_Tooltip_All then
        registTooltipControl(self._slots[index].icon, Panel_Widget_Tooltip_Item_All)
      else
