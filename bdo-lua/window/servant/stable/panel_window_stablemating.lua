...
function stableMating:init()
  for ii = 0, self._config.slotCount - 1 do
    ...
    slot._buttonStart:SetTextMode(__eTextMode_AutoWrap)
    ...
    slot._buttonMating:SetTextMode(__eTextMode_AutoWrap)
    ...
    slot._buttonCancel:SetTextMode(__eTextMode_AutoWrap)
    ...
    slot._buttonReceive:SetTextMode(__eTextMode_AutoWrap)
    ...
    slot._buttonEnd:SetTextMode(__eTextMode_AutoWrap)
        ...
        if nil ~= skillWrapper then
          if skillSlotNo < self._config.skill.count then
            if tempIndex >= slot._startSlotIndex then
              ...
              slot._skill[skillSlotNo]._skillIcon:addInputEvent("Mouse_On", "HandleEventOn_StableMatingInfo_SkillTooltip(" .. listIdx .. ", " .. jj .. ", " .. skillSlotNo .. ", " .. slotNo .. ")")
              if true == _ContentsGroup_NewUI_Tooltip_All then
                slot._skill[skillSlotNo]._skillIcon:addInputEvent("Mouse_Out", "PaGlobal_Tooltip_Skill_Servant_All_Close()")
              else
                slot._skill[skillSlotNo]._skillIcon:addInputEvent("Mouse_Out", "PaGlobal_Tooltip_Servant_Close()")
              end
              slot._skill[skillSlotNo]._skillText:SetTextMode(__eTextMode_AutoWrap)
...
function HandleEventOn_StableMatingInfo_SkillTooltip(listIdx, jj, skillSlotNo, slotNo)
  local myAuctionInfo = RequestGetAuctionInfo()
  local auctionServantInfo = myAuctionInfo:getServantAuctionListAt(listIdx)
  local skillWrapper = auctionServantInfo:getSkill(jj)
  local slot = stableMating._slots[slotNo]
  if true == _ContentsGroup_NewUI_Tooltip_All then
    PaGlobal_Tooltip_Skill_Servant_All_Open(skillWrapper, slot._skill[skillSlotNo]._skillIcon, false)
  else
    PaGlobal_Tooltip_Servant_Open(skillWrapper, slot._skill[skillSlotNo]._skillIcon, false)
  end
end
...
function stableMating:registEventHandler()
  ...
  PaGlobal_Util_RegistHelpTooltipEvent(self._buttonQuestion, "\"PanelWindowStableMating\"")
