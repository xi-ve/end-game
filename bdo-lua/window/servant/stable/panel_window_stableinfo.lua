...
function carrageInfo:init()
  ...
  for index = 0, self.slotCount - 1 do
    local temp = {}
    temp._horseSlot = UI.createControl(__ePAUIControl_Static, carriagePanel, "Static_HorseSlot_" .. index)
    CopyBaseProperty(self._horseSlot, temp._horseSlot)
    temp._horseSlot:SetPosY(self._horseSlot:GetPosY() + self.gapY * index)
    temp._name = UI.createControl(__ePAUIControl_StaticText, carriagePanel, "StaticText_CarriageHorse_Name_" .. index)
    CopyBaseProperty(self._name, temp._name)
    temp._name:SetPosY(self._name:GetPosY() + self.gapY * index)
    temp._level = UI.createControl(__ePAUIControl_StaticText, carriagePanel, "StaticText_Horse_Level_" .. index)
    CopyBaseProperty(self._level, temp._level)
    temp._level:SetPosY(self._level:GetPosY() + self.gapY * index)
    temp._btnRelease = UI.createControl(__ePAUIControl_Button, carriagePanel, "Button_ReleaseHorse_" .. index)
    CopyBaseProperty(self._btnRelease, temp._btnRelease)
    temp._btnRelease:SetPosY(self._btnRelease:GetPosY() + self.gapY * index)
    temp._expText = UI.createControl(__ePAUIControl_StaticText, carriagePanel, "Horse_EXP_CountData_" .. index)
    CopyBaseProperty(self._expText, temp._expText)
    temp._expText:SetPosY(self._expText:GetPosY() + self.gapY * index)
    temp._expBg = UI.createControl(__ePAUIControl_Static, carriagePanel, "Horse_EXP_Bg_" .. index)
    CopyBaseProperty(self._expBg, temp._expBg)
    temp._expBg:SetPosY(self._expBg:GetPosY() + self.gapY * index)
    temp._expGauge = UI.createControl(__ePAUIControl_Static, carriagePanel, "Horse_EXP_GaugeBar_" .. index)
...
function stableInfo:init()
  ...
  self._staticTextChangeDesc:SetTextMode(__eTextMode_AutoWrap)
  self._staticTextHelpChangeDesc:SetTextMode(__eTextMode_AutoWrap)
  self._staticTextHelpDeleteDesc:SetTextMode(__eTextMode_AutoWrap)
  ...
  for ii = 0, self._config.slot.count - 1 do
    ...
    slot.button:SetTextMode(__eTextMode_LimitText)
    slot.buttonDel:SetTextMode(__eTextMode_LimitText)
    slot.buttonTarget:SetTextMode(__eTextMode_LimitText)
    slot.buttonTargetRelease:SetTextMode(__eTextMode_LimitText)
    slot.buttonTraining:SetTextMode(__eTextMode_LimitText)
    ...
    if nil ~= skillWrapper and false == skillWrapper:isTrainingSkill() then
      if slotNo < self._config.slot.count then
        if temporarySlotIndex >= self._startSlotIndex then
          ...
          slot.icon:addInputEvent("Mouse_On", "HandleEventOn_StableInfo_SkillTooltip(" .. servantInfo:getActorKeyRaw() .. ", " .. ii .. ", " .. slotNo .. ")")
          ...
          if slot.buttonTraining:GetShow() then
            slot.dec:SetSize(150, slot.dec:GetSizeY())
          else
            slot.dec:SetSize(200, slot.dec:GetSizeY())
          end
          slot.dec:SetText(skillWrapper:getDescription())
          UI.setLimitAutoWrapTextAndAddTooltip(slot.dec, 2, slot.name:GetText(), slot.dec:GetText())
...
function HandleEventOn_StableInfo_SkillTooltip(actorKeyRaw, ii, slotNo)
  local self = stableInfo
  local servantWrapper, skillWrapper
  if nil == StableList_SelectSlotNo() then
    servantWrapper = getServantInfoFromActorKey(actorKeyRaw)
  else
    servantWrapper = stable_getServant(StableList_SelectSlotNo())
  end
  skillWrapper = servantWrapper:getSkill(ii)
  if nil == skillWrapper then
    skillWrapper = servantWrapper:getSkillXXX(ii)
  end
  if true == _ContentsGroup_NewUI_Tooltip_All then
    PaGlobal_Tooltip_Skill_Servant_All_Open(skillWrapper, self._skill[slotNo], true)
  else
    PaGlobal_Tooltip_Servant_Open(skillWrapper, self._skill[slotNo], true)
  end
end
