...
function stableList:init()
  for ii = 0, self._config.slotCount - 1 do
    ...
    slot.registerMating:SetTextMode(__eTextMode_AutoWrap)
    slot.registerMarket:SetTextMode(__eTextMode_AutoWrap)
    slot.mating:SetTextMode(__eTextMode_AutoWrap)
    slot.matingComplete:SetTextMode(__eTextMode_AutoWrap)
    slot.regionChanging:SetTextMode(__eTextMode_AutoWrap)
    slot.registerForRentTag:SetTextMode(__eTextMode_AutoWrap)
    slot.rentTag:SetTextMode(__eTextMode_AutoWrap)
    slot.training:SetTextMode(__eTextMode_AutoWrap)
    slot.coma:SetTextMode(__eTextMode_AutoWrap)
  ...
  self._buttonSupplyForQuest = UI.createAndCopyBasePropertyControl(Panel_Window_StableList, "Button_SupplyForQuest", self._staticButtonListBG, "StableList_Button_SupplyForQuest")
...
function stableList:registEventHandler()
  ...
  self._buttonSupplyForQuest:addInputEvent("Mouse_LUp", "StableList_SupplyForQuest()")
...
function StableList_ButtonOpen(eType, slotNo)
  ...
  self._buttonSupplyForQuest:SetShow(false)
        ...
        if false == isPcroomOnly and false == hasRentOwnerFlag then
          if CppEnums.VehicleType.Type_Horse == vehicleType or CppEnums.VehicleType.Type_Donkey == vehicleType or CppEnums.VehicleType.Type_Camel == vehicleType or CppEnums.VehicleType.Type_RidableBabyElephant == vehicleType and stallionTraining ~= getState then
            if nowMating ~= getState and regMarket ~= getState and regMating ~= getState and training ~= getState then
              if stable_isMarket() and 15 <= servantLevel and CppEnums.VehicleType.Type_Horse == vehicleType and isContentsEnableSupply then
          ...
          if servantInfo:isSuppliableForQuest() then
            buttonList[buttonSlotNo] = self._buttonSupplyForQuest
            buttonSlotNo = buttonSlotNo + 1
          end
        end
...
function StableList_SupplyForQuest()
  StableList_ButtonClose()
  local servantInfo = stable_getServant(StableList_SelectSlotNo())
  if nil == servantInfo then
    return
  end
  ToClient_RequestSupplyServant(StableList_SelectSlotNo())
end
...
function stable_WindowOpenCheck()
  if MessageBoxGetShow() or Panel_Window_StableRegister:GetShow() or Panel_Servant_Market_Input:GetShow() then
    return true
  end
  return false
end
...
function FromClient_ServantClearDeadCount(servantNo, isOceanShip)
...
function FromClient_StartStallionSkillTraining(servantNo)
  if not (isContentsStallionEnable and isContentsNineTierEnable) or not isContentsNineTierTraining then
    return
  end
  local servantInfo = stable_getServantByServantNo(servantNo)
  if nil == servantInfo then
...
function changeServantRegion:init()
  ...
  self._regionList2:registEvent(__ePAUIList2EventType_LuaChangeContent, "ChangeServantRegion_HandleListChange")
  self._regionList2:createChildContent(__ePAUIList2ElementManagerType_List)
