...
function stableMarket:registEventHandler()
  ...
  PaGlobal_Util_RegistHelpTooltipEvent(self._buttonQuestion, "\"PanelWindowStableMarket\"")
          ...
          if nil ~= skillWrapper then
            if slotIndex < self._config.skill.count then
              if tempIndex >= slot._startSlotIndex then
                ...
                slot._skill[slotIndex]._skillIcon:addInputEvent("Mouse_On", "HandleEventOn_StableMarketInfo_SkillTooltip(" .. listIdx .. ", " .. jj .. ", " .. slotIndex .. ", " .. slotNo .. ")")
                if true == _ContentsGroup_NewUI_Tooltip_All then
                  slot._skill[slotIndex]._skillIcon:addInputEvent("Mouse_Out", "PaGlobal_Tooltip_Skill_Servant_All_Close()")
                else
                  slot._skill[slotIndex]._skillIcon:addInputEvent("Mouse_Out", "PaGlobal_Tooltip_Servant_Close()")
                end
                slot._skill[slotIndex]._skillText:SetTextMode(__eTextMode_AutoWrap)
...
function HandleEventOn_StableMarketInfo_SkillTooltip(listIdx, jj, skillSlotNo, slotNo)
  local myAuctionInfo = RequestGetAuctionInfo()
  local auctionServantInfo = myAuctionInfo:getServantAuctionListAt(listIdx)
  local skillWrapper = auctionServantInfo:getSkill(jj)
  local slot = stableMarket._slots[slotNo]
  if true == _ContentsGroup_NewUI_Tooltip_All then
...
function StableMarket_SetSkillFilter()
  ...
  stableMarket._comboBox_SkillFilter:ToggleListbox()
  if 0 < selectSkillIndex and 1 < #stableMarket._skillFilterSelectIndex then
