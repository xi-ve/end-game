...
function servantInfo:init()
  ...
  for ii = 0, self._config._skill.count - 1 do
    ...
    slot.dec:SetTextMode(__eTextMode_Limit_AutoWrap)
  ...
  if self._exp:GetPosX() + self._exp:GetTextSizeX() > self._staticGaugeBar_ExpBG:GetPosX() then
    self._exp:SetTextMode(__eTextMode_Limit_AutoWrap)
  ...
  if self.deadCountTitle:GetPosX() + self.deadCountTitle:GetTextSizeX() > self.deadCountValue:GetPosX() then
    self.deadCountTitle:SetTextMode(__eTextMode_Limit_AutoWrap)
...
function servantInfo:update()
  ...
  self._staticName:SetTextMode(__eTextMode_LimitText)
  ...
  local total_weight = Int64toInt32(total_weight_s64 / Defines.s64_const.s64_10000)
  ...
  if self._staticText_Life:GetPosX() + self._staticText_Life:GetTextSizeX() > self._staticText_LifeValue:GetPosX() then
    self._staticText_Life:SetTextMode(__eTextMode_Limit_AutoWrap)
...
function PaGlobal_ServantInfo_SkillTooltip(actorKeyRaw, ii, slotNo)
  ...
  local skillWrapper = servantWrapper:getSkill(ii)
  if true == _ContentsGroup_NewUI_Tooltip_All then
    PaGlobal_Tooltip_Skill_Servant_All_Open(skillWrapper, self._skillSlots[slotNo], true)
  else
    PaGlobal_Tooltip_Servant_Open(skillWrapper, self._skillSlots[slotNo], true)
  end
end
...
function ServantInfo_BeforOpenByActorKeyRaw(actorKeyRaw)
  ...
  elseif UI_VT.Type_Boat == vehicleType or UI_VT.Type_Raft == vehicleType or UI_VT.Type_FishingBoat == vehicleType or UI_VT.Type_FastShip == vehicleType then
...
function ServantInfo_Close()
  ...
  if true == _ContentsGroup_NewUI_Tooltip_All then
    if PaGlobal_Tooltip_Skill_Servant_All_GetShow() then
      PaGlobal_Tooltip_Skill_Servant_All_Close()
    end
  elseif Panel_Tooltip_Skill_Servant:GetShow() then
