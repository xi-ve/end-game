...
function PaGlobalFunc_StableExchange_ScrollEvent(isUpScroll, isLeft)
  ...
  if true == isLeft then
    ...
    if (_ContentsGroup_UsePadSnapping or ToClient_IsDevelopment()) and beforeSlotIndex ~= self._value.leftStartIndex then
    ...
    if (_ContentsGroup_UsePadSnapping or ToClient_IsDevelopment()) and beforeSlotIndex ~= self._value.rightStartIndex then
      ToClient_padSnapIgnoreGroupMove()
    end
