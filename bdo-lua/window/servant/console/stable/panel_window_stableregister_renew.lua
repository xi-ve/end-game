...
function Panel_Window_StableRegister_info:childControl()
  ...
  self._ui.edit_Desc = UI.getChildControl(Panel_Window_StableRegister, "StaticText_Edit_Desc")
  local defaultNameDecs = PAGetStringParam2(Defines.StringSheet_GAME, "LUA_STABLEINFO_EDITBOX_INFO_PARAM", "min", tostring(getGameServiceTypeNameMinLength()), "max", tostring(getGameServiceTypeServantNameLength()))
  self._ui.edit_Desc:SetText(defaultNameDecs)
  ...
  self._ui.txt_emblemName:SetTextMode(__eTextMode_LimitText)
end
...
function Panel_Window_StableRegister_info:setBaseInfo()
  ...
  if CppEnums.VehicleType.Type_Carriage == vehicleType or CppEnums.VehicleType.Type_CowCarriage == vehicleType then
    self._ui.staticText_HP:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SERVANTINFO_CARRIAGE_MP"))
