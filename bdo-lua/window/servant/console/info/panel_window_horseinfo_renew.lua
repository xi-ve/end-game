...
function Panel_Window_HorseInfo_info:resize()
  ...
  if 388 < self._ui.staticText_AutoCarrot:GetTextSizeX() then
    ...
    self._ui.staticText_AutoCarrot:SetTextMode(__eTextMode_AutoWrap)
...
function Panel_Window_HorseInfo_info:childControl()
  ...
  self._ui.staticText_EXPTitle = UI.getChildControl(self._ui.static_InfoBg, "StaticText_EXP")
  ...
  self._ui.stc_autoCarrotKeyGuide = UI.getChildControl(self._ui.static_InfoBg, "Static_PadIcon")
  ...
  self._ui.list2_SkillList:registEvent(__ePAUIList2EventType_LuaChangeContent, "PaGlobalFunc_HorseInfo_SkillList")
  self._ui.list2_SkillList:createChildContent(__ePAUIList2ElementManagerType_List)
...
function Panel_Window_HorseInfo_info:updateBaseInfo()
  ...
  if servantType ~= CppEnums.VehicleType.Type_Horse then
    ...
    if servantType == CppEnums.VehicleType.Type_Horse and 9 ~= servantInfo:getTier() and false == servantInfo:isPcroomOnly() and isContentsStallionEnable then
  ...
  self._ui.staticText_Name:SetTextMode(__eTextMode_AutoWrap)
  ...
  if servantType == CppEnums.VehicleType.Type_Elephant or servantType == CppEnums.VehicleType.Type_Tank then
    self._ui.checkbtn_AutoCarrot:SetShow(false)
    self._ui.staticText_AutoCarrot:SetShow(false)
    self._ui.stc_autoCarrotKeyGuide:SetShow(false)
    if servantType == CppEnums.VehicleType.Type_Elephant then
      self._ui.staticText_EXPTitle:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_GUILDSERVANTINFO_INJURY"))
    else
      self._ui.staticText_EXPTitle:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_STABLEINFO_DESTROYCOUNT"))
    end
    self._ui.staticText_EXPVal:SetText(deadCount * 10 .. "%")
    self._ui.progress2_EXP:SetProgressRate(deadCount * 10)
  else
    self._ui.checkbtn_AutoCarrot:SetShow(true)
    self._ui.staticText_AutoCarrot:SetShow(true)
    self._ui.stc_autoCarrotKeyGuide:SetShow(true)
    self._ui.staticText_EXPTitle:SetText(PAGetString(Defines.StringSheet_RESOURCE, "SERVANT_INFO_TEXT_EXP"))
  ...
  if nil ~= vehicleInfo then
    self._ui.staticText_DefenseVal:SetText(vehicleInfo:get():getDefense())
  end
  ...
  if servantType ~= CppEnums.VehicleType.Type_Elephant and servantType ~= CppEnums.VehicleType.Type_Tank then
    local deadCount = servantInfo:getDeadCount()
...
function PaGlobalFunc_HorseInfo_SkillList(list_content, key)
  ...
  staticText_Command:SetTextMode(__eTextMode_LimitText)
