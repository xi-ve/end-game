...
function Panel_Window_CarriageInfo_info:updateBaseInfo()
  ...
  if CppEnums.VehicleType.Type_RepairableCarriage == vehicleType then
    self._ui.staticText_SP:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SERVANT_SHIPINFO_MP"))
  else
...
function Panel_Window_CarriageInfo_info:updateEquip()
  ...
  for key, value in pairs(self._equipOverfitSlotNo) do
    ...
    if nil ~= itemWrapper then
      ...
      slot:setItem(itemWrapper)
      slot.icon:SetMonoTone(false)
      extendedCount = extendedCount + self:getExtendedSlotCountUpdateInfo(itemWrapper, value)
    else
      slot.iconBG:SetShow(true)
      slot:clearItem()
    end
  end
  if 0 < extendedCount then
    for extendSlotNo, parentSlotNo in pairs(self._extendedSlotInfoArray) do
      local itemWrapper = servantInfo:getEquipItem(parentSlotNo)
      if nil ~= itemWrapper then
        slot = self._ui.equipOverfitSlot[extendSlotNo]
        slot.iconBG:SetShow(false)
        slot:setItem(itemWrapper)
...
function Panel_Window_CarriageInfo_info:getExtendedSlotCountUpdateInfo(itemWrapper, slotNo)
