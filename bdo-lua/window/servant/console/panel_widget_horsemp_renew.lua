...
registerEvent("EventCanShipMoveFastForConsole", "FGlobal_ToggleShipFast_PadInput")
...
function HorseMP_Update()
  ...
  if true == oceanShipSkillUI then
    if 21 <= getSelfPlayer():get():getLifeExperienceLevel(9) then
      servantMpBar._button_AutoCarrot:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SHIPFASTMOVE_AUTOUSE"))
      servantMpBar._button_AutoCarrot:SetShow(true)
      servantMpBar._button_AutoCarrot:SetSpanSize(servantMpBar._button_AutoCarrot:GetSpanSize().x, servantMpBar._originalAutoBtnPanY - Panel_Widget_SailControl:GetSizeY())
      ...
      if false == servantMpBar._autoBtnFlag then
        ...
        servantMpBar._button_AutoCarrot:addInputEvent("Mouse_On", "HorseMP_SimpleTooltips( true, 6, nil )")
        servantMpBar._button_AutoCarrot:addInputEvent("Mouse_Out", "HorseMP_SimpleTooltips( false, 6 )")
        servantMpBar._button_AutoCarrot:setTooltipEventRegistFunc("HorseMP_SimpleTooltips( true, 6, nil )")
      end
      local currentFlag = ToClient_IsCanShipMoveFast()
      local settingFlag = servantMpBar._button_AutoCarrot:IsCheck()
      if currentFlag ~= settingFlag then
        ToClient_SetShipCanMoveFast(settingFlag)
      end
    else
      servantMpBar._button_AutoCarrot:SetShow(false)
    end
  elseif getSelfPlayer():isNavigationLoop() and getSelfPlayer():isNavigationMoving() then
...
function FGlobal_ToggleShipFast_PadInput()
  if false == servantMpBar._button_AutoCarrot:GetShow() then
    return
  end
  if false == servantMpBar._autoBtnFlag then
    return
  end
  if false == servantMpBar._button_AutoCarrot:IsCheck() then
    servantMpBar._button_AutoCarrot:SetCheck(true)
  else
    servantMpBar._button_AutoCarrot:SetCheck(false)
  end
