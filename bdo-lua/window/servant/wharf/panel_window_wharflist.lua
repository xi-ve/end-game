...
function wharfList:init()
  ...
  self._buttonSailorManager = UI.createAndCopyBasePropertyControl(Panel_Window_WharfList, "Button_SailorManager", self._staticButtonListBG, "WharfList_Button_SailorManager")
...
function wharfList:registEventHandler()
  ...
  self._buttonSailorManager:addInputEvent("Mouse_LUp", "WharfList_SailorManager()")
...
function WharfList_ButtonOpen(eType, slotNo)
  ...
  self._buttonSailorManager:SetShow(false)
  ...
  if eType == self._const.eTypeSealed then
    ...
    if regionName == servantRegionName then
      ...
      if servantInfo:getIsDamageShip() then
        self._buttonClearDeadCount:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SHIP_DAMAGE_RECOVERY"))
      end
      if FGlobal_IsCommercialService() then
  ...
  elseif eType == self._const.eTypeUnsealed then
    ...
    local servantInfo = temporaryWrapper:getUnsealVehicle(stable_getServantType())
    local chracterKeyRaw = servantInfo:getCharacterKeyRaw()
    local shipStaticStatus = ToClient_EmployeeCharacterShipStaticStatusWrapper(chracterKeyRaw)
    buttonList[button_Index] = self._buttonSeal
    if true == _ContentsGroup_OceanCurrent and true == _ContentsGroup_Sailor and nil ~= shipStaticStatus and 0 < shipStaticStatus:getEmployeeMaxCost() then
      buttonList[button_Index + 1] = self._buttonSailorManager
      buttonList[button_Index + 2] = self._buttonCompulsionSeal
      button_Index = button_Index + 3
    else
...
function WharfList_SailorManager()
  WharfFunction_RegisterAck()
...
function WharfList_ClearDeadCount()
  ...
  if true == servantInfo:getIsDamageShip() then
    ...
    local clearDeadCountDo = function()
      stable_clearOceanShipDeadCount(WharfList_SelectSlotNo(), MessageBoxCheck.isCheck())
    end
    local messageData = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_DAMAGE_RESTORE_NEED_MONEYLUA_DAMAGE_RESTROE_NEED_MONEY", "money", tostring(makeDotMoney(needMoney)))
    local messageBoxData = {
      title = PAGetString(Defines.StringSheet_GAME, "LUA_WARNING"),
      content = messageData,
      functionApply = clearDeadCountDo,
      functionCancel = MessageBox_Empty_function,
      priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW
    }
    MessageBoxCheck.showMessageBox(messageBoxData)
...
function wharfList_SortDataupdate()
  ...
  local affiliatedTerritory = function(exploerKey)
    ...
    elseif 1100 < exploerKey and exploerKey <= 1300 then
      territoryKey = 3
    elseif 1300 < exploerKey and exploerKey < 1395 then
      territoryKey = 4
    else
      territoryKey = 5
    end
