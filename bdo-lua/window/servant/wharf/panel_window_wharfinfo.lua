...
function wharfInfo:update(unsealType)
  ...
  self._staticText_MoveSpeed:SetText(string.format("%.1f", wharfInfo:getTotalStat(servantInfo, __eEmployeeAbility_Servant_Speed) * 1.0E-4) .. "%")
  self._staticText_Acceleration:SetText(string.format("%.1f", wharfInfo:getTotalStat(servantInfo, __eEmployeeAbility_Servant_Acceleration) * 1.0E-4) .. "%")
  self._staticText_Cornering:SetText(string.format("%.1f", wharfInfo:getTotalStat(servantInfo, __eEmployeeAbility_Servant_Cornering) * 1.0E-4) .. "%")
  self._staticText_BrakeSpeed:SetText(string.format("%.1f", wharfInfo:getTotalStat(servantInfo, __eEmployeeAbility_Servant_Brake) * 1.0E-4) .. "%")
  ...
  elseif servantInfo:getVehicleType() == CppEnums.VehicleType.Type_Boat or servantInfo:getVehicleType() == CppEnums.VehicleType.Type_Raft or servantInfo:getVehicleType() == CppEnums.VehicleType.Type_FishingBoat or servantInfo:getVehicleType() == CppEnums.VehicleType.Type_SailingBoat then
    if servantInfo:getIsDamageShip() then
      self._deadCount:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SHIPINFO_WOUNDEDPERCENT"))
    else
      self._deadCount:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_STABLEINFO_DESTROYCOUNT"))
    end
  end
...
function wharfInfo:getTotalStat(servantInfo, eEmployeeAbility)
  local basicStat = 0
  local addStat = 0
  if __eEmployeeAbility_Servant_Speed == eEmployeeAbility then
    basicStat = servantInfo:getStat(CppEnums.ServantStatType.Type_MaxMoveSpeed)
  elseif __eEmployeeAbility_Servant_Acceleration == eEmployeeAbility then
    basicStat = servantInfo:getStat(CppEnums.ServantStatType.Type_Acceleration)
  elseif __eEmployeeAbility_Servant_Cornering == eEmployeeAbility then
    basicStat = servantInfo:getStat(CppEnums.ServantStatType.Type_CorneringSpeed)
  elseif __eEmployeeAbility_Servant_Brake == eEmployeeAbility then
    basicStat = servantInfo:getStat(CppEnums.ServantStatType.Type_BrakeSpeed)
  end
  addStat = ToClient_GetEmployeeAbillityByServant(servantInfo:getServantNo(), eEmployeeAbility)
