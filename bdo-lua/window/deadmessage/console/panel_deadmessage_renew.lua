...
function deadMessage:createControl()
  ...
  self._ui._staticText_Y_ConsoleUI = UI.getChildControl(self._ui._static_BottomBg, "StaticText_Y_ConsoleUI")
  self._ui._staticText_A_ConsoleUI:SetShow(false)
  self._ui._staticText_X_ConsoleUI:SetShow(false)
  self._ui._staticText_Y_ConsoleUI:SetShow(false)
  self._keyGuides = {
    self._ui._staticText_X_ConsoleUI,
    self._ui._staticText_Y_ConsoleUI,
    self._ui._staticText_A_ConsoleUI
  }
end
...
function deadMessage:setButtonEvent()
  ...
  buttonEvent[button.ExitBloodAltar] = "PaGlobalFunc_DeadMessage_ButtonPushEvent_ExitBloodAltar"
end
...
function deadMessage:setSize()
  ...
  self._ui._staticText_Y_ConsoleUI:ComputePos()
  ...
  PaGlobalFunc_ConsoleKeyGuide_SetAlign(self._keyGuides, self._ui._static_BottomBg, CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_CENTER)
...
function PaGlobalFunc_DeadMessage_Update(deltaTime)
  ...
  if true == selfPlayer:isDead() or 0 >= selfPlayer:get():getHp() then
    ...
    if nil == self._resurrectionTime or true == ToClient_IsInstanceFieldPlayerbyContentsType(__eInstanceContentsType_SavageDefence) then
    ...
    if 0 < self._resurrectionTime then
      ...
      if self._config._eDeadType.DeadLocate_InCompetition == self._currentDeadType then
        self._ui._staticText_resurrectionTime:SetShow(false)
      else
...
function deadMessage:closeChannelMoveWindow()
  if false == _ContentsGroup_RenewUI_ExitGame then
    ...
    if true == _ContentsGroup_NewUI_ChannelSelect_All then
      if nil ~= Panel_ChannelSelect_All and Panel_ChannelSelect_All:GetShow() then
        PaGlobalFunc_ChannelSelect_All_Close()
        return
      end
    elseif true == _ContentsGroup_RenewUI_ServerSelect and true == PaGlobalFunc_ServerSelect_GetShow() then
...
function deadMessage:buttonMouseOnEvent(buttonType)
  ...
  deadMessage._isSetXKey = true
  deadMessage._isSetYKey = true
  local selfProxy = getSelfPlayer()
  local regionInfo = getRegionInfoByPosition(selfProxy:get():getPosition())
  if _eDeadType.DeadLocate_InPlunder == self._currentDeadType or _eDeadType.DeadLocate_InPlunder == self._currentDeadType or _eDeadType.DeadLocate_InBattleGround == self._currentDeadType or _eDeadType.DeadLocate_InCompetition == self._currentDeadType or _eDeadType.DeadLocate_InSavageDefence == self._currentDeadType or _eDeadType.DeadLocate_InGuildBattle == self._currentDeadType or _eDeadType.DeadLocate_InGuildTeamBattle == self._currentDeadType or _eDeadType.DeadLocate_InPVP == self._currentDeadType or _eDeadType.DeadLocate_InLocarWar == self._currentDeadType or _eDeadType.DeadLocate_InPrision == self._currentDeadType or _eDeadType.IsSpecialCharacter == self._currentDeadType then
    deadMessage._isSetXKey = false
    deadMessage._isSetYKey = false
  ...
  if true == deadMessage._isSetXKey then
  ...
  if true == deadMessage._isSetYKey then
  ...
  self._ui._staticText_X_ConsoleUI:SetShow(deadMessage._isSetXKey)
  self._ui._staticText_Y_ConsoleUI:SetShow(deadMessage._isSetYKey)
  if button.Observer == buttonType or button.ExitBloodAltar == buttonType then
    self._ui._staticText_A_ConsoleUI:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_COMMON_SELECT"))
    deadMessage._isSetXKey = false
    deadMessage._isSetYKey = false
  else
    self._ui._staticText_A_ConsoleUI:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_DEADMESSAGE_NORMALREVIVAL"))
  end
  ...
  Panel_DeadMessage_Renew:registerPadEvent(__eConsoleUIPadEvent_Up_Y, "")
  if true == deadMessage._isSetXKey then
    Panel_DeadMessage_Renew:registerPadEvent(__eConsoleUIPadEvent_Up_X, buttonEvent .. "(true , false)")
  end
  if true == deadMessage._isSetYKey then
    Panel_DeadMessage_Renew:registerPadEvent(__eConsoleUIPadEvent_Up_Y, buttonEvent .. "(nil , true)")
  end
  PaGlobalFunc_ConsoleKeyGuide_SetAlign(self._keyGuides, self._ui._static_BottomBg, CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_CENTER)
end
...
function PaGlobalFunc_DeadMessage_ButtonPushEvent_Immediate(useItem, useFairy)
  deadMessage:buttonPushedEvent_Immediate(useItem, useFairy)
end
function deadMessage:buttonPushedEvent_Immediate(useItem, useFairy)
  ...
  elseif true == useFairy then
    useCheckFairy(self._config._eRespawnType.respawnType_Immediate)
...
function useCheckFairy(respawnType)
  ...
  if true == isArena and respawnType == self._config._eRespawnType.respawnType_Immediate then
    deadMessage_Revival(self._config._eRespawnType.respawnType_Immediate, 0, CppEnums.ItemWhereType.eCashInventory, getSelfPlayer():getRegionKey(), false, toInt64(0, 0), false)
  else
...
function PaGlobalFunc_DeadMessage_ButtonPushEvent_Exploration(useItem, useFairy)
  deadMessage:buttonPushedEvent_Exploration(useItem, useFairy)
end
function deadMessage:buttonPushedEvent_Exploration(useItem, useFairy)
  local selfProxy = getSelfPlayer()
  local regionInfo = getRegionInfoByPosition(selfProxy:get():getPosition())
  ...
  if (false == isSiegeBeing or false == isKingOrLordWarZone and false == isVillageWarZone) and nil == useItem and nil == useFairy and (true == self._isHasResotreExp or prevExp > currentExp) then
    contentString = contentString .. "\n" .. PAGetString(Defines.StringSheet_GAME, "DEADMESSAGE_TEXT_EXP_DOWN")
  end
  local function revivalExplorationConfirm()
    _AudioPostEvent_SystemUiForXBOX(50, 1)
    deadMessage_Revival(self._config._eRespawnType.respawnType_Exploration, 255, 0, getSelfPlayer():getRegionKey(), false, toInt64(0, 0), useFairy)
    SetUIMode(Defines.UIMode.eUIMode_Default)
  end
...
function PaGlobalFunc_DeadMessage_ButtonPushEvent_NearTown(useItem, useFairy)
  deadMessage:buttonPushedEvent_NearTown(useItem, useFairy)
end
function deadMessage:buttonPushedEvent_NearTown(useItem, useFairy)
  local selfProxy = getSelfPlayer()
  local regionInfo = getRegionInfoByPosition(selfProxy:get():getPosition())
  local linkedSiegeRegionInfoWrapper = ToClient_getVillageSiegeRegionInfoWrapperByPosition(selfProxy:get():getPosition())
  local isVillageWarZone = linkedSiegeRegionInfoWrapper:get():isVillageWarZone()
  ...
  if false == isSiegeBeing or false == isKingOrLordWarZone and false == isVillageWarZone then
    if nil == useItem and nil == useFairy and (true == self._isHasResotreExp or prevExp > currentExp) then
      contentString = contentString .. "\n" .. PAGetString(Defines.StringSheet_GAME, "DEADMESSAGE_TEXT_EXP_DOWN")
    end
  ...
  local function revivalVillageConfirm()
    _AudioPostEvent_SystemUiForXBOX(50, 1)
    deadMessage_Revival(self._config._eRespawnType.respawnType_NearTown, 255, 0, getSelfPlayer():getRegionKey(), false, toInt64(0, 0), useFairy)
    SetUIMode(Defines.UIMode.eUIMode_Default)
  end
  local messageboxData = {
    title = PAGetString(Defines.StringSheet_GAME, "DEADMESSAGE_TEXT_RESPAWN_MB_TITLE"),
...
function PaGlobalFunc_DeadMessage_ButtonPushEvent_ExitBloodAltar()
  deadMessage:buttonPushedEvent_ExitBloodAltar()
end
function deadMessage:buttonPushedEvent_ExitBloodAltar()
