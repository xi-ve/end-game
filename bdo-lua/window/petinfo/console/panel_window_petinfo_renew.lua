...
function petInfo:update()
  ...
  for idx = 0, petActionCount - 1 do
    ...
    if true == isLearn then
      ...
      uiIdx = uiIdx + 1
    elseif self._action_HasCount > self._config._action_MaxCount and idx >= self._config._action_MaxCount and true == self._isActionScrollDown then
      actionBtn[idx - self._config._action_MaxCount]._icon:SetShow(false)
      actionBtn[idx - self._config._action_MaxCount]._name:SetShow(false)
    end
  ...
  if self._currentPetLv[petNum_S32] ~= petLevel then
    if 1 < petLevel and 1 < self._currentPetLv[petNum_S32] then
...
function petInfo:createButton()
  ...
  for idx = 0, self._config._skill_MaxCount - 1 do
    ...
    info._index = nil
    info._name:SetTextMode(__eTextMode_Limit_AutoWrap)
...
function petInfo:setAbilityInfoByType(PcPetData, skillNo)
  ...
  if self._config._skillType_Default == skillNo then
    ...
    self._ui._petInfoFrame._static_defaultSkill:SetTextMode(__eTextMode_AutoWrap)
  ...
  elseif self._config._skillType_Special == skillNo then
    ...
    self._ui._petInfoFrame._static_specialSkill:SetTextMode(__eTextMode_AutoWrap)
