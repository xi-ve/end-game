...
function petRegister:initialize()
  ...
  if isGameTypeEnglish() or isGameTypeTaiwan() or isGameTypeTR() or isGameTypeTH() or isGameTypeID() then
    petRegisterUI._static_PetIconSlot:SetTextMode(__eTextMode_AutoWrap)
  end
  ...
  petRegisterUI._staticText_RegisterDesc:SetTextMode(__eTextMode_AutoWrap)
  ...
  petRegisterUI._staticText_RegisterDesc:SetText(defaultDesc)
...
function FGlobal_EscapeEditBox_PetCompose(bool)
  if true == Panel_Window_PetExchange_Renew:GetShow() and nil ~= Panel_Window_PetExchange_Renew then
    TooltipSimple_Hide()
  end
  ClearFocusEdit()
end
