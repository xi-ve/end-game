...
function PetExchange:Initialize()
  ...
  self._ui._txt_mainPetName:SetTextMode(__eTextMode_LimitText)
...
function PaGlobalFunc_PetExchange_EditControlXUp()
  ...
  local panelPosX = Panel_Window_PetExchange_Renew:GetPosX()
  TooltipSimple_Common_Pos(panelSizeX + panelPosX + 20, self._ui._edit_newPetName:GetPosY() + self._ui._stc_bottomBG:GetPosY())
...
function PaGlobalFunc_PetExchange_ExchangeConfirm()
  ...
  local messageBoxData = {
    ...
    functionYes = PaGlobalFunc_PetExchange_ExchangeExcute,
    functionNo = MessageBox_Empty_function,
    priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW
  }
  MessageBox.showMessageBox(messageBoxData)
end
function PaGlobalFunc_PetExchange_ExchangeExcute()
  local self = PetExchange
  ...
  local count = 1
  local petNo = 0
  local appearanceIndex = self._newAppearanceIndex - 1
  for index = 0, self._subPetTotalCount - 1 do
    if true == self._subPetSelectStateList[index] then
      local petWrapper = ToClient_getPetSealedDataByIndex(self._subPetIndexList[index])
      local petRace = petWrapper:getPetStaticStatus():getPetRace()
      petNo = petWrapper._petNo
      ToClient_pushFusionPetList(petNo, count)
      count = count + 1
      if 99 == petRace and count <= appearanceIndex + 1 then
...
function PetExchange:CreateSubPetButton()
  ...
  for row = 0, self._config._rowCount - 1 do
    for col = 0, self._config._columnCount - 1 do
      ...
      self._ui._txt_subPetNameList[index]:SetTextMode(__eTextMode_LimitText)
...
function PetExchange:Open(selectPetNoStr)
  ...
  self:Update()
  PaGlobalFunc_PetExchange_SelectNewAppearance(0, -1)
