...
function petCommand:selectOrder(orderType, value)
  ...
  local pcPetData, petNo
  if true == self._isCommandAll then
    ...
    pcPetData = ToClient_getPetUnsealedDataByIndex(self._selectPetIndex)
...
function FGlobal_PetCommand_Init(petIndex, isCommandAll)
  if nil == petIndex then
    return
  end
  petCommand._isCommandAll = isCommandAll
