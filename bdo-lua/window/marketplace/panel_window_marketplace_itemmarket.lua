...
function _itemMarket_FavoriteItemRegist_World()
  ...
  elseif true == self._ui.edit_ItemName:GetShow() then
    ...
    if 0 < itemMarketSummaryCount then
...
function PaGlobalFunc_ItemMarket_CreateControlMarketItemSubList(contents, key)
  ...
  local itemNameStr = self:setNameAndEnchantLevel(enchantLevel, itemSSW:getItemType(), itemSSW:getName(), itemSSW:getItemClassify(), itemSSW:isSpecialEnchantItem())
  if minEnchantLevel ~= maxEnchantLevel then
    itemNameStr = itemNameStr .. "( +" .. minEnchantLevel .. " ~ +" .. maxEnchantLevel .. " ) "
  end
  local itemNameSize = txt_ItemName:GetSizeX()
  txt_ItemName:SetText(itemNameStr)
  if itemNameSize <= txt_ItemName:GetTextSizeX() then
...
function ItemMarket:setNameAndEnchantLevel(enchantLevel, itemType, itemName, itemClassify, isSpecialEnchantItem)
  ...
  if 1 == itemType and 15 < enchantLevel then
...
function PaGlobalFunc_ItemMarket_UpdateMyInfo()
  ...
  if 1000 <= isTotalFamilyPoint and isTotalFamilyPoint <= 3999 then
    isTaxBonus = 0.5
  elseif 4000 <= isTotalFamilyPoint and isTotalFamilyPoint <= 6999 then
    isTaxBonus = 1
  elseif 7000 <= isTotalFamilyPoint then
    isTaxBonus = 1.5
  end
...
function PaGlobalFunc_ItemMarket_UpdateItemList()
  ...
  if nil ~= itemInfo and self._itemListType.detailListByKey == self._currentListType then
    local itemStaticStatus = itemInfo:getItemEnchantStaticStatusWrapper():get()
