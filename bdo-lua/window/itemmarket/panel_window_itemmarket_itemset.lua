...
function ItemmarketItemSet_ListUpdate(contents, key)
  ...
  if nil ~= myItemInfo then
    ...
    if nil ~= iess then
      ...
      if 1 == iess:getItemType() and 15 < enchantLevel then
        ItemName:SetText(HighEnchantLevel_ReplaceString(enchantLevel) .. " " .. iess:getName())
      elseif 0 < enchantLevel and CppEnums.ItemClassifyType.eItemClassify_Accessory == iess:getItemClassify() and false == iess:isSpecialEnchantItem() then
