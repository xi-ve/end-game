...
function defalut_Control:Init_Control()
  self._mail._Template._Sender_Name:SetTextMode(__eTextMode_LimitText)
  self._mail._Template._Mail_Title:SetTextMode(__eTextMode_LimitText)
  self._mail._Template._Mail_Num:SetTextMode(__eTextMode_LimitText)
...
function defalut_Control:Init_Function()
  ...
  PaGlobal_Util_RegistHelpTooltipEvent(self._mail._buttonQuestion, "\"Panelmail\"")
...
function _mail_Data:setData()
  ...
  for massMailIndex = 1, massMailCount do
    ...
    local sender_Name = massMailInfo:getSenderName()
    local mail_Title = massMailInfo:getTitle()
    local mail_No = massMailInfo:getMailNo()
    local mail_isExistItem = massMailInfo:isExistItem()
    local mail_getMailReceiveDate = massMailInfo:getMailReceiveDate()
    local idx = (pageIndex - 1) % SlotMax + 1
  ...
  for index = 1, SlotMax do
    if nil ~= self._Data[pageNo] then
      if nil ~= self._Data[pageNo][index] then
        ...
        local _isMassMail = self._Data[pageNo][index]._isMassMail
        ...
        defalut_Control._mail._checkBtn[index]:SetShow(not _isMassMail)
        defalut_Control._mail._Mail_Num[index]:SetShow(not _isMassMail)
...
function Mail_GetDetail(index)
  ...
  local isMassMail = _mail_Data._Data[pageNo][index]._isMassMail
  ...
  if true == isMassMail then
    ToClient_RequestMassMailDetail(_mail_Data._Data[pageNo][index]._mail_No)
  else
...
function MailList_SelectDelete()
  ...
  for index = 1, SlotMax do
    if defalut_Control._mail._checkBtn[index]:IsCheck() and false == self._Data[pageNo][index]._mail_GetItem then
      lastMailIndex = index
    end
  end
  for index = 1, SlotMax do
    if defalut_Control._mail._checkBtn[index]:GetShow() and defalut_Control._mail._checkBtn[index]:IsCheck() then
      ...
      local isMassMail = self._Data[pageNo][index]._isMassMail
      if true == isMassMail then
