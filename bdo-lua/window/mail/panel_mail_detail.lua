...
function mailDetail:registMessageHandler()
  ...
  registerEvent("FromClient_ShowMassMailDetail", "FromClient_ShowMassMailDetail")
...
function mailDetail:registEventHandler()
  ...
  PaGlobal_Util_RegistHelpTooltipEvent(self._buttonQuestion, "\"PanelMailDetail\"")
...
function FromClient_ShowMassMailDetail(mailNo)
  ...
  self.isMassMail = true
  self._sender:SetText(ToClient_GetSelectedMassMailSender())
  self._title:SetText(ToClient_GetSelectedMassMailTitle())
  UI.setLimitTextAndAddTooltip(self._title, "", self._title:GetText())
  self._contents:SetAutoResize(true)
  self._contents:SetTextMode(__eTextMode_AutoWrap)
  self._contents:SetText(ToClient_GetSelectedMassMailContant())
  ...
  if nil ~= mailItem and 0 < Int64toInt32(itemCount_s64) then
    ...
    self._itemSlot:setItemByStaticStatus(mailItem, itemCount_s64)
    ...
    self._itemCount:SetShow(itemCount_s64 > toInt64(0, 1))
    if 1 == itemKey then
...
function Mail_Detail_Open(mailNo)
  ...
  if 1 == RequestMail_getMailType() then
    ...
    self._itemText:SetShow(1 < itemCount)
    self._itemCount:SetShow(1 < itemCount)
...
function Mail_Detail_Delete()
  ...
  if nil ~= mailDetail.openMailNo then
    if self.isMassMail then
      ToClient_RemoveMassMail(mailDetail.openMailNo)
    else
...
function Mail_Detail_GetItem()
  local itemESSW = mail_getMailItemStatic()
...
function PaGlobalFunc_MailDetail_RequestMailItem()
  ...
  if self.isMassMail then
    ...
    local isWarehouseCheck = mailDetail._checkboxToWarehouse:IsCheck()
