...
function looting:init()
  for ii = 1, looting.MAX_LOOTING_SLOT_COUNT do
    ...
    local looting_SlotBG = UI.createControl(__ePAUIControl_Static, Panel_Looting, "looting_SlotBG_" .. ii)
...
function Panel_Looting_Update(empty)
  ...
  _buttonLootAll:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_LOOTING_BTN_GETALL"))
  if 1 == empty then
    Panel_Looting_Value_isCloseLooting = true
...
function PaGloabl_Looting_All_GetShowPanel()
  if nil == Panel_Looting then
    return false
  end
  return Panel_Looting:GetShow()
end
looting:init()
