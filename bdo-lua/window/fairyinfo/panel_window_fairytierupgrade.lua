...
function PaGlobal_FairyTierUpgrade:initialize()
  ...
  self._ui._bottomDescText:SetTextMode(__eTextMode_AutoWrap)
...
function PaGlobal_FairyTierUpgrade_Open(PositionReset)
  ...
  if false == PaGlobal_FairyInfo_EnableFairyTierUpgrade() then
    local contentString = ""
    if false == PaGlobal_FairyInfo_isMaxLevel() then
