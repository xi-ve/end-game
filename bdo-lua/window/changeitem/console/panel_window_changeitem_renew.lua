...
function ChangeItem:init()
  ...
  self._ui.txt_keyGuideX = UI.getChildControl(self._ui.stc_keyGuideArea, "StaticText_X")
...
function ChangeItem:registEventHandler()
  self._ui.equipIcon:registerPadEvent(__eConsoleUIPadEvent_Up_X, "PaGlobal_ChangeItem_ShowTooltip(true,false)")
  self._ui.avatarIcon:registerPadEvent(__eConsoleUIPadEvent_Up_X, "PaGlobal_ChangeItem_ShowTooltip(true,true)")
  self._ui.equipIcon:addInputEvent("Mouse_Out", "PaGlobal_ChangeItem_ShowTooltip(false,false)")
  self._ui.avatarIcon:addInputEvent("Mouse_Out", "PaGlobal_ChangeItem_ShowTooltip(false,true)")
...
function FromClient_ShowItemChange_ChangeItem()
  ...
  _panel:ignorePadSnapUpdate(false)
end
...
function PaGlobalFunc_ChangeItem_Rclick(slotNo, itemWrapper, count_s64, inventoryType)
  ...
  self._equipItemWrapper = itemWrapper
...
function PaGlobal_ChangeItem_ShowTooltip(isShow, isChangeItem)
  if isShow then
    if true == isChangeItem and nil ~= self._changeItemSSW and false == doChange then
      PaGlobalFunc_TooltipInfo_Open(Defines.TooltipDataType.ItemSSWrapper, self._changeItemSSW, Defines.TooltipTargetType.Item, 0)
    elseif false == isChangeItem and nil ~= self._equipItemWrapper then
      local inventory = getSelfPlayer():get():getInventory()
      local inventoryType = Inventory_GetCurrentInventoryType()
      local itemWrapper = getInventoryItemByType(inventoryType, self._slotNo)
