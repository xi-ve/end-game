...
function FromClient_UpdateFightState(fightState, isShowingResultMessage)
  ...
  elseif CppEnums.CompetitionFightState.eCompetitionFightState_SelectAttend == fightState then
    ...
    if true == ToClient_IsMyselfInArena() then
      if false == ToClient_IsPlayingReplay() and false == ToClient_IsReplayStreamingMode() then
        Panel_Arsha_TeamWidget:SetShow(true)
      else
        Panel_Arsha_TeamWidget:SetShow(false)
      end
...
function FromClient_ReplayArshaLimitTime(limitTime)
  local self = arshaPvPWidget
  self.freeTime:SetText(convertSecondsToClockTime(limitTime))
  self.roundTime:SetText(convertSecondsToClockTime(limitTime))
  self.personalMatchTime:SetText(convertSecondsToClockTime(limitTime))
end
ArshaPvP_Widget_Init()
