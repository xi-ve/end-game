...
journalContents:SetTextMode(__eTextMode_AutoWrap)
...
local Copy_UI_CharChange = {
  ...
  _copy_Inven = UI.getChildControl(Panel_GameExit, "Button_Inven"),
...
_exitConfirm_TrayString_Old:SetTextMode(__eTextMode_AutoWrap)
...
function Panel_GameExit_Initialize()
  ...
  for idx = 0, totalCharacterCount - 1 do
    local charSlotBG2 = UI.createControl(__ePAUIControl_Static, _charSlotBG, "Static_CharSlotBG2_" .. idx)
    ...
    local charSlot = UI.createControl(__ePAUIControl_Static, _charSlotBG, "Static_CharSlot_" .. idx)
    ...
    local charLevel = UI.createControl(__ePAUIControl_StaticText, charSlot, "StaticText_CharLevel_" .. idx)
    CopyBaseProperty(Copy_UI_CharChange._copy_CharLevel, charLevel)
    charLevel:SetPosY(203)
    charLevel:SetShow(false)
    charLevelPool[idx] = charLevel
    local charName = UI.createControl(__ePAUIControl_StaticText, charSlot, "StaticText_CharName_" .. idx)
    CopyBaseProperty(Copy_UI_CharChange._copy_CharName, charName)
    charName:SetShow(false)
    charName:SetPosY(charLevel:GetPosY() + 26)
    charName:SetTextMode(__eTextMode_AutoWrap)
    charNamePool[idx] = charName
    local charWorkTxt = UI.createControl(__ePAUIControl_StaticText, charSlot, "StaticText_CharWorkText_" .. idx)
    ...
    local charWorkRemainText = UI.createControl(__ePAUIControl_StaticText, charSlot, "StaticText_PcDeliveryRamainTimeText_" .. idx)
    ...
    local charPositionTxt = UI.createControl(__ePAUIControl_StaticText, charSlot, "StaticText_CharPosition_" .. idx)
    CopyBaseProperty(Copy_UI_CharChange._copy_CharPosition, charPositionTxt)
    charPositionTxt:SetShow(false)
    charPositionTxt:SetPosY(charName:GetPosY() + 22)
    charPositionPool[idx] = charPositionTxt
    local charEnterWaitingTxt = UI.createControl(__ePAUIControl_StaticText, charSlot, "StaticText_EnterWaiting_" .. idx)
    ...
    local inven = UI.cloneControl(Copy_UI_CharChange._copy_Inven, charSlotBG2, "Button_Inventory_" .. idx)
    inven:SetShow(false)
    inven:SetPosX(5)
    inven:SetPosY(isCharacterSlotBG[idx]:GetSizeY() - 35)
    InventoryButton[idx] = inven
    local charChange = UI.createControl(__ePAUIControl_Button, charSlotBG2, "Button_CharChange_" .. idx)
    CopyBaseProperty(Copy_UI_CharChange._copy_CharChange, charChange)
    charChange:SetShow(false)
    charChange:SetPosX(InventoryButton[idx]:GetSizeX() + 5)
    charChange:SetPosY(isCharacterSlotBG[idx]:GetSizeY() - 35)
    CharacterChangeButton[idx] = charChange
    local charSelected = UI.createControl(__ePAUIControl_Static, charSlotBG2, "Static_CharSelected_" .. idx)
    ...
    local normalStack = UI.createControl(__ePAUIControl_StaticText, charSlot, "StaticText_NormalStack_" .. idx)
    ...
    local charWpCount = UI.createControl(__ePAUIControl_StaticText, charSlot, "StaticText_charWpCount_" .. idx)
    ...
    for servantIdx = 0, _eServantView.max do
      local servantView = UI.createControl(__ePAUIControl_Static, charSlot, "Static_ServantView_" .. idx .. servantIdx)
  ...
  _btn_ChangeChannel:addInputEvent("Mouse_LUp", "PaGlobalFunc_ServerSelct_Open()")
...
function refreshCharacterInfoData(startIdx)
  ...
  for idx = startIdx, characterDatacount - 1 do
    ...
    charPositionPool[uiCount]:SetTextMode(__eTextMode_LimitText)
    ...
    InventoryButton[uiCount]:addInputEvent("Mouse_LUp", "PaGlobal_SimpleInventory:requestSimpleInventory(" .. idx .. ")")
    ...
    if false == isInPostion(isCharacterSlotBG[uiCount], currentMousePosX, currentMousePosY) then
      InventoryButton[uiCount]:SetShow(false)
      CharacterChangeButton[uiCount]:SetShow(false)
    else
      InventoryButton[uiCount]:SetShow(true)
    ...
    if characterNo_64 == characterData._characterNo_s64 then
      ...
      InventoryButton[uiCount]:SetShow(false)
      InventoryButton[uiCount]:SetIgnore(true)
      InventoryButton[uiCount]:SetEnable(false)
      CharacterChangeButton[uiCount]:SetShow(false)
      ...
      InventoryButton[uiCount]:SetIgnore(false)
      InventoryButton[uiCount]:SetEnable(true)
...
function Panel_GameExit_ClickCharSlot(idx)
  if prevClickIndex ~= idx then
    ...
    InventoryButton[prevClickIndex]:SetShow(false)
  ...
  if idx ~= _nowPlayCharaterSlotNo then
    InventoryButton[idx]:SetShow(true)
...
function GameExitShowToggle(isAttacked)
  ...
  if true == isShow then
    ...
    if true == Panel_Window_SimpleInventory:IsShow() then
  ...
  for index = 0, 3 do
    ...
    if characterNo_64 ~= characterData._characterNo_s64 then
      ...
      InventoryButton[index]:SetShow(false)
...
function Panel_GameExit:setNextReward()
  ...
  if 0 == slotIndex then
    ...
    _exitConfirm_RewardDesc:SetTextMode(__eTextMode_AutoWrap)
    _exitConfirm_RewardDesc:SetText(_exitConfirm_RewardDesc:GetText())
...
local function registEventHandler()
  ...
  PaGlobal_Util_RegistHelpTooltipEvent(_buttonQuestion, "\"PanelGameExit\"")
