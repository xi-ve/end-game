...
local channelSelect = {
  ...
  _prepareToStopIcon = UI.getChildControl(Panel_ChannelSelect, "StaticText_PrepareToStop"),
...
local channelChildControl = {
  ...
  _prepareToStopIcon = UI.getChildControl(channelSelect._channelBg, "Static_PrepareToStop"),
...
function ChannelSelect_Init()
  ...
  for index = 0, bgIndex do
    if nil == self.groupBg[index] then
      self.groupBg[index] = UI.createControl(__ePAUIControl_Static, Panel_ChannelSelect, "Static_ChannelGroupBg_" .. index)
  ...
  if isChannelCountLow then
    for index = 0, channelCount - 1 do
      local channelList = {}
      channelList.channelBg = UI.createControl(__ePAUIControl_StaticText, self.groupBg[0], "create_ChannelBg_" .. index)
      channelList.channelCurrentBg = UI.createControl(__ePAUIControl_StaticText, channelList.channelBg, "create_ChannelCurrentBg_" .. index)
      channelList.channelSelectedBg = UI.createControl(__ePAUIControl_StaticText, channelList.channelBg, "create_ChannelSelectedBg_" .. index)
      channelList.channelName = UI.createControl(__ePAUIControl_StaticText, channelList.channelBg, "create_ChannelName_" .. index)
      channelList.channelStatus = UI.createControl(__ePAUIControl_StaticText, channelList.channelBg, "create_ChannelStatus_" .. index)
      channelList.channelWaricon = UI.createControl(__ePAUIControl_Static, channelList.channelBg, "create_ChannelWaricon_" .. index)
      channelList.channelnoEnterIcon = UI.createControl(__ePAUIControl_Static, channelList.channelBg, "create_ChannelNoEnterIcon_" .. index)
      channelList.channelMaintenanceIcon = UI.createControl(__ePAUIControl_Static, channelList.channelBg, "create_ChannelMaintenanceIcon_" .. index)
      channelList.channelPrepareToStopIcon = UI.createControl(__ePAUIControl_Static, channelList.channelBg, "create_ChannelPrepareToStopIcon_" .. index)
      channelList.channelExpIcon = UI.createControl(__ePAUIControl_Static, channelList.channelBg, "create_ChannelExpIcon_" .. index)
      channelList.channelPremiumIcon = UI.createControl(__ePAUIControl_Static, channelList.channelBg, "create_ChannelPremiumIcon_" .. index)
      channelList.channelPKIcon = UI.createControl(__ePAUIControl_Static, channelList.channelBg, "create_ChannelPKIcon_" .. index)
      ...
      CopyBaseProperty(channelChildControl._prepareToStopIcon, channelList.channelPrepareToStopIcon)
    ...
    for index = 0, channelCount - 1 do
      ...
      local channelList = {}
      channelList.channelBg = UI.createControl(__ePAUIControl_StaticText, self.groupBg[_bgIndex], "create_ChannelBg_" .. index)
      channelList.channelCurrentBg = UI.createControl(__ePAUIControl_StaticText, channelList.channelBg, "create_ChannelCurrentBg_" .. index)
      channelList.channelSelectedBg = UI.createControl(__ePAUIControl_StaticText, channelList.channelBg, "create_ChannelSelectedBg_" .. index)
      channelList.channelName = UI.createControl(__ePAUIControl_StaticText, channelList.channelBg, "create_ChannelName_" .. index)
      channelList.channelStatus = UI.createControl(__ePAUIControl_StaticText, channelList.channelBg, "create_ChannelStatus_" .. index)
  ...
  channelMainDesc._serverDesc:SetTextMode(__eTextMode_AutoWrap)
  ...
  if isGameTypeTH() or isGameTypeID() then
    ...
    channelMainDesc._speedTitle:SetShow(true)
    channelMainDesc._speedDesc:SetShow(true)
  ...
  elseif isGameTypeRussia() or isGameTypeEnglish() or isGameTypeJapan() or isGameTypeTR() or isGameTypeSA() or isGameTypeTaiwan() then
    channelMainDesc._pkTitle:SetShow(true)
    channelMainDesc._pkDesc:SetShow(true)
    sizeControl = channelMainDesc._pkDesc
  ...
  channelMainDesc._speedDesc:SetTextMode(__eTextMode_AutoWrap)
  ...
  channelMainDesc._pkDesc:SetTextMode(__eTextMode_AutoWrap)
  if isGameTypeKorea() then
  ...
  self._prepareToStopIcon:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SERVERSELECT_CHANNELSELECT_BOTTOM_MAINTANANCE"))
  self._PKIcon:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_GAMEEXIT_SEVERSELECT_PK"))
  local bottomIconList = {
    [1] = self._warIcon,
    [2] = self._noEnteranceIcon,
    [3] = self._maintenanceIcon,
    [4] = self._expIcon,
    [5] = self._prepareToStopIcon,
    [6] = self._PKIcon,
    [7] = self._PremiumIcon
  }
  local gabX = 10
  local gabY = 5
  local startPosX = 15
  local startPosY = self.groupBg[0]:GetPosY() + self.groupBg[0]:GetSizeY() + gabX
  local panelSizeX = {
    original = Panel_ChannelSelect:GetSizeX(),
    countLow = 410,
    countHigh = 796
  }
  ...
  if isChannelCountLow then
    Panel_ChannelSelect:SetSize(panelSizeX.countLow, Panel_ChannelSelect:GetSizeY())
    self._title:SetSize(panelSizeX.countLow - gabX, self._title:GetSizeY())
  else
    Panel_ChannelSelect:SetSize(panelSizeX.countHigh, Panel_ChannelSelect:GetSizeY())
    self._title:SetSize(panelSizeX.countHigh - gabX, self._title:GetSizeY())
  end
  if isGameTypeKorea() then
  ...
  if isGameTypeKR2() or isGameTypeGT() then
    self._warIcon:SetShow(false)
    self._noEnteranceIcon:SetShow(false)
    self._maintenanceIcon:SetShow(false)
    self._prepareToStopIcon:SetShow(false)
    self._expIcon:SetShow(false)
    self._PremiumIcon:SetShow(false)
    self._PKIcon:SetShow(false)
  end
  local panelStartPosX = Panel_ChannelSelect:GetPosX()
  local panelEndPosX = panelStartPosX + self.groupBg[0]:GetSizeX()
  local subFrameEndPosY = self._subframe:GetPosY() + self._subframe:GetSizeY()
  local lastIndex = 1
  for index = 1, #bottomIconList do
    if true == bottomIconList[index]:GetShow() then
      local iconEndPosX = panelStartPosX + lastPosX + bottomIconList[index]:GetSizeX() + bottomIconList[index]:GetTextSizeX() + gabX
      if panelEndPosX > iconEndPosX + 10 then
        bottomIconList[index]:SetPosX(lastPosX)
        bottomIconList[index]:SetPosY(lastPosY)
        lastPosX = lastPosX + bottomIconList[index]:GetSizeX() + bottomIconList[index]:GetTextSizeX() + gabX
      else
        lastPosX = startPosX
        lastPosY = lastPosY + bottomIconList[index]:GetSizeY() + gabY
        bottomIconList[index]:SetPosX(lastPosX)
        bottomIconList[index]:SetPosY(lastPosY)
        lastPosX = lastPosX + bottomIconList[index]:GetSizeX() + bottomIconList[index]:GetTextSizeX() + gabX
      end
  ...
  if subFrameEndPosY < lastPosY + bottomIconList[lastIndex]:GetSizeY() + gabY then
    local addSize = lastPosY + bottomIconList[lastIndex]:GetSizeY() - subFrameEndPosY
    Panel_ChannelSelect:SetSize(Panel_ChannelSelect:GetSizeX(), Panel_ChannelSelect:GetSizeY() + addSize)
    self._subframe:SetSize(self._subframe:GetSizeX(), self._subframe:GetSizeY() + addSize)
  end
...
function ChannelSelect_Update()
  ...
  for chIndex = 0, channelCount - 1 do
    ...
    if nil ~= _serverData then
      ...
      self.channelSelectUIPool[chIndex].channelPrepareToStopIcon:SetShow(false)
      local busyState = _serverData._busyState
      local isPrepareToStopServer = _serverData:isPrepareToStop()
      if true == isPrepareToStopServer then
        tempStr = PAGetString(Defines.StringSheet_GAME, "LUA_PREPARE_TO_STOP_SERVER")
        self.channelSelectUIPool[chIndex].channelPrepareToStopIcon:SetShow(true)
      elseif busyState == 0 or _serverData:isClosed() then
      ...
      if self.channelSelectUIPool[chIndex].channelMaintenanceIcon:GetShow() or self.channelSelectUIPool[chIndex].channelPrepareToStopIcon:GetShow() then
        self.channelSelectUIPool[chIndex].channelMaintenanceIcon:SetPosX(basePosX)
        self.channelSelectUIPool[chIndex].channelPrepareToStopIcon:SetPosX(basePosX)
      ...
      if false == _ContentsGroup_isConsolePadControl then
        ...
        self.channelSelectUIPool[chIndex].channelPrepareToStopIcon:addInputEvent("Mouse_On", "ChannelSelect_IconToolTip(" .. 6 .. "," .. chIndex .. ")")
        self.channelSelectUIPool[chIndex].channelPrepareToStopIcon:addInputEvent("Mouse_Out", "ChannelSelect_IconToolTip()")
      end
    end
  end
...
function ChannelSelect_IconToolTip(iconType, index)
  ...
  elseif 6 == iconType then
    control = channelSelect.channelSelectUIPool[index].channelPrepareToStopIcon
    name = PAGetString(Defines.StringSheet_GAME, "LUA_PREPARE_TO_STOP_SERVER")
  end
...
function EventUpdateServerInformation_Exit()
  ...
  for chIndex = 0, channelCount - 1 do
    ...
    if restrictedServerNo ~= 0 and toInt64(0, 0) ~= channelMoveableGlobalTime then
      if restrictedServerNo == tempChannel._serverNo then
        isAdmission = true
      elseif 0 < channelMoveableRemainTime then
        isAdmission = true
      else
        isAdmission = false
      end
