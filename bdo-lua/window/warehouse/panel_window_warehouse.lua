...
local _wareHouse_HelpMovie = UI.createControl(__ePAUIControl_WebControl, Panel_Window_Warehouse, "Static_ClassMovie")
...
function warehouse:isDeliveryWindow()
  if true == _ContentsGroup_NewUI_Delivery_All then
    return PaGlobalFunc_DeliveryAll_GetReadyforSend()
  elseif true == _ContentsGroup_RenewUI_Delivery then
...
function warehouse:registEventHandler()
  ...
  PaGlobal_Util_RegistHelpTooltipEvent(self.buttonQuestion, "\"WareHouse\"")
...
function FGlobal_Warehouse_IsMoveItem()
  local self = warehouse
  if not (not self:isNpc() and not self:isInstallation() and self:isWorldMapNode()) or self:isDeliveryWindow() or self:isGuildHouse() or self:isMaid() or self:isFurnitureWareHouse() then
    return true
  end
  return false
end
...
function HandleClickedWarehouseItem(index)
  ...
  if true == _ContentsGroup_NewUI_Delivery_All then
    if true == PaGlobalFunc_PanelDelivery_GetShow() and false == self:isDeliveryWindow() then
      return
    end
  elseif true == _ContentsGroup_NewDelivery and true == PaGlobalFunc_PanelDelivery_GetShow() and false == self:isDeliveryWindow() then
    return
  end
...
function HandleClicked_ManufactureOpen()
  ...
  if false == _ContentsGroup_NewUI_Manufacture_All then
    ...
    PaGlobalFunc_Manufacture_All_Open(nil, CppEnums.ItemWhereType.eWarehouse, true, nil, self._currentWaypointKey)
  end
...
function Warehouse_OpenPanel(waypointKey, fromType)
  ...
  if CppEnums.WarehoouseFromType.eWarehoouseFromType_Maid == fromType then
    if true == _ContentsGroup_NewUI_Inventory_All then
      Panel_Window_Warehouse:SetPosX(getScreenSizeX() - Panel_Window_Inventory_All:GetSizeX() - Panel_Window_Warehouse:GetSizeX())
    else
