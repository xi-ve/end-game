...
function Warehouse:init()
  ...
  self._ui.txt_NotAvaliable:SetTextMode(__eTextMode_AutoWrap)
...
function Warehouse:open(waypointKey, fromType, isSetWarehouseList)
  ...
  if eWarehouseTypeNpc == self._fromType or eWarehouseTypeManufacture == self._fromType then
    if true == ToClient_isPossibleManufactureAtWareHouse() then
      ...
      if false == isManufactureOpen and false == self:isDeliveryWindow() and true == isSetWarehouseList then
...
function WarehouseListMenu:updateWarehouseList()
  ...
  Warehouse:setWareHouseList(self._isSetWarehouseList)
  InputMLUp_WarehouseListMenu_TerritoryOpen(currentTerritoryKey, true)
end
function Warehouse:setWareHouseList(isSet)
  if false == isSet then
    ...
    Warehouse._ui.stc_BottomBg:SetSize(Warehouse._panelSizeX - Warehouse._ui.stc_LeftBg:GetSizeX(), Warehouse._ui.stc_BottomBg:GetSizeY())
    ...
    Warehouse._ui.stc_BottomBg:SetSize(Warehouse._panelSizeX, Warehouse._ui.stc_BottomBg:GetSizeY())
    ...
    WarehouseListMenu._list:SetShow(true)
    for index = 0, WarehouseListMenu._maxTerritoryKeyCount - 1 do
      if nil ~= WarehouseListMenu._warehouseInfo.territoryGroup[index] then
        WarehouseListMenu._list:getElementManager():pushKey(toInt64(0, index))
      end
    end
  end
end
...
function Warehouse:registEvent()
  self._ui.list_Warehouse:registEvent(__ePAUIList2EventType_LuaChangeContent, "PaGlobalFunc_Warehouse_CreateWarehouseListControl")
  self._ui.list_Warehouse:createChildContent(__ePAUIList2ElementManagerType_List)
...
function FromClient_Warehouse_GuildWarehouseUpdate(bForcedOpen, bForcedClose)
  ...
  self._buttonData.guildUpdate = true
  Warehouse_SetIgnoreMoneyButton(true)
  _panel:SetShow(true, true)
  WarehouseListMenu._isSetWarehouseList = false
  Warehouse:setWareHouseList(WarehouseListMenu._isSetWarehouseList)
  PaGlobalFunc_Warehouse_OpenWithInventory()
