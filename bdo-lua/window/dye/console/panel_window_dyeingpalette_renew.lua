...
function DyeingPalette:open(targetType, slotNo)
  ...
  if 29 == slotNo and __eClassType_ShyWaman == self._classType then
    if -1 == self._selectedShaiActionIndex then
      InputMLUp_DyeingPalette_SelectShaiWeaponActionIndex(shaiWeaponActionIndex.Tring)
    else
      InputMLUp_DyeingPalette_SelectShaiWeaponActionIndex(self._selectedShaiActionIndex)
    end
...
function InputMLUp_DyeingPalette_SelectShaiWeaponActionIndex(actionIndex)
  local self = DyeingPalette
  self._selectedShaiActionIndex = actionIndex
  for key, value in pairs(shaiWeaponActionIndex) do
