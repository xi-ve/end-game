...
function DyeReNew:Initialize()
  ...
  for ii = 0, enControlValue.MaxEquipSlotCount - 1 do
    ...
    self._arrEquipSlotBG[ii] = UIEquipBG
    self._arrEquipSlotPosX[ii] = UIEquipBG:GetPosX()
    if false == self._enableAwaken then
      local awakenWeaponIndex = {2, 9}
      local maxRowCount = 7
      for index = 1, #awakenWeaponIndex do
        if awakenWeaponIndex[index] == ii then
          self:GetUIEquipIconByIndex(ii):SetShow(self._enableAwaken)
          self:GetUIEquipBGByIndex(ii):SetShow(self._enableAwaken)
        end
        if ii > awakenWeaponIndex[index] and ii < maxRowCount * index then
          self._arrEquipSlotBG[ii]:SetPosX(self._arrEquipSlotPosX[ii - 1])
        end
      end
    end
  end
