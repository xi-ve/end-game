...
function CannonInven:initControl()
  ...
  self._ui._txt_cannonTitle:SetTextMode(__eTextMode_AutoWrap)
...
function CannonInven:updateCannonInven()
  ...
  if nil ~= self._cannonUsingSlotCount then
    for iSlot = 1, self._cannonSlotCountMax do
      self._ui._slot_cannonSlotList[iSlot]:clearItem()
    end
  end
...
function CannonInven:isUsableVehicleType()
  local vehicleActorWrapper = getVehicleActor(self._vehicleActorKeyRaw)
  if nil == vehicleActorWrapper then
    return false
  end
  ...
  if __eVehicleType_Siege_Bastille == vehicleType or __eVehicleType_Siege_FrameTower == vehicleType or __eVehicleType_Cannon == vehicleType or __eVehicleType_TrainingCannon == vehicleType then
    return true
  end
  return false
end
function CannonInven:updatePlayerInven()
  local selfPlayer = getSelfPlayer()
  if nil == self._whereTypeToOpen then
    return
  end
  ...
  if false == self:isUsableVehicleType() then
    return
  end
  ...
  for iSlot = 1, invenCapacity do
    ...
    if nil ~= itemWrapper and true == ToClient_isUsableCannonItem(self._vehicleActorKeyRaw, iSlot) then
      cannonballTypeCount = cannonballTypeCount + 1
      if cannonballTypeCount > self._playerSlotCountMax then
        self._playerUsingSlotCount = cannonballTypeCount - 1
        return
      end
