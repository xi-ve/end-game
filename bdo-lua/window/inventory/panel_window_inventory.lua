  ...
  for ii = 0, self.config.slotCount - 1 do
    ...
    if nil ~= itemWrapper then
      ...
      if not isUsableServant and not Panel_Window_Exchange:GetShow() then
        if isEquip and nil ~= defencePoint and nil ~= offencePoint and 0 < currentEndurance and true == matchEquip and true == PaGlobalFunc_Util_CheckSeasonEquip(itemSSW) and false == isAccessory and defencePoint + offencePoint > equipDefencePoint + equipOffencePoint then
        ...
        if 0 < currentEndurance and true == matchEquip and true == PaGlobalFunc_Util_CheckSeasonEquip(itemSSW) and true == isAccessory and offencePoint + defencePoint > equipOffencePoint + equipDefencePoint then
          slot.icon:AddEffect("fUI_BetterItemAura01", true, 0, 0)
          local equipPos = itemWrapper:getStaticStatus():getEquipSlotNo()
          if false == _ContentsGroup_RemasterUI_Main_Alert then
            Panel_NewEquip_Update(equipPos)
