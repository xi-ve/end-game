...
function HandleEventLUp_UiSet_PositionCheck(index)
  ...
  elseif index >= PaGlobal_UIModify._panelID.SkillCoolTimeQuickSlot0 and index <= PaGlobal_UIModify._panelID.SkillCoolTimeQuickSlot9 then
    HandleEventLUp_UiSet_PositionCheck_SetPos(index, PaGlobal_UIModify._panelID.SkillCoolTimeQuickSlot0, PaGlobal_UIModify._panelID.SkillCoolTimeQuickSlot9)
  end
end
...
function HandleEventLUp_UiSet_ControlShowToggle(idx)
  ...
  elseif idx >= PaGlobal_UIModify._panelID.NewQuickSlot0 and idx <= PaGlobal_UIModify._panelID.NewQuickSlot19 and not isUseNewQuickSlot() then
    ...
    PaGlobal_UIModify._panelPool[idx].close:SetCheck(PaGlobal_UIModify._panelControl[idx].isShow)
    return
  elseif PaGlobal_UIModify._panelControl[idx].isShow then
  ...
  if idx >= PaGlobal_UIModify._panelID.Chat0 and idx <= PaGlobal_UIModify._panelID.Chat4 then
    Chatting_setIsOpenValue(idx - PaGlobal_UIModify._panelID.Chat0, panelOpen)
  end
end
function HandleClicked_UiSet_ConfirmSetting(isShortcuts)
  ...
  if PaGlobal_UIModify._prevRemasterUI ~= PaGlobal_UIModify._isShowRemasterUI then
    FromClient_MainStatus_SwapUIOption(PaGlobal_UIModify._isShowRemasterUI)
  end
  ...
  local reset_GameUI = function()
    ...
    for idx = 1, PaGlobal_UIModify._panelCount do
      ...
      elseif idx == panelID.ClassResource then
        if __eClassType_Sorcerer == getSelfPlayer():getClassType() or __eClassType_Combattant == getSelfPlayer():getClassType() or __eClassType_Mystic == getSelfPlayer():getClassType() or __eClassType_ShyWaman == getSelfPlayer():getClassType() or __eClassType_BladeMasterWoman == getSelfPlayer():getClassType() then
          panelControl[idx].control:SetShow(panelControl[idx].isShow)
        end
...
function HandleLUp_SavePresetInfo(presetIndex)
  PaGlobal_UIModify:savePresetInfo(presetIndex)
end
