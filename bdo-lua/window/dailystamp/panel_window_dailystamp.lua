...
function DailyStamp_DescSet()
  ...
  dailyStamp.eventPeriod:SetTextMode(__eTextMode_AutoWrap)
  dailyStamp.acceptPeriod:SetTextMode(__eTextMode_AutoWrap)
  dailyStamp.eventDesc:SetTextMode(__eTextMode_AutoWrap)
  dailyStamp.weekEndDesc:SetTextMode(__eTextMode_AutoWrap)
  ...
  dailyStamp.acceptPeriodTitle:SetShow(false)
  dailyStamp.acceptPeriod:SetShow(false)
    ...
    for index = 1, yesterdayTabCount do
      if index < dailyStampCount then
        ...
        if nil == self.tapControl[index] then
          self.tapControl[index] = UI.createControl(__ePAUIControl_RadioButton, Panel_Window_DailyStamp, "DailyStamp_Tap_" .. index)
...
function dailyStamp:TabUpdate()
  ...
  for index = 1, dailyStampCount do
    ...
    local rewardCount = ToClient_getReceivedRewardCount(attendanceKey)
    if 0 < myAttendanceCount - rewardCount then
...
function dailyStamp:Init()
  ...
  for index = 0, self.rewardCount - 1 do
    local temp = {}
    temp._dayControl = UI.createControl(__ePAUIControl_Static, Panel_Window_DailyStamp, "Static_DailyStamp_Day_" .. index)
    ...
    temp._dayText = UI.createControl(__ePAUIControl_StaticText, temp._dayControl, "StaticText_DailyStamp_Day_" .. index)
    CopyBaseProperty(self.tempDay, temp._dayText)
    temp._dayText:SetPosX(5)
    temp._dayText:SetPosY(5)
    temp._dayText:SetText(index + 1)
    temp._rewardName = UI.createControl(__ePAUIControl_StaticText, temp._dayControl, "StaticText_DailyStamp_RewardName_" .. index)
    CopyBaseProperty(self.tempName, temp._rewardName)
    temp._rewardName:SetPosX(6)
    temp._rewardName:SetPosY(70)
    temp._rewardName:SetTextMode(__eTextMode_AutoWrap)
    ...
    temp._stamp = UI.createControl(__ePAUIControl_Static, temp._dayControl, "Static_DailyStamp_StampIcon_" .. index)
...
function DailyStamp_SetData(rewardIndex)
  ...
  if false == (1 <= rewardIndex and rewardIndex <= dailyStampCount) then
    return
  end
...
function DailStamp_Hide()
  ...
  elseif true == _ContentsGroup_NewUI_EventNotify_All then
    Panel_EventNotify_All_Open()
  else
    EventNotify_Open()
  end
