...
local DailyStamp = {
  ...
  _blockedByUIMode = false,
  _blockedAttandenceKey = -1,
...
function DailyStamp:initControl()
  ...
  self._ui._txt_eventPeriodAlert:SetTextMode(__eTextMode_AutoWrap)
  self._ui._txt_eventPeriodAlert:SetText(self._ui._txt_eventPeriodAlert:GetText())
  self._ui._txt_weekEndDesc:SetTextMode(__eTextMode_AutoWrap)
...
function FromClient_AttendanceUpdate_DailyStampRenew(attendanceKey)
  ...
  if GetUIMode() ~= Defines.UIMode.eUIMode_Default then
    DailyStamp._blockedByUIMode = true
    DailyStamp._blockedAttandenceKey = attendanceKey
    return
  end
...
function FromClient_AttendanceTimer_DailyStampRenew()
  ...
  if GetUIMode() ~= Defines.UIMode.eUIMode_Default then
    return
  elseif true == DailyStamp._blockedByUIMode and -1 ~= DailyStamp._blockedAttandenceKey then
    FromClient_AttendanceUpdate_DailyStampRenew(DailyStamp._blockedAttandenceKey)
    DailyStamp._blockedByUIMode = false
    DailyStamp._blockedAttandenceKey = -1
  end
  local waitingTime = -1
