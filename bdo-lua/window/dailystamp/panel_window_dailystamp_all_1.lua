...
function PaGlobal_DailyStamp_All:dailyInit()
  ...
  for index = 0, self._rewardCount - 1 do
    local temp = {}
    temp._dayControl = UI.createControl(__ePAUIControl_Button, self._ui.stc_buttonGroup, "Button_RewardSlot_" .. index)
...
function PaGlobal_DailyStamp_All:descInit()
  ...
  local eventAlertStr = ""
  if _ContentsGroup_UsePadSnapping and true ~= _ContentsGroup_NewUI_TotalReward_All then
    eventAlertStr = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_DAILYSTAMP_NEWDESC_CONSOLE")
  else
...
function PaGlobal_DailyStamp_All:setData(tabIndex)
  ...
  if false == (1 <= tabIndex and tabIndex <= self._dailyStampCount) then
    return
  end
