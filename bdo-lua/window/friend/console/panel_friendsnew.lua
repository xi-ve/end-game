...
function PaGlobal_FriendNew:AlignkeyGuides()
  local keyGuideList = {
  ...
  local totalSizeX = self._ui._Static_BottomBg:GetSizeX()
  local posX = totalSizeX
  local posY = self._ui._StaticText_KeyGuideClose:GetPosY()
  local keySize = 44
  local keySpace = 20
  for key, value in ipairs(keyGuideList) do
    if true == value:GetShow() then
      local spaceFromRight = value:GetTextSizeX() + keySize + keySpace
      posX = posX - spaceFromRight
      if posX < 0 then
        posX = totalSizeX - spaceFromRight
        posY = posY - 40
      end
      value:SetPosXY(posX, posY)
    end
  end
  if self._ui._StaticText_KeyGuideClose:GetPosY() == posY then
    _panel:SetSize(_panel:GetSizeX(), 540)
  else
    _panel:SetSize(_panel:GetSizeX(), 580)
  end
...
function PaGlobal_FriendNew:UpdatePCFriendList()
  ...
  if nil ~= selfPlayer then
    ...
    if true == isGuildMaster and 0 < friendCount then
      self._ui._StaticText_KeyGuideInviteParty:SetShow(true)
    end
  end
end
...
function PaGlobal_FriendNew:Init()
  ...
  self._ui._List2_FriendBg:createChildContent(__ePAUIList2ElementManagerType_List)
  self._ui._List2_FriendBg:registEvent(__ePAUIList2EventType_LuaChangeContent, "FriendNew_CreateFriendList")
  self._ui._List2_OfferBg:createChildContent(__ePAUIList2ElementManagerType_List)
  self._ui._List2_OfferBg:registEvent(__ePAUIList2EventType_LuaChangeContent, "FriendNew_CreateOfferList")
