  ...
  elseif true == selfPlayer:get():isGuildMaster() then
    if 1 == memberInfo:getGrade() then
      ...
      if true == _ContentsGroup_LargeParty then
        self:addButton(self._btnType.inviteLargeParty)
      end
    ...
    if true == memberInfo:isOnline() and _ContentsGroup_isXBOXUI then
...
function GuildMemberFunction:openGuildSetting()
  ...
  if false == PaGlobalFunc_checkIsSelfVolunteer() then
    self:addButton(self._btnType.guildWareHouse)
  end
...
function GuildMemberFunction:openClanMemberSetting()
  ...
  elseif true == isGuildMaster then
    if 1 == self._currentMemberInfo:getGrade() then
      self:addButton(self._btnType.cancelCommander)
    elseif 2 == self._currentMemberInfo:getGrade() then
      self:addButton(self._btnType.appointCommander)
    end
...
function GuildMemberFunction:addButton(btnType)
  ...
  elseif btnType == self._btnType.guildWareHouse then
    btnTemplate:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_GUILD_SIMPLETOOLTIP_WAREHOUSE"))
  elseif btnType == self._btnType.inviteLargeParty then
    btnTemplate:SetText(PAGetString(Defines.StringSheet_RESOURCE, "INTERACTION_BTN_LARGEPARTY"))
  end
...
function InputMLUp_GuildMemberFunction_PressButton(btnType)
  ...
  elseif btnType == self._btnType.guildIntro then
    if false == ToClient_isPS4() then
      ...
      PaGlobalFunc_GuildIntroduction_Open()
    end
  elseif btnType == self._btnType.guildNotice then
  ...
  elseif btnType == self._btnType.guildWareHouse then
    PaGlobalFunc_GuildSettingFunction_OpenGuildWareHouse()
  elseif btnType == self._btnType.inviteLargeParty then
    PaGlobalFunc_GuildMemberInfo_InviteLargeParty()
  end
  _AudioPostEvent_SystemUiForXBOX(50, 1)
  self:close()
end
function PaGlobalFunc_GuildSettingFunction_OpenGuildWareHouse()
  GuildWarehouseOpen()
end
...
function PaGlobalFunc_GuildMemberInfo_InviteLargeParty()
  local self = GuildMemberFunction
  if nil == self then
    _PA_ASSERT(false, "\237\140\168\235\132\144\236\157\180 \236\161\180\236\158\172\237\149\152\236\167\128 \236\149\138\236\138\181\235\139\136\235\139\164!! : GuildMemberFunction")
    return
  ...
  if false == ToClient_IsGuildMemberBlocked(self._currentMemberIdx) then
    do
      local isOnline = self._currentMemberInfo:isOnline()
      local characterName = self._currentMemberInfo:getCharacterName()
      local function guildMemberLargePartyInvite()
      ...
      if true == isOnline then
        ...
        local messageboxData = {
          title = messageTitle,
          content = messageContent,
          functionYes = guildMemberLargePartyInvite,
          functionNo = MessageBox_Empty_function,
          priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW
        }
        MessageBox.showMessageBox(messageboxData, "middle")
