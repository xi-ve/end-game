...
function GuildIntro:updateNotice()
  ...
  self._ui.edit_Text:SetMaxInput(self._currentMaxInput)
  self._ui.edit_Text:SetMaxEditLine(2)
  self._ui.edit_Text:SetTextMode(__eTextMode_AutoWrap)
...
function GuildIntro:init()
  ...
  self._ui.txt_EditInfoTitle:SetTextMode(__eTextMode_AutoWrap)
...
function GuildIntro:initLimitString()
  self._noticeMaxInputLimitString = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_GUILDINFO_INPUT_EDITTEXT", "maxLength", 15)
...
function PaGlobalFunc_GuildIntro_EndVirtualKeyboard(str)
  ...
  if self._currentMaxInput < getWstringLength(str) then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_SymbolNo, "eErrInputTooLong"))
    str = getMaxSliceString(str, self._currentMaxInput)
  end
  ClearFocusEdit()
