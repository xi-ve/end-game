...
function GuildWarfareInfo:init()
  ...
  self._ui.list2_WarfareInfo:registEvent(__ePAUIList2EventType_LuaChangeContent, "PaglobalFunc_WarfareInfo_CreateList")
  self._ui.list2_WarfareInfo:createChildContent(__ePAUIList2ElementManagerType_List)
...
function PaglobalFunc_WarfareInfo_CreateList(control, key)
  ...
  btn_list:addInputEvent("Mouse_On", "InputMO_GuildWarfareInfo_ShowTooltip(" .. key32 .. ")")
end
...
function InputMO_GuildWarfareInfo_ShowTooltip(index)
  local self = GuildWarfareInfo
  if nil == self then
  ...
  local contents = self._ui.list2_WarfareInfo:GetContentByKey(toInt64(0, index))
  local btnList = UI.getChildControl(contents, "Button_Content")
  local name = UI.getChildControl(btnList, "StaticText_Name")
  if true == name:IsLimitText() then
    self._ui.stc_NameTooltipBG:SetShow(true)
    self._ui.txt_NameTooltip:SetText(name:GetText())
    self._ui.stc_NameTooltipBG:SetSize(self._ui.txt_NameTooltip:GetTextSizeX() + 40, self._ui.stc_NameTooltipBG:GetSizeY())
    self._ui.stc_NameTooltipBG:SetPosX(name:GetPosX())
    self._ui.stc_NameTooltipBG:SetPosY(self._ui.list2_WarfareInfo:GetPosY() + contents:GetPosY() + btnList:GetSizeY() + contents:GetSizeY() + name:GetSizeY() + self._ui.stc_NameTooltipBG:GetSizeY())
  else
