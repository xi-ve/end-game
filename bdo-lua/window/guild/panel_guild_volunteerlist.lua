...
function PaGlobal_VolunteerListInfoPage:init()
  ...
  self._ui.volunteerListBG:registEvent(__ePAUIList2EventType_LuaChangeContent, "Volunteerlist_ControlCreate")
  self._ui.volunteerListBG:createChildContent(__ePAUIList2ElementManagerType_List)
...
function HandleClicked_VolunteerList_Menu(index)
  ...
  if true == volunteerMember:isOnline() and false == volunteerMember:isGhostMode() then
    local control = self._ui.volunteerListBG:GetContentByKey(index)
    self._subButtonMenuBG:SetPosX(self._ui.staticText_charName:GetPosX() + 150)
    self._subButtonMenuBG:SetPosY(self._ui.listInfoTitleBG:GetSizeY() + control:GetPosY())
...
function HandleClicked_VolunteerList_ActivityButton()
  ...
  if true == _ContentsGroup_NewUI_Guild_All then
    PaGlobalFunc_GuildUseFund_All_ShowToggle(self.curClickVolunteerIdx - 1, true, true)
  else
