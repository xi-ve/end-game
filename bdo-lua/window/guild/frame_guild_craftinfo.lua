...
function guildCraftInfo:Init()
  if nil == Panel_Window_Guild or nil == Panel_Guild_CraftInfo then
    return
  end
  Panel_Guild_CraftInfo:SetShow(false)
  guildCraftInfo.control._frameDefaultBG = UI.getChildControl(Panel_Window_Guild, "Static_Frame_CraftInfoBG")
  guildCraftInfo.control._scroll = UI.getChildControl(Panel_Guild_CraftInfo, "Scroll_CraftInfo")
  ...
  for exchange_Idx = 0, self.config.maxExchangeCount - 1 do
    ...
    exchangeSlot.bg = UI.createAndCopyBasePropertyControl(Panel_Guild_CraftInfo, "Static_FunctionBG", self.control._frameDefaultBG, "Guild_CraftInfo_Exchange_BG_" .. exchange_Idx)
  ...
  guildCraftInfo.control._frameDefaultBG:MoveChilds(guildCraftInfo.control._frameDefaultBG:GetID(), Panel_Guild_CraftInfo)
end
...
function Guild_CraftInfo_EnchantItemToolTip(isShow, enchantItemKeyRaw, exchangeUi_Idx, enchant_Idx)
  ...
  if isShow then
    if true == _ContentsGroup_NewUI_Tooltip_All then
      registTooltipControl(control, Panel_Widget_Tooltip_Item_All)
    else
...
function FGlobal_Guild_CraftInfo_Open(isShow)
  ...
  if true == isShow then
    guildCraftInfo.control._frameDefaultBG:SetShow(true)
    guildCraftInfo:Update()
  else
    guildCraftInfo.control._frameDefaultBG:SetShow(false)
  end
end
...
function FromClient_GuildCraftInfo_Init()
  PaGlobal_GuildCraftInfo_Init()
end
function PaGlobal_GuildCraftInfo_Init()
  guildCraftInfo:Init()
  guildCraftInfo:registEventHandler()
end
registerEvent("FromClient_luaLoadComplete", "FromClient_GuildCraftInfo_Init")
