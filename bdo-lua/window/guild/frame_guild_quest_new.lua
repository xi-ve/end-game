...
function GuildQuestInfoPage:initialize()
  ...
  self._btnBattle:SetTextMode(__eTextMode_LimitText)
  self._btnLife:SetTextMode(__eTextMode_LimitText)
  self._btnTrade:SetTextMode(__eTextMode_LimitText)
  ...
  function createQuestListInfo(pIndex)
    ...
    rtGuildQuestListInfo._list_Desc:SetIgnore(false)
  ...
  for index = 0, self._constGuildQuestMaxCount - 1 do
    ...
    if questIndex < #self._currentQuestList then
      ...
      if nil ~= guildQuestList then
        ...
        if self._list[index]._list_Desc:IsLimitText() then
          self._list[index]._list_Desc:addInputEvent("Mouse_On", "GuildQuest_Simpletooltips(true," .. index .. ")")
          self._list[index]._list_Desc:addInputEvent("Mouse_Out", "GuildQuest_Simpletooltips(false)")
        else
          self._list[index]._list_Desc:addInputEvent("Mouse_On", "")
          self._list[index]._list_Desc:addInputEvent("Mouse_Out", "")
        end
...
function GuildQuest_Simpletooltips(isShow, index)
  if nil == Panel_Window_Guild then
    return
  end
  if not isShow then
  ...
  local control = GuildQuestInfoPage._list[index]._list_Desc
  local name = PAGetString(Defines.StringSheet_GAME, "LUA_GUILDQUEST_TOOLTIP_NAME")
