...
function PaGlobal_AutoQuestManager:mouseProgress()
  ...
  elseif self._stateType == stateTypeValue.startQuestToNpc then
    ...
    elseif true == _ContentsGroup_RenewUI_Dailog then
      buttonPosition = PaGlobalFunc_MainDialog_Bottom_GetFuncPositionNewQuestButton()
      if false == buttonPosition._Return then
        return
      end
      local sizeX, sizeY = PaGlobalFunc_MainDialog_Bottom_GetFuncButtonSizeXY()
      dialogPosY = Panel_Npc_Dialog_All:GetPosY()
      mouseX = buttonPosition._PosX + sizeX / 2
      mouseY = PaGlobalFunc_MainDialog_Bottom_GetSizeY() + buttonPosition._PosY + sizeY / 2
    else
  ...
  elseif self._stateType == stateTypeValue.endQuestToNpc then
    ...
    if selCount ~= 0 then
      ...
      if true == _ContentsGroup_NewUI_Dialog_All then
        ...
        buttonPosition = FGlobal_getSelectRewardPosition()
      end
