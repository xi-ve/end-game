...
function QuickSlot_DropHandler(slotIndex)
  ...
  elseif DragManager.dragStartPanel == Panel_Window_SkillGroup_QuickSlot and slotIndex ~= DragManager.dragSlotInfo then
    quickSlot_Swap(slotIndex, DragManager.dragSlotInfo)
    PaGlobal_SkillGroup_QuickSlot:update()
  end
...
function QuickSlot_UpdatePerFrame(fDeltaTime)
  ...
  for idx, slot in ipairs(self.slots) do
    ...
    if CppEnums.QuickSlotType.eItem == quickSlotInfo._type or CppEnums.QuickSlotType.eCashItem == quickSlotInfo._type or CppEnums.QuickSlotType.eInstanceItem == quickSlotInfo._type then
      ...
      if isNearFusionCore() and isFusionItem(whereType, slot.keyValue) and 3 < onEffectTime then
...
local closeNpcTalkDialog = function()
  ...
  elseif Panel_Window_Repair:IsShow() then
    if true == _ContentsGroup_NewUI_Equipment_All then
      PaGlobalFunc_Equipment_All_SetShow(false)
    else
      FGlobal_Equipment_SetHide(false)
  ...
  if false == _ContentsGroup_NewUI_Servant_All then
    if true == Panel_Window_StableFunction:IsShow() and nil ~= Panel_Window_StableFunction then
      StableFunction_Close()
      return true
    end
    if true == Panel_Window_GuildStableFunction:IsShow() and nil ~= Panel_Window_GuildStableFunction then
      GuildStableFunction_Close()
      return true
    end
    if true == Panel_Window_WharfFunction:GetShow() and nil ~= Panel_Window_WharfFunction then
      WharfFunction_Close()
      return true
    end
    if true == Panel_Window_GuildWharfFunction:GetShow() and nil ~= Panel_Window_GuildWharfFunction then
      GuildWharfFunction_Close()
      return true
    end
  elseif true == Panel_Dialog_ServantFunction_All:GetShow() and nil ~= Panel_Dialog_ServantFunction_All then
