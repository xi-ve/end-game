...
function bossWave:Init()
  for index = 0, self._maxWaveCount - 1 do
    local temp = {}
    temp._icon = UI.createControl(__ePAUIControl_Static, Panel_BossWave, "Static_BossWave_Icon_" .. index)
    CopyBaseProperty(self._icon, temp._icon)
    temp._icon:SetPosY(self._icon:GetPosY() + 40 * index)
    temp._clearIcon = UI.createControl(__ePAUIControl_Static, Panel_BossWave, "Static_BossWave_ClearIcon_" .. index)
    ...
    temp._hpProgressBg = UI.createControl(__ePAUIControl_Static, Panel_BossWave, "Static_BossWave_HpProgressBg_" .. index)
    CopyBaseProperty(self._hpProgressBg, temp._hpProgressBg)
    temp._hpProgressBg:SetPosY(self._hpProgressBg:GetPosY() + 40 * index)
    temp._hpProgress = UI.createControl(__ePAUIControl_Progress2, Panel_BossWave, "Progress2_BossWave_HpProgress_" .. index)
    ...
    temp._bossName = UI.createControl(__ePAUIControl_StaticText, Panel_BossWave, "StaticText_BossWave_Name_" .. index)
...
function BossWave_Check(deltaTime)
  ...
  if 10 < clearTime then
    bossWave:Close()
  end
end
function BossWave_ResetPosition()
  if _ContentsGroup_RenewUI then
    Panel_BossWave:SetPosX(Panel_MainQuest:GetPosX() - Panel_BossWave:GetSizeX() - 10)
    Panel_BossWave:SetPosY(Panel_MainQuest:GetPosY())
  end
end
function bossWave:registerEvent()
  ...
  registerEvent("onScreenResize", "BossWave_ResetPosition")
