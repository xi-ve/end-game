...
function TownfunctionNavi_Set()
  ...
  for index = 1, UI_ST.eSpawnType_Count - 1 do
    local function pushTownNpcIcon(i, index)
      ...
      local tempIcon
      if _ContentsGroup_NewUI_NpcNavi_All then
        local npcNaviTopArea = UI.getChildControl(Panel_NpcNavi_All, "Static_TopInsdieBG")
        tempIcon = UI.createControl(__ePAUIControl_StaticText, npcNaviTopArea, "NpcNavi_Icon_" .. index)
      else
        tempIcon = UI.createControl(__ePAUIControl_StaticText, Panel_NpcNavi, "NpcNavi_Icon_" .. index)
      end
      ...
      npcNaviIcon[i]:SetTextMode(__eTextMode_LimitText)
      ...
      if not isInstrument and UI_ST.eSpawnType_Instrument == index then
        npcNaviIcon[i]:SetShow(false)
      end
  ...
  for ii = 0, iconTypeCount - 1 do
    if true == npcNaviIcon[ii]:GetShow() then
      local row = math.floor(settingIndex / slotConfig.slotCols)
      local col = settingIndex % slotConfig.slotCols
  ...
  if true == _ContentsGroup_NewUI_NpcNavi_All then
    FGlobal_NpcNavi_PanelSizeSetting(settingIndex)
  end
end
...
function FGlobal_TownNpcNavi_UpdatePerFrame(deltaTime)
  ...
  if 10 < townNaviEffectTimeCheck then
