  ...
  if value.isOpen then
    ...
    if 0 < value.imageCount then
      ...
      for index = 1, value.imageCount do
        local targetControl = UI.createControl(__ePAUIControl_Static, _panel, "Static_ServerSelectBg_" .. imageIndex)
...
function ServerSelectRemaster:init()
  if false == _isInitialized then
    ...
    self._ui.stc_channelDesc[CHANNEL_TYPE.SINGLE_WORLD_SYSTEM] = UI.getChildControl(self._ui.stc_rightBg, "SINGLE_WORLD_SYSTEM")
    ...
    self._ui.btn_mainServer:SetTextMode(__eTextMode_AutoWrap)
    ...
    self._ui.btn_recentServer:SetTextMode(__eTextMode_AutoWrap)
...
function ServerSelectRemaster:PlayMovie()
  ...
  if nil == _ui_web_loadingMovie then
    _ui_web_loadingMovie = UI.createControl(__ePAUIControl_WebControl, self._ui.stc_movieBG, "Static_BgMovie")
  end
...
function ServerSelectRemaster:initTreeData()
  ...
  tree:registEvent(__ePAUIList2EventType_LuaChangeContent, "PaGlobal_ServerSelect_ServerList_ControlCreate")
...
function PaGlobal_ServerSelect_CreateChannelBranch(content, key)
  ...
  local stc_prepareToStop = UI.getChildControl(channelButtonBG, "Static_PrepareToStop")
  ...
  stc_prepareToStop:SetShow(false)
  ...
  if true == isPrepareToStopServer then
    stateStr = PAGetString(Defines.StringSheet_GAME, "LUA_PREPARE_TO_STOP_SERVER")
    txt_State:SetSpanSize(35, 2)
    stc_prepareToStop:SetShow(true)
...
function ServerSelectRemaster:showChannelIcon(buttonBG, iconIndex, monotone)
  for ii = 1, 4 do
    ...
    if false == stc:GetShow() then
      stc:ChangeTextureInfoName("Combine/Icon/Combine_Basic_Icon_00.dds")
...
function InputMLUp_ServerSelect_EnterLastJoinedServer()
  ...
  for ii = 1, #self._treeData do
    if nil ~= self._treeData[ii].channelIndex then
      ...
      if nil ~= serverData and lastJoinServerNo == serverData._serverNo and false == self._isConnecting then
...
function InputMLUp_ServerSelect_EnterRandomServer()
  ...
  if false == self._isConnecting then
    self._isConnecting = true
...
function InputMLUp_ServerSelect_EnterMainServer()
  ...
  for ii = 1, #self._treeData do
    if nil ~= self._treeData[ii].channelIndex then
      ...
      if nil ~= serverData and mainServerNo == serverData._serverNo and false == self._isConnecting then
        self._isConnecting = true
        PaGlobalFunc_ServerSelectRemaster_EnterMemorizedChannel(self._treeData[ii].channelIndex)
        break
      end
...
function PaGlobal_ServerSelect_PerFrameUpdate(deltaTime)
  ...
  if false == _startAnimationFlag then
    ...
    if 1 < _animationDelayCount then
