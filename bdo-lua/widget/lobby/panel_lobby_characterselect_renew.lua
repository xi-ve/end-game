...
local CharacterSelect = {
  _ui = {
    ...
    txt_RemainTime = UI.getChildControl(_panel, "StaticText_RemainTime"),
...
function CharacterSelect:init()
  ...
  self._ui.list2_Character:registEvent(__ePAUIList2EventType_LuaChangeContent, "PaGlobal_CharacterSelect_CharacterList_ControlCreate")
  ...
  self._ui.txt_RemainTime:SetPosY(self._ui.txt_CharacterSelect:GetPosY() + self._ui.txt_CharacterSelect:GetTextSizeY() * 2)
...
function CharacterSelect:updateData(isChangeSpecialTab, isPushKey)
...
function CharacterSelect:characterView(index, classType, isSpecialCharacter, isChangeSpecialTab)
  ...
  elseif classType == __eClassType_Guardian then
    viewCharacter(index, -20, 0, -94, -0.4, isSpecialCharacter, isChangeSpecialTab)
    viewCharacterPitchRoll(0, 0)
    viewCharacterFov(0.75)
    setWeatherTime(8, 8)
  else
...
function CharacterSelect:changeTexture_Class(control, classType)
  ...
  elseif classType == __eClassType_RangerMan then
    ...
    if classType == __eClassType_Guardian then
      control:ChangeTextureInfoName("renewal/ui_icon/console_classsymbol.dds")
      local x1, y1, x2, y2 = setTextureUV_Func(control, 115, 115, 171, 171)
      control:getBaseTexture():setUV(x1, y1, x2, y2)
      control:setRenderTexture(control:getBaseTexture())
    else
    end
...
function PaGlobal_CharacterSelect_SelectCharacter(charIdx, isRefresh)
  ...
  self._ui.txt_Delete_ConsoleUI:SetShow(false)
  self._ui.txt_DeleteCancel_ConsoleUI:SetShow(false)
  self._ui.txt_RemainTime:SetShow(false)
  if nil ~= characterData then
    local classType = getCharacterClassType(characterData)
    local removeTime = getCharacterDataRemoveTime(charIdx, isSpecialCharacter)
    if nil ~= removeTime then
      self._ui.txt_RemainTime:SetShow(true)
      self._ui.txt_RemainTime:SetFontColor(Defines.Color.C_FFBA2737)
      self._ui.txt_RemainTime:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_CHARACTER_DELETE") .. " ( " .. removeTime .. " )")
    end
...
function InputMLUp_CharacterSelect_PlayGame()
  ...
  if nil ~= characterData then
    if getContentsServiceType() == CppEnums.ContentsServiceType.eContentsServiceType_CBT or getContentsServiceType() == CppEnums.ContentsServiceType.eContentsServiceType_OBT or getContentsServiceType() == CppEnums.ContentsServiceType.eContentsServiceType_Commercial then
      if 1 == characterData._level and 1 == characterCount and false == _ContentsGroup_UsePadSnapping then
...
function FromClient_CharacterSelect_FailToRemoveCharacter()
  local self = CharacterSelect
  self._isBlockSelectCharacter = false
end
function CharacterSelect:registEventHandler()
  ...
  registerEvent("FromClient_FailToRemoveCharacter", "FromClient_CharacterSelect_FailToRemoveCharacter")
