...
function ClassSelect:open()
  ...
  for classIdx = 0, count - 1 do
    classType = getPossibleClassTypeFromIndex(classIdx)
    if false == FGlobal_getIsSpecialCharacter() and classType == __eClassType_Hashashin and nil ~= self:getClassButtonData(classType) then
      self._ui.list_ClassSelect:getElementManager():pushKey(toInt64(0, classType))
  ...
  for classIdx = 0, count - 1 do
    classType = getPossibleClassTypeFromIndex(classIdx)
    if classType ~= __eClassType_Hashashin and nil ~= self:getClassButtonData(classType) then
      self._ui.list_ClassSelect:getElementManager():pushKey(toInt64(0, classType))
      if nil == topClassType then
        topClassType = classType
      end
...
function ClassSelect:getClassButtonData(classType)
  ...
  elseif classType == __eClassType_Valkyrie then
    ...
    buttonData.mainWeaponType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_WEAPONTYPE_LONGSWORD")
    buttonData.subWeaponType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_WEAPONTYPE_SHIELD")
    buttonData.manaType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_MPTYPE_SP")
  ...
  elseif classType == __eClassType_Hashashin then
    ...
    buttonData.mainWeaponType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_WEAPONTYPE_SAGOKDO")
    buttonData.subWeaponType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_WEAPONTYPE_GEUMGAE")
    buttonData.manaType = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CLASSSELECT_MPTYPE_EP")
