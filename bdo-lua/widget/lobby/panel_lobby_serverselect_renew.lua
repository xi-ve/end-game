...
local openContent = _ContentsGroup_UsePadSnapping
  ...
  if value.isOpen then
    ...
    if 0 < value.imageCount then
      ...
      for index = 1, value.imageCount do
        ...
        targetControl:SetSize(getOriginScreenSizeX(), getOriginScreenSizeY())
...
local _channelDescriptionData = {
  [CHANNEL_TYPE.SINGLE_WORLD_SYSTEM] = {
    ...
    desc = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SERVERSELECT_CHANNELSELECTDESC_CONSOLE")
  },
...
function ServerSelect:init()
  ...
  self._ui.list_Server:registEvent(__ePAUIList2EventType_LuaChangeContent, "PaGlobal_ServerSelect_ServerList_ControlCreate")
  self._ui.list_Server:createChildContent(__ePAUIList2ElementManagerType_List)
  ...
  if _ContentsGroup_XB_Obt then
    self._ui.stc_leftBg:SetShow(false)
    self._ui.txt_ServerSelectTitle:SetShow(false)
  end
end
...
function ServerSelect:initListData_XXX()
  ...
  for serverIndex = 0, serverCount - 1 do
    if false == ToClient_SelfPlayerIsGM() then
      local channelServerData = getGameChannelServerDataByIndex(worldIdx, serverIndex)
...
function PaGlobal_ServerSelect_ServerList_ControlCreate(content, key)
  ...
  if true == _ContentsGroup_UsePadSnapping and true == ToClient_isServiceMaintenance() then
    busyState = 0
  end
