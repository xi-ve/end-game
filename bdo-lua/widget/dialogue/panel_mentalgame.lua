...
local function uiInit()
  ...
  zodiac.control = UI.createControl(__ePAUIControl_Static, zodiac.panel, "ZodiacCenterPanelImage")
  ...
  tooltip.nextBonus:SetTextMode(__eTextMode_AutoWrap)
  left.gameDesc:SetTextMode(__eTextMode_AutoWrap)
  ...
  left.comboCount:SetTextMode(__eTextMode_LimitText)
  left.failCount:SetTextMode(__eTextMode_LimitText)
  left.cumulativePoint:SetTextMode(__eTextMode_LimitText)
  left.bestPoint:SetTextMode(__eTextMode_LimitText)
...
local function insertCardList()
  ...
  for index = 0, count - 1 do
    ...
    if nil == rightCardList[index].ui.cardText then
      aTextControl[index] = createControlRight(right.panel, __ePAUIControl_StaticText, "cardText", rightCardList[index], index)
      ...
      aControl[index] = createControlRight(aTextControl[index], __ePAUIControl_Static, "cardIcon", rightCardList[index], index)
...
local function giftIconInit()
  ...
  for index = 0, count - 1 do
    ...
    if 0 <= percent and percent <= 1 and ToClient_checkIntimacyInformationFixedState(intimacyInformationData) then
      ...
      if nil == target then
        target = UI.createControl(__ePAUIControl_Static, left.panel, "GiftIcon_" .. tostring(index))
