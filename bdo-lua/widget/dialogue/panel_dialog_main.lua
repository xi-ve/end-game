...
local intimacyNoticeText = UI.createControl(__ePAUIControl_StaticText, intimacyNotice, "intimacyNoticeText")
...
function FGlobal_Dialog_CheckConditionTutorialMenuButton()
  ...
  if 10 <= playerLevel and playerLevel <= 40 and false and -1 ~= FGlobal_Dialog_FindFuncButtonIndexByType(CppEnums.ContentsType.Contents_Shop) then
  ...
  if 25 <= playerLevel and playerLevel <= 53 and false and -1 ~= FGlobal_Dialog_FindFuncButtonIndexByType(CppEnums.ContentsType.Contents_Enchant) then
    return not _ContentsGroup_isUsedNewEnchant
  end
  if 40 <= playerLevel and playerLevel <= 56 and false and -1 ~= FGlobal_Dialog_FindFuncButtonIndexByType(CppEnums.ContentsType.Contents_Extract) then
    return true
  end
  if 50 <= playerLevel and playerLevel <= 56 and false and -1 ~= FGlobal_Dialog_FindFuncButtonIndexByType(CppEnums.ContentsType.Contents_Enchant) then
    return true
  end
  return false
end
function FGlobal_Dialog_ShowTutorialStartButtonList()
  ...
  if 10 <= playerLevel and playerLevel <= 40 and -1 ~= FGlobal_Dialog_FindFuncButtonIndexByType(CppEnums.ContentsType.Contents_Shop) then
    local talker = dialog_getTalker()
    if nil == talker then
      return false
    end
  ...
  if 25 <= playerLevel and playerLevel <= 53 and -1 ~= FGlobal_Dialog_FindFuncButtonIndexByType(CppEnums.ContentsType.Contents_Enchant) then
    placePos(_tutorialStartButtonList[2], orderNum)
    orderNum = orderNum + 1
  end
  if 40 <= playerLevel and playerLevel <= 56 and -1 ~= FGlobal_Dialog_FindFuncButtonIndexByType(CppEnums.ContentsType.Contents_Extract) then
    placePos(_tutorialStartButtonList[3], orderNum)
    orderNum = orderNum + 1
  end
  if 50 <= playerLevel and playerLevel <= 56 and -1 ~= FGlobal_Dialog_FindFuncButtonIndexByType(CppEnums.ContentsType.Contents_Enchant) then
...
function Panel_Npc_Dialog:Initialize()
  for index = 0, 3 do
    ...
    _uiDialogIcon[index] = UI.createControl(__ePAUIControl_Static, _uiDialogButton[index], "StaticText_DialogIcon_" .. tostring(index))
  ...
  for index = 0, 5 do
    ...
    if not isGameTypeKorea() then
      _uiFuncButton[index]:SetTextMode(__eTextMode_LimitText)
    end
  end
  for index = 0, 5 do
  ...
  for index = 0, 3 do
    _uiNoticeNeedInfo[index] = UI.createControl(__ePAUIControl_StaticText, Panel_Npc_Dialog, "StaticText_Notice_" .. tostring(index))
  ...
  for index = 0, 3 do
    _uiNeedWpAni[index] = UI.createControl(__ePAUIControl_Static, Panel_Npc_Dialog, "Static_NeedWpAni_" .. tostring(index))
  ...
  for index = 0, 3 do
    _uiIntimacyIcon[index] = UI.createControl(__ePAUIControl_Static, Panel_Npc_Dialog, "Static_Intimacy_Button_" .. tostring(index))
...
local function PreclosePanel_OpenDialog()
  ...
  elseif Panel_ColorBalance:GetShow() then
      ...
      for index = 0, count - 1 do
        ...
        if nil ~= intimacyInformationData then
          ...
          if 0 <= percent and percent <= 1 and ToClient_checkIntimacyInformationFixedState(intimacyInformationData) then
            ...
            if nil == target then
              target = UI.createControl(__ePAUIControl_Static, Panel_Npc_Dialog, "GiftIcon_" .. tostring(index))
...
function Dialog_updateButtons(isVisible)
  ...
  local isBlackStone_16001 = false
  local isBlackStone_16002 = false
  local value_IsSocket = false
  if true == _ContentsGroup_NewUI_Inventory_All then
    isBlackStone_16001 = PaGlobalFunc_Inventory_All_HasWeaponBlackStone()
    isBlackStone_16002 = PaGlobalFunc_Inventory_All_HasArmorBlackStone()
    value_IsSocket = PaGlobalFunc_Inventory_All_HasSocketItem()
  else
    isBlackStone_16001 = Panel_Inventory_isBlackStone_16001
    isBlackStone_16002 = Panel_Inventory_isBlackStone_16002
    value_IsSocket = Panel_Inventory_isSocketItem
  end
  local dialogData = ToClient_GetCurrentDialogData()
  if nil == dialogData then
    return
  ...
  if 4 < dialogButtonCount and true == isVisible then
  ...
  for i = _dialogIndex, dialogButtonCount - 1 do
    ...
    if nil ~= talker then
      ...
      for indexIntimacy = 0, count - 1 do
        ...
        if giftDesc == dialogButton:getText() and 1 < intimacyValue then
  ...
  for questIndex = 1, progressQuestCount do
    ...
    if 0 < groupCount and groupCount >= progressQuest:getQuestGroupQuestNo() then
    ...
    if isGuildStable() then
      if false == _ContentsGroup_NewUI_Servant_All then
        if CppEnums.ServantType.Type_Vehicle == stable_getServantType() then
          if nil ~= Panel_Window_GuildStableFunction then
            GuildStableFunction_Open()
          end
        elseif CppEnums.ServantType.Type_Ship == stable_getServantType() and nil ~= Panel_Window_GuildWharfFunction then
          GuildWharfFunction_Open()
        end
      elseif nil ~= Panel_Dialog_ServantFunction_All then
      ...
      if false == _ContentsGroup_NewUI_Servant_All then
        if CppEnums.ServantType.Type_Vehicle == stable_getServantType() then
          if false == _ContentsGroup_NewUI_Servant_All and nil ~= Panel_Window_StableFunction then
            StableFunction_Open()
          end
...
function HandleClickedFuncButton(index)
  ...
  elseif Panel_Window_Enchant:GetShow() then
    if true == PaGlobal_Enchant_IsAnimating() then
      return
    end
  ...
  elseif CppEnums.ContentsType.Contents_Stable == funcButtonType then
    if false == _ContentsGroup_NewUI_Servant_All then
      if nil ~= Panel_Window_GuildStableFunction and nil ~= Panel_Window_GuildWharfFunction and nil ~= Panel_Window_StableFunction and nil ~= Panel_Window_WharfFunction then
        if isGuildStable() then
          if CppEnums.ServantType.Type_Vehicle == stable_getServantType() then
            GuildStableFunction_Open()
          elseif CppEnums.ServantType.Type_Ship == stable_getServantType() then
  ...
  elseif CppEnums.ContentsType.Contents_Employee == funcButtonType then
    local talker = dialog_getTalker()
    if nil ~= talker then
    ...
    do
      ...
      elseif true == ToClient_isAnyLockedItem(ItemEnchantKey(dialogButton:getNeedItemKey())) then
        ...
        do
          ...
          if UI_BTN_TYPE.eDialogButton_Exchange == dialogButton._dialogButtonType and 0 < needItemCount then
            ...
            if itemStaticWrapper ~= nil then
              ...
              if 0 < itemCount then
                ...
                if 1 < exchangeCount and dialogButton._isValidMultipleExchange then
...
function FGlobal_HideDialog()
  ...
  if true == _ContentsGroup_NewUI_SpiritEnchant_All then
    if nil ~= Panel_Window_StackExtraction_All and true == Panel_Window_StackExtraction_All:GetShow() then
      if true == PaGlobal_SpiritEnchant_All_IsAnimating() then
        return
      end
      PaGlobalFunc_SpiritEnchant_All_Close()
    end
...
function FruitageItem_ShowTooltip(percent)
  ...
  local noticePosY = getMousePosY() - 35
  local noticeSizeY = giftNotice:GetSizeY()
  if screenSizeY > noticePosY + noticeSizeY then
    giftNotice:SetPosY(noticePosY - Panel_Npc_Dialog:GetPosY())
  else
    local gap = noticePosY + noticeSizeY - screenSizeY
    giftNotice:SetPosY(noticePosY - Panel_Npc_Dialog:GetPosY() - gap - 15)
  end
...
function FruitageValue_ShowTooltip(isShow)
  ...
  intimacyNoticeText:SetTextMode(__eTextMode_AutoWrap)
...
function Panel_Dialog_EnchantHelp_Func(isOn)
  ...
  if isOn == true then
    ...
    _txt_EnchantHelp_Desc:SetTextMode(__eTextMode_AutoWrap)
...
function Panel_Dialog_SocketHelp_Func(isOn)
  ...
  if isOn == true then
    ...
    _txt_SocketHelp_Desc:SetTextMode(__eTextMode_AutoWrap)
...
function wpHelp_Func(isOn)
  ...
  if Wp < 10 and isOn == true then
    ...
    _wpHelp:SetTextMode(__eTextMode_AutoWrap)
...
function FromClient_CloseAllPanelWhenNpcGoHome()
  ...
  if GetUIMode() == Defines.UIMode.eUIMode_Trade then
    if false == _ContentsGroup_NewUI_TradeMarket_All then
      if nil ~= Panel_Npc_Trade_Market and true == Panel_Npc_Trade_Market:GetShow() then
        closeNpcTrade_Basket()
      end
    elseif nil ~= Panel_Dialog_Trade_Function_All and true == Panel_Dialog_Trade_Function_All:GetShow() then
...
function FGlobal_CloseNpcDialogForDetail()
  ...
  elseif nil ~= Panel_Npc_Trade_Market and Panel_Npc_Trade_Market:IsShow() then
    closeNpcTrade_Basket()
    return true
  end
  if false == _ContentsGroup_NewUI_Servant_All and nil ~= Panel_Window_StableFunction and true == Panel_Window_StableFunction:IsShow() then
    StableFunction_Close()
  elseif nil ~= Panel_Dialog_ServantFunction_All and true == Panel_Dialog_ServantFunction_All:GetShow() then
    PaGlobalFunc_ServantFunction_All_Close()
  end
  if false == _ContentsGroup_NewUI_ExtractionFunction_All then
    if nil ~= Panel_Window_Extraction and Panel_Window_Extraction:IsShow() then
      PaGlobal_Extraction:openPanel(false)
      retval = true
    end
  elseif nil ~= Panel_Window_Extraction_All and Panel_Window_Extraction_All:IsShow() then
    PaGlobal_Extraction_All_Close()
    retval = true
  end
  if false == _ContentsGroup_NewUI_BlackSmith_All then
  ...
  if true == _ContentsGroup_NewUI_RepairFunction_All then
    if nil ~= Panel_Dialog_Repair_Function_All and true == Panel_Dialog_Repair_Function_All:IsShow() and (true ~= _ContentsGroup_NewUI_RepairFunction_All and false == PaGlobal_Camp:getIsCamping() or true == _ContentsGroup_NewUI_RepairFunction_All and false == PaGlobalFunc_Camp_All_GetIsCamping()) then
      Panel_FixEquip:SetShow(false)
      if false == _ContentsGroup_NewUI_BlackSmith_All then
        Panel_FixEquip:SetShow(false)
      else
  ...
  elseif Panel_Window_Repair:IsShow() and (true ~= _ContentsGroup_NewUI_RepairFunction_All and false == PaGlobal_Camp:getIsCamping() or true == _ContentsGroup_NewUI_RepairFunction_All and false == PaGlobalFunc_Camp_All_GetIsCamping()) then
    Panel_FixEquip:SetShow(false)
    if false == _ContentsGroup_NewUI_BlackSmith_All then
      Panel_FixEquip:SetShow(false)
    else
  ...
  if false == _ContentsGroup_NewUI_Servant_All and nil ~= Panel_Window_GuildStableFunction then
  ...
  if false == _ContentsGroup_NewUI_Servant_All and nil ~= Panel_Window_WharfFunction then
    if true == Panel_Window_WharfFunction:GetShow() then
      WharfFunction_Close()
      return true
    end
  elseif nil ~= Panel_Dialog_ServantFunction_All and true == Panel_Dialog_ServantFunction_All:GetShow() then
    PaGlobalFunc_ServantFunction_All_ForceClose()
    return true
  end
  if false == _ContentsGroup_NewUI_Servant_All and nil ~= Panel_Window_GuildWharfFunction then
    if true == Panel_Window_GuildWharfFunction:GetShow() then
