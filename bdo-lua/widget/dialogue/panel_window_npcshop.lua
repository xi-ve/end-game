...
function npcShop:init()
  ...
  self.radioButtons[0]:SetTextMode(__eTextMode_LimitText)
  self.radioButtons[1]:SetTextMode(__eTextMode_LimitText)
  self.radioButtons[2]:SetTextMode(__eTextMode_LimitText)
  ...
  _npcShopHelp:SetTextMode(__eTextMode_AutoWrap)
end
...
function npcShop:createSlot()
  ...
  for ii = 1, self.config.slotRows do
    for jj = 1, self.config.slotCols do
      ...
      slot.slotNo = index - 1
      slot.button = UI.createControl(__ePAUIControl_Button, Panel_Window_NpcShop, "NpcShop_Button_" .. strId)
      ...
      slot.price = UI.createControl(__ePAUIControl_StaticText, Panel_Window_NpcShop, "StaticText_Price_" .. strId)
      CopyBaseProperty(self.template.staticCurrentPrice, slot.price)
      slot.coinIcon = UI.createControl(__ePAUIControl_Static, Panel_Window_NpcShop, "Static_CoinIcon_" .. strId)
      CopyBaseProperty(self.template.static_CoinIcon, slot.coinIcon)
      slot.remainCount = UI.createControl(__ePAUIControl_StaticText, Panel_Window_NpcShop, "StaticText_RemainCount_" .. strId)
      CopyBaseProperty(self.template.staticRemainCount, slot.remainCount)
      slot.trend = UI.createControl(__ePAUIControl_StaticText, Panel_Window_NpcShop, "StaticText_Trend_" .. strId)
      ...
      slot.selectEffect = UI.createControl(__ePAUIControl_Static, Panel_Window_NpcShop, "Button_Effect_" .. strId)
      CopyBaseProperty(self.template.buttonSelected, slot.selectEffect)
      slot.invenCount = UI.createControl(__ePAUIControl_StaticText, Panel_Window_NpcShop, "StaticText_InventoryCount" .. strId)
      CopyBaseProperty(self.template.staticInvenCount, slot.invenCount)
      slot.rentTime = UI.createControl(__ePAUIControl_StaticText, Panel_Window_NpcShop, "StaticText_RentTime_" .. strId)
      ...
      function slot:setItem(itemStaticWrapper, s64_stackCount, s64_price, s64_invenCount, rentTime, Intimacy, disable)
        ...
        self.button:SetTextMode(__eTextMode_Limit_AutoWrap)
        ...
        if nil ~= Intimacy and 0 < Intimacy and (false == npcShop._isCamping or nil == npcShop._isCamping) then
...
function NpcShop_UpdateContent()
  ...
  if _ContentsGroup_NewUI_Camp_All then
    npcShop._isCamping = PaGlobalFunc_Camp_All_GetIsCamping()
  else
  ...
  if false == npcShop._isCamping and false == npcShop_isShopContents() and false == npcShop_isGuildShopContents() then
    return
  end
  if false == npcShop._isCamping or nil == npcShop._isCamping then
    local actorProxyWrapper = getNpcActor(talker:getActorKey())
      ...
      elseif self.tabIndexSell == self.lastTabIndex then
        do
          ...
          local function sellDoit()
            ...
            if 2 == toWhereType and Defines.s64_const.s64_0 ~= s64_remainingTime then
          ...
          local itemWrapper = npcShop_getItemWrapperByShopSlotNo(slot.slotNo)
...
function npcShop:registEventHandler()
  ...
  PaGlobal_Util_RegistHelpTooltipEvent(self.buttonQuestion, "\"NpcShop\"")
...
function Panel_NpcShop_InvenRClick_SellItem(itemCount, slotNo)
  ...
  local function checkItemMarket()
    if false ~= itemSSW:isDisplayWorldMarket() and 0 ~= itemEndurance then
      local messageBoxMemo = PAGetString(Defines.StringSheet_GAME, "LUA_NPCSHOP_ITEMMARKET_USE_MSGMEMO")
      local messageBoxData = {
        title = PAGetString(Defines.StringSheet_GAME, "LUA_NPCSHOP_SELL_ALERT_2"),
        content = messageBoxMemo,
...
function npcShop_GetIsCamping()
  return npcShop._isCamping
end
npcShop:init()
npcShop:createSlot()
