local Panel_Dialog_Main_Right_Info = {
  ...
  _enum = {eDefaultIndex = -1, eDefaultDialogSize = 5},
  _value = {
...
function Panel_Dialog_Main_Right_Info:initControl()
  ...
  self._ui.staticText_DialogTitle:SetTextMode(__eTextMode_LimitText)
  ...
  self._ui.list2_Exchange_List:registEvent(__ePAUIList2EventType_LuaChangeContent, "PaGlobalFunc_MainDialog_Right_List2EventControlCreateExchange")
  self._ui.list2_Exchange_List:createChildContent(__ePAUIList2ElementManagerType_List)
  self._ui.static_DialogGroup = UI.getChildControl(self._ui.static_RightBg, "Static_DialogGroup")
  self._ui.frame_DialogButton = UI.getChildControl(self._ui.static_DialogGroup, "Frame_DialogButton")
  self._ui.frame_DialogButton_Content = UI.getChildControl(self._ui.frame_DialogButton, "Frame_1_Content")
  self._ui.frame_DialogButton_VScroll = UI.getChildControl(self._ui.frame_DialogButton, "Frame_1_VerticalScroll")
  self._ui.static_Dialog = UI.getChildControl(self._ui.frame_DialogButton_Content, "Static_Dialog0")
  self._dialogBtnControl[0] = self._ui.static_Dialog
  for index = 1, self._dialogInfo._controlCnt - 1 do
    local contol = UI.cloneControl(self._ui.static_Dialog, self._ui.frame_DialogButton_Content, "Static_Dialog" .. index)
    if nil ~= contol then
      ToClient_padSnapRefreshTarget(contol)
      self._dialogBtnControl[index] = contol
    end
  end
...
function Panel_Dialog_Main_Right_Info:updateDialogList(dialogData, showButton)
  ...
  elseif dialogCount < 5 then
    self._dialogInfo._isScrollShow = false
  else
    self._dialogInfo._isScrollShow = true
  end
  self._value.currentDialogButtonIndex = 0
  self._value.lastDialogButtonIndex = -1
  self._ui.static_DialogGroup:SetShow(true)
  for index = 0, self._dialogInfo._controlCnt - 1 do
    if nil ~= self._dialogBtnControl[index] then
      self._dialogBtnControl[index]:SetShow(false)
    end
  end
  ...
  self._ui.frame_DialogButton_Content:SetSize(self._ui.frame_DialogButton_Content:GetSizeX(), self._dialogInfo._nextPosY)
  local bottomBgSize = PaGlobalFunc_MainDialog_Bottom_GetSizeY()
  if nil == bottomBgSize then
    bottomBgSize = 282
  end
  local dialogGroupSizeY = self._ui.static_DialogGroup:GetSizeY()
  if dialogCount < 4 then
    dialogGroupSizeY = self._dialogInfo._nextPosY
  end
  self._ui.static_RightBg:SetPosY(getScreenSizeY() - (bottomBgSize + self._ui.static_Bg:GetSizeY() + self._ui.staticText_DialogTitle:GetSizeY() + dialogGroupSizeY))
...
function Panel_Dialog_Main_Right_Info:dialogControlCreate(index)
  ...
  local list_content = self._dialogBtnControl[index]
  ...
  text_Dialog:SetTextMode(__eTextMode_AutoWrap)
  text_Dialog:SetText(dialogText)
  if self._dialogInfo._isScrollShow then
    btn_Dialog:SetSize(list_content:GetSizeX() - 15, btn_Dialog:GetSizeY())
  else
    btn_Dialog:SetSize(list_content:GetSizeX(), btn_Dialog:GetSizeY())
  end
  ...
  if isNeedThings then
    ...
    if true == isNeedItem then
      ...
      local posX = button_A:GetPosX() - needItemIcon:GetSizeX() - 70
  ...
  if false == dialogButton._invenPushable then
    btn_Dialog:addInputEvent("Mouse_LUp", "InputAUp_Dialog_Main_Right_ShowInvenFullMessage()")
    btn_Dialog:SetMonoTone(true)
    static_TypeIcon:SetMonoTone(true)
    text_Dialog:SetMonoTone(true)
    textNeed_Dialog:SetMonoTone(true)
    button_A:SetMonoTone(true)
    needItemIcon:SetMonoTone(true)
    needWpIcon:SetMonoTone(true)
  else
    btn_Dialog:SetMonoTone(false)
    static_TypeIcon:SetMonoTone(false)
    text_Dialog:SetMonoTone(false)
    textNeed_Dialog:SetMonoTone(false)
    button_A:SetMonoTone(false)
    needItemIcon:SetMonoTone(false)
    needWpIcon:SetMonoTone(false)
  end
...
function Panel_Dialog_Main_Right_Info:ResizeContents(dialogData, showDialogButton, showExchange)
  ...
  self._ui.frame_DialogButton_VScroll:SetControlPos(0)
  self._ui.frame_DialogButton:UpdateContentPos()
  self._ui.frame_DialogButton:UpdateContentScroll()
end
...
function Panel_Dialog_Main_Right_Info:HandleClickedDialogButton_Trade(index)
  ...
  if self:ExpirationItemCheck(dialogButton:getNeedItemKey()) then
    ...
    do
      ...
      if CppEnums.DialogButtonType.eDialogButton_Exchange == dialogButton._dialogButtonType and 0 < needItemCount then
...
function PaGlobalFunc_MainDialog_Right_GetSizeY()
  local self = Panel_Dialog_Main_Right_Info
  return self._ui.static_RightBg:GetSizeY()
end
function PaGlobalFunc_MainDialog_Right_GetPosY()
  local self = Panel_Dialog_Main_Right_Info
  return self._ui.static_RightBg:GetPosY()
end
...
function PaGlobalFunc_MainDialog_Right_List2EventControlCreateExchange(list_content, key)
  ...
  StaticText_Before:SetTextMode(__eTextMode_Limit_AutoWrap)
  StaticText_Before:setLineCountByLimitAutoWrap(2)
  StaticText_After:SetTextMode(__eTextMode_Limit_AutoWrap)
...
function InputAUp_Dialog_Main_Right_ShowInvenFullMessage()
  Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_PLAYER_INVENTORY_FULL"))
end
