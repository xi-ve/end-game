...
function Panel_Dialog_Main_Info:preclosePanel_OpenMainDialog()
  ...
  if PaGlobalFunc_Camp_All_GetShow() then
...
function PaGlobalFunc_MainDialog_CloseMainDialogForDetail()
  ...
  if false == _ContentsGroup_NewUI_TradeMarket_All and PaGlobal_GetIsTrading() then
  ...
  elseif Panel_Window_StableFunction:IsShow() and nil ~= Panel_Window_StableFunction then
  ...
  if true == _ContentsGroup_RenewUI_Repair then
    ...
    if Panel_Window_Repair:IsShow() and nil ~= Panel_Window_Camp_All and false == PaGlobalFunc_Camp_All_GetIsCamping() then
      Panel_FixEquip:SetShow(false)
      PaGlobal_Repair:repair_OpenPanel(false, nil)
      retval = true
    end
  ...
  elseif false == _ContentsGroup_RenewUI_Stable then
    if Panel_Window_StableFunction:GetShow() and nil ~= Panel_Window_StableFunction then
      StableFunction_Close()
      retval = true
    end
    if Panel_Window_WharfFunction:GetShow() and nil ~= Panel_Window_WharfFunction then
      WharfFunction_Close()
      retval = true
    end
    if Panel_Window_GuildWharfFunction:GetShow() and nil ~= Panel_Window_GuildWharfFunction then
      GuildWharfFunction_Close()
      retval = true
    end
    if Panel_Window_GuildStableFunction:IsShow() and nil ~= Panel_Window_GuildStableFunction then
      GuildStableFunction_Close()
      retval = true
    end
  else
    if PaGlobalFunc_StableFunction_GetShow() and nil ~= Panel_Window_StableFunction_Renew then
      StableFunction_Close()
      retval = true
    end
    if PaGlobalFunc_WharfFunction_GetShow() and nil ~= Panel_Window_WharfFunction_Renew then
      WharfFunction_Close()
      retval = true
    end
  end
...
function FromClient_CloseAllPanelWhenNpcGoHome()
  ...
  if GetUIMode() == Defines.UIMode.eUIMode_Trade and false == _ContentsGroup_NewUI_TradeMarket_All then
    closeNpcTrade_Basket()
  else
  end
