...
function randomWorker:update(workerShopSlotNo)
  ...
  for index = 0, sellCount - 1 do
    ...
    if workerShopSlotNo == shopItem.shopSlotNo then
      ...
      local currentWP = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_WORKERRANDOMSELECT_NOWWPVAL", "getWP", MyWp)
...
function FGlobalFunc_Open_RandomWorker(shopType, slotNo)
  ...
  if true == _ContentsGroup_NewUI_Dialog_All then
    if nil ~= Panel_Npc_Dialog_All and false == Panel_Npc_Dialog_All:GetShow() then
      return
    end
  elseif true == PaGlobal_DialogMain_GetAlreadyClose() then
    return
  end
