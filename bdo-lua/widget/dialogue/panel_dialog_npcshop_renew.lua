...
function ItemSlotTemplete:New()
  ...
  function slot:setItem(itemStaticWrapper, s64_stackCount, s64_price, s64_invenCount, rentTime, Intimacy, disable)
    ...
    if nil ~= Intimacy and 0 < Intimacy and (false == shopInfo._value.isCamping or nil == shopInfo._value.isCamping) then
...
function Panel_Dialog_NPCShop_Info:resize()
  ...
  if true == isInvenOpen then
    ...
    local startPosX = invenPosX - _panel:GetSizeX() - 60
    if 0 < startPosX then
...
function Panel_Dialog_NPCShop_Info:createSlot()
  ...
  for row = 0, self._config.slotRows - 1 do
    for col = 0, self._config.slotCols - 1 do
      ...
      slot.slotNo = index
      if index % 2 == 0 then
        slot.radioButton = UI.createControl(__ePAUIControl_RadioButton, self._ui.frame_Content, "Content_Item_Button_" .. strId)
        CopyBaseProperty(self._ui.radioButton_Left_templete, slot.radioButton)
        slot.item_Slot = UI.createControl(__ePAUIControl_Static, slot.radioButton, "Content_Item_Slot_" .. strId)
        CopyBaseProperty(self._ui.static_Item_Slot_Left_templete, slot.item_Slot)
        slot.staticText_Name = UI.createControl(__ePAUIControl_StaticText, slot.radioButton, "Content_Item_Name_" .. strId)
        CopyBaseProperty(self._ui.staticText_Name_Left_templete, slot.staticText_Name)
        slot.staticText_Desc = UI.createControl(__ePAUIControl_StaticText, slot.radioButton, "Content_Item_Desc_" .. strId)
        CopyBaseProperty(self._ui.staticText_Desc_Left_templete, slot.staticText_Desc)
        slot.staticText_Price = UI.createControl(__ePAUIControl_StaticText, slot.radioButton, "Content_Item_Prce_" .. strId)
        CopyBaseProperty(self._ui.staticText_Price_Left_templete, slot.staticText_Price)
        slot.staticText_Name:SetTextMode(__eTextMode_Limit_AutoWrap)
        slot.static_PriceIcon = UI.createControl(__ePAUIControl_Static, slot.radioButton, "Content_PriceIcon_" .. strId)
...
function Panel_Dialog_NPCShop_Info:checkInit()
  ...
  if self._ui.chk_invenMoney:IsCheck() or self._ui.chk_warehouseMoney:IsCheck() then
    if nil ~= Panel_Window_Camp_All and PaGlobalFunc_Camp_All_GetIsCamping() then
...
function Panel_Dialog_NPCShop_Info:updateGuildPriceLimit()
  ...
  if false == npcShop_isGuildShopContents() then
    return
  end
...
function PaGlobalFunc_Dialog_NPCShop_Close()
  ...
  Panel_Dialog_NPCShop_Info._value.isCamping = nil
end
...
function PaGlobalFunc_Dialog_NPCShop_InvenRClick_SellItem(itemCount, slotNo)
  ...
  if npcShop_isGuildShopContents() then
    ...
    if self._ui.chk_warehouseMoney:IsCheck() and ToClient_HasWareHouseFromNpc() then
      toWhereType = CppEnums.ItemWhereType.eWarehouse
    else
      toWhereType = CppEnums.ItemWhereType.eInventory
    end
    if true == _ContentsGroup_OceanCurrent and true == itemSSW:get():isTradePrice() then
  ...
  local function sellDoit()
    ...
    if toWhereType ~= CppEnums.ItemWhereType.eGuildWarehouse and ToClient_HasWareHouseFromNpc() then
  ...
  if nil ~= tradeMasterInfo and 0 ~= itemEndurance then
    ...
    MessageBox.showMessageBox(messageBoxData)
  elseif false ~= itemSSW:isDisplayWorldMarket() and 0 ~= itemEndurance then
    local messageBoxMemo = PAGetString(Defines.StringSheet_GAME, "LUA_NPCSHOP_ITEMMARKET_USE_MSGMEMO")
    local messageBoxData = {
      title = PAGetString(Defines.StringSheet_GAME, "LUA_NPCSHOP_SELL_ALERT_2"),
      content = messageBoxMemo,
...
function PaGlobalFunc_Dialog_NPCShop_BuySomeOrSellAllItem(index)
  local self = Panel_Dialog_NPCShop_Info
  self._ui.stc_cirprogressBG:SetShow(false)
  if true == self._slots[index].hasIntimacy then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_NPCSHOP_HASINTIMACY"))
    return
  end
  if nil ~= index then
    self._value.selectedSlotIndex = index
  ...
  if nil ~= self._value.selectedSlotIndex then
    do
      ...
      if self._enum.etabIndexBuy == self._value.lastTabIndex then
        ...
        if true == _ContentsGroup_OceanCurrent and true == shopItemWrapper:getStaticStatus():isTradePrice() then
      ...
      elseif self._enum.etabIndexSell == self._value.lastTabIndex then
        do
          ...
          local function sellDoit()
            ...
            if 500000 <= pricePiece and ToClient_HasWareHouseFromNpc() and Defines.s64_const.s64_0 ~= s64_remainingTime then
        ...
        if self._enum.etabIndexRepurchase == self._value.lastTabIndex then
          ...
          if true == _ContentsGroup_OceanCurrent then
            local shopItemWrapper = npcShop_getItemRepurchase(slot.slotNo)
    ...
    do
      ...
      local function sellAllDoit()
        ...
        local function sellConfirm(count, slotNo)
          ...
          local s64_remainingTime = getLeftSecond_s64(shopItemSSW:getExpirationDate())
          if ToClient_HasWareHouseFromNpc() and Defines.s64_const.s64_0 ~= s64_remainingTime then
      ...
      elseif true == _ContentsGroup_RenewUI_ItemMarketPlace_Only then
        if false ~= shopItemSSW:isDisplayWorldMarket() and 0 ~= shopItemSSW:get():getEndurance() then
          local messageBoxMemo = PAGetString(Defines.StringSheet_GAME, "LUA_NPCSHOP_ITEMMARKET_USE_MSGMEMO")
          local messageBoxData = {
            title = PAGetString(Defines.StringSheet_GAME, "LUA_NPCSHOP_SELL_ALERT_2"),
            content = messageBoxMemo,
...
function PaGlobalFunc_Dialog_NPCShop_ScrollEvent(isUpScroll)
  ...
  if (_ContentsGroup_UsePadSnapping or ToClient_IsDevelopment()) and beforeSlotIndex ~= self._value.startSlotIndex then
...
function FromClient_Dialog_NPCShop_UpdateContent()
  local self = Panel_Dialog_NPCShop_Info
  local talker = dialog_getTalker()
  if true ~= self._value.isCamping then
    self._value.isCamping = PaGlobalFunc_Camp_All_GetIsCamping()
  end
  if nil == talker and (false == self._value.isCamping or nil == self._value.isCamping) then
    return
  end
  if false == self._value.isCamping and false == npcShop_isShopContents() and false == npcShop_isGuildShopContents() then
    return
  end
...
function Toggle_NPCShopTab_forPadEventFunc(value)
  local self = Panel_Dialog_NPCShop_Info
  if true == PaGlobalFunc_Camp_All_GetIsCamping() then
    return
  end
