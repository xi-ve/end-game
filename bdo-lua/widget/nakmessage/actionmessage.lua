...
function ActionMessageShowByObservertory()
  ...
  if _ContentsGroup_UsePadSnapping then
    if PaGlobalFunc_ConsoleKeyGuide_IsSearchModeKeyGudieShow() then
      ActionMessageHide()
      return
    end
    local actorKeyRaw = getSelfPlayer():get():getRideVehicleActorKeyRaw()
    local VehicleActorProxyWrapper = getVehicleActor(actorKeyRaw)
    if nil ~= VehicleActorProxyWrapper then
      local vehicleType = VehicleActorProxyWrapper:get():getVehicleType()
      if CppEnums.VehicleType.Type_Airballoon == vehicleType then
        ActionMessageHide()
        return
      end
    end
...
function ActionMessagePanel_Resize()
  if _ContentsGroup_UsePadSnapping then
    Panel_ActionMessage:ComputePos()
  end
  Panel_ActionMessage:SetPosX((getOriginScreenSizeX() - Panel_ActionMessage:GetSizeX()) * 0.5)
end
ActionMessagePanel_Resize()
if _ContentsGroup_UsePadSnapping then
  registerEvent("onScreenResize", "ActionMessagePanel_Resize")
  registerEvent("FromClient_NotifyObserverModeStart", "ActionMessageHide")
end
