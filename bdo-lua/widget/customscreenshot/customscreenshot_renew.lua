...
local renderMode = RenderModeWrapper.new(80, {
...
local basePosX = getOriginScreenSizeX() / 2 - Panel_Widget_ScreenShotFrame:GetSizeX() / 2
local basePosY = getOriginScreenSizeY() / 2 - Panel_Widget_ScreenShotFrame:GetSizeY() / 2
...
local prevUiMode, prevIsShowKeyGuide
...
function FullScreenShotFrame_Show()
  ...
  if gameoptionController:getCropModeEnable() == false then
    ...
    Panel_Widget_ScreenShotFrame:SetSize(getOriginScreenSizeX(), getOriginScreenSizeY())
    Panel_Widget_ScreenShotFrame:SetPosX(0)
    Panel_Widget_ScreenShotFrame:SetPosY(0)
    frameArea:SetSize(getOriginScreenSizeX(), getOriginScreenSizeY())
    ...
    Panel_Widget_ScreenShotFrame:SetPosX(0)
    Panel_Widget_ScreenShotFrame:SetPosY(0)
    frameArea:SetSize(getOriginScreenSizeX() * gameoptionController:getCropModeScaleX(), getOriginScreenSizeY() * gameoptionController:getCropModeScaleY())
...
function ScreenShotFrame_Show()
  ...
  if nil ~= Panel_ConsoleKeyGuide and false == Panel_ConsoleKeyGuide:GetShow() then
    prevIsShowKeyGuide = false
    Panel_ConsoleKeyGuide:SetShow(true)
  end
  PaGlobalFunc_ConsoleKeyGuide_SetScreenShotGuideState(true)
...
function ScreenShotFrameSize_Increase()
  ...
  local scrSizeX = getOriginScreenSizeX() * uiScale
  local scrSizeY = getOriginScreenSizeY() * uiScale
  ...
  if panelPosY + frameArea:GetPosY() - increaseSizeY < 0 or scrSizeY < height + increaseSizeY then
    return
  end
...
function ScreenShotSave_ForWeb()
  PaGlobalFunc_ConsoleKeyGuide_SetScreenShotGuideState(false)
...
function ScreenShotFrame_Close()
  ...
  if nil ~= Panel_ConsoleKeyGuide and false == prevIsShowKeyGuide then
    prevIsShowKeyGuide = nil
...
function ScreenShotFrame_RePos(deltaTime)
  ...
  elseif isPadDown(__eJoyPadInputType_RightShoulder) then
    ScreenShotFrameSize_Increase()
  elseif isPadDown(__eJoyPadInputType_RightTrigger) then
  ...
  if scrY - 10 < posY + sizeY then
    Panel_Widget_ScreenShotFrame:SetPosY(scrY - sizeY - 10)
  end
end
