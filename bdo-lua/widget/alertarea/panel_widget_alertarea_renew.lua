...
function PaGlobalFunc_Widget_Alert_Check_Hunting()
  ...
  local isHuntingButtonShow = 0 < totalCount
...
function PaGlobalFunc_Widget_Alert_Check_ChallengeReward()
  local self = Panel_Widget_Alert_info
  local challengeRWCount = PaGlobalFunc_Widget_Alert_GetRemainRewardCount()
    ...
    if nil ~= itemWrapper then
      ...
      if nil ~= itemSSW then
        ...
        if not isServantEquip and true == isEquip and true == itemWrapper:checkConditions() then
          ...
          if nil ~= defencePoint and nil ~= offencePoint and 0 < currentEndurance and true == matchEquip and true == PaGlobalFunc_Util_CheckSeasonEquip(itemSSW) and defencePoint + offencePoint > equipDefencePoint + equipOffencePoint then
...
function FGlobal_ItemMarket_SetCount()
  ...
  if false == _ContentsGroup_RenewUI_ItemMarketPlace then
    ...
    self._ui.StaticText_MarketPlaceCount:SetShow(0 < alarmCount)
...
function FromClient_Widget_Alert_NewKnowledge(mentalCardStaticStatusWrapper)
  ...
  if 0 < #cardListNormal then
    self:setButtonShow(AlertType.eALERT_NewKnowledge, true)
    self:updateIcons()
  end
  if 0 < #cardListImportant then
    self:setButtonShow(AlertType.eALERT_NewStoryKnowledge, true)
    self:updateIcons()
  end
end
...
function FromClient_updateReservePersonalMonster(characterKey, positionIndex)
  ...
  if 0 < showMonsterCount then
    ...
    self._ui.txt_blakcDesc:SetTextMode(__eTextMode_AutoWrap)
    ...
    local sizeY = math.max(self._defaultBgSizeY, self._ui.txt_blakcDesc:GetTextSizeY() + 35)
    self._ui.stc_blackBubbleBG:SetSize(self._ui.stc_blackBubbleBG:GetSizeX(), sizeY)
    self._ui.stc_blackBubbleBG:SetEnableArea(0, 0, self._ui.stc_blackBubbleBG:GetSizeX(), sizeY - 20)
  else
