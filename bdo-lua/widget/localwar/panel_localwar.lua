...
local _txt_LocalWarTime, _txt_LocalWarTime, _txt_LocalWarBlack, _txt_LocalWarRed, _txt_TeamBlack, _txt_TeamRed
if true == _ContentsGroup_NewUI_LocalWar_All then
  _txt_LocalWarTime = UI.getChildControl(Panel_LocalWarTeam, "StaticText_TimeLine")
  _txt_LocalWarBlack = UI.getChildControl(Panel_LocalWarTeam, "StaticText_ScoreTeamBlackDesert")
  _txt_LocalWarRed = UI.getChildControl(Panel_LocalWarTeam, "StaticText_ScoreTeamRedDesert")
  _txt_TeamBlack = UI.getChildControl(Panel_LocalWarTeam, "StaticText_MyTeamBlack")
  _txt_TeamRed = UI.getChildControl(Panel_LocalWarTeam, "StaticText_MyTeamRed")
else
  _txt_LocalWarTime = UI.getChildControl(Panel_LocalWarTeam, "StaticText_TimeLine")
  _txt_LocalWarBlack = UI.getChildControl(Panel_LocalWarTeam, "StaticText_TeamBlackDesert")
  _txt_LocalWarRed = UI.getChildControl(Panel_LocalWarTeam, "StaticText_TeamRedDesert")
  _txt_TeamBlack = UI.getChildControl(Panel_LocalWarTeam, "StaticText_MyTeamBlack")
  _txt_TeamRed = UI.getChildControl(Panel_LocalWarTeam, "StaticText_MyTeamRed")
end
...
function NewLocalWar_Update()
  ...
  if true == _ContentsGroup_NewUI_LocalWar_All then
    ...
    _txt_LocalWarBlack:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_LOCALWAR_BLACKDESERTTEAM_TITLE") .. " : " .. PAGetStringParam1(Defines.StringSheet_GAME, "LUA_LOCALWAR_BLACKDESERTTEAM", "teamBlackPoint", teamBlackPoint))
    _txt_LocalWarRed:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_LOCALWAR_REDDESERTTEAM", "teamRedPoint", teamRedPoint) .. " : " .. PAGetString(Defines.StringSheet_RESOURCE, "PANEL_LOCALWAR_REDDESERTTEAM_TITLE"))
  end
end
...
function LocalwarReloadUI()
  ...
  if 0 == ToClient_GetLocalwarState() then
    if true == _ContentsGroup_InstanceLocalWar then
      _txt_LocalWarTime:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_LOCALWAR_READY"))
    end
...
function NewLocalWar_TimeUpdate(deltaTime)
  ...
  if delayTime <= localwarDeltaTime then
    ...
    if 0 < saveLocalWarTime and 0 == warTime then
      _txt_LocalWarTime:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_LOCALWAR_SOONFINISH"))
    end
...
function Panel_NewLocalWar_Repos()
  Panel_LocalWarTeam:SetPosX(getOriginScreenSizeX() / 2 - Panel_LocalWarTeam:GetSizeX() / 2)
end
