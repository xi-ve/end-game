...
local ChatSubMenu = {
  ...
  _uiButtonInviteVolunteer = UI.getChildControl(Panel_Chat_SubMenu, "Button_InviteVolunteer"),
...
function ChatSubMenu:initialize()
  ...
  self._uiButtonInviteVolunteer:addInputEvent("Mouse_LUp", "HandleClicked_ChatSubMenu_InviteVolunteer()")
...
function ChatSubMenu:SetShow(isShow, isInviteParty, isInviteGuild, isInviteCompetition, isGameManager, clickedName, clickedUserNickName)
  if isShow then
    ...
    local guildMasterFlag = getSelfPlayer() and getSelfPlayer():get():isGuildMaster()
    ...
    if true == isGameManager then
      ...
      self._uiButtonBlockVote:SetShow(isCountryShow and guildMasterFlag)
    ...
    if self._uiButtonInviteVolunteer:GetShow() then
      local isGuildMaster = getSelfPlayer():get():isGuildMaster()
      local isGuildSubMaster = getSelfPlayer():get():isGuildSubMaster()
      if true == _ContentsGroup_BattleFieldVolunteer and (true == isGuildMaster or true == isGuildSubMaster) then
        self._uiButtonInviteVolunteer:SetPosY(buttonPosY)
        buttonPosY = buttonPosY + gapY
        bgSizeY = bgSizeY + gapY
        if self._uiButtonInviteVolunteer:GetSizeX() - 10 < self._uiButtonInviteVolunteer:GetTextSizeX() then
          self._uiButtonInviteVolunteer:SetTextMode(__eTextMode_AutoWrap)
          local temptext = self._uiButtonInviteVolunteer:GetText()
          self._uiButtonInviteVolunteer:SetText(temptext)
          self._uiButtonInviteVolunteer:SetSize(self._uiButtonInviteVolunteer:GetSizeX(), self._uiButtonInviteVolunteer:GetSizeY() + 10)
          bgSizeY = bgSizeY + 10
        end
      else
        self._uiButtonInviteVolunteer:SetShow(false)
      end
    end
    ...
    clickedMsg, clickedName, clickedUserNickName = nil, nil, nil
  end
...
function ChatUIPoolManager:createPanel(poolIndex, stylePanel)
  local panel
  if 0 == poolIndex then
    panel = UI.createPanelAndSetPanelRenderMode("Panel_Chat" .. poolIndex, UI_Group.PAGameUIGroup_Dialog, ChatRenderMode)
  else
    panel = UI.createPanelAndSetPanelRenderMode("Panel_Chat" .. poolIndex, UI_Group.PAGameUIGroup_Chatting, ChatRenderMode)
  end
...
function ChatUIPoolManager:createPool(poolIndex, poolStylePanel, poolParentPanel)
  local ChatUIPool = {
    ...
    _maxcount_ChatEmoticon = self._maxListCount * 15,
    _count_At = 0,
  ...
  function ChatUIPool:initialize(stylePanel, parentControl, poolIndex)
    ChatUIPool:prepareControl(stylePanel, parentControl, self._list_PanelBG, __ePAUIControl_Static, "Static_ChattingBG", self._maxcount_PanelBG)
    ChatUIPool:prepareControl(stylePanel, parentControl, self._list_TitleTab, __ePAUIControl_Button, "Button_TitleButton", self._maxcount_TitleTab)
    ChatUIPool:prepareControl(stylePanel, parentControl, self._list_TitleTabText, __ePAUIControl_StaticText, "StaticText_chattingText", self._maxcount_TitleTabText)
    ChatUIPool:prepareControl(stylePanel, parentControl, self._list_TitleTabConfigButton, __ePAUIControl_Button, "Button_Setting", self._maxcount_TitleTabConfigButton)
    ChatUIPool:prepareControl(stylePanel, parentControl, self._list_OtherTab, __ePAUIControl_Button, "Button_OtherChat", self._maxcount_OtherTab)
    ChatUIPool:prepareControl(stylePanel, parentControl, self._list_AddTab, __ePAUIControl_Button, "Button_AddChat", self._maxcount_AddTab)
    ChatUIPool:prepareControl(stylePanel, parentControl, self._list_PanelDivision, __ePAUIControl_Button, "Button_PanelDivision", self._maxcount_PanelDivision)
    ChatUIPool:prepareControl(stylePanel, parentControl, self._list_PanelDelete, __ePAUIControl_Button, "Button_PanelCombine", self._maxcount_PanelDelete)
    ChatUIPool:prepareControl(stylePanel, parentControl, self._list_ResizeButton, __ePAUIControl_Button, "Button_Size", self._maxcount_ResizeButton)
    ChatUIPool:prepareControl(stylePanel, self._list_PanelBG[0], self._list_ChattingIcon, __ePAUIControl_StaticText, "Static_ChatIcon", self._maxcount_ChattingIcon)
    ChatUIPool:prepareControl(stylePanel, self._list_PanelBG[0], self._list_ChattingSender, __ePAUIControl_StaticText, "StaticText_ChatSender", self._maxcount_ChattingSender)
    ChatUIPool:prepareControl(stylePanel, self._list_PanelBG[0], self._list_ChattingContents, __ePAUIControl_StaticText, "StaticText_ChatContent", self._maxcount_ChattingContents)
    ChatUIPool:prepareControl(stylePanel, self._list_PanelBG[0], self._list_ChattingLinkedItem, __ePAUIControl_StaticText, "StaticText_ChatLinkedItem", self._maxcount_ChattingLinkedItem)
    ChatUIPool:prepareControl(stylePanel, self._list_PanelBG[0], self._list_ChattingLinkedGuild, __ePAUIControl_StaticText, "StaticText_ChatLinkedGuild", self._maxcount_ChattingLinkedGuild)
    ChatUIPool:prepareControl(stylePanel, self._list_PanelBG[0], self._list_ChattingGuildMark, __ePAUIControl_Static, "Static_GuildMark", self._maxcount_ChattingGuildMark)
    ChatUIPool:prepareControl(stylePanel, self._list_PanelBG[0], self._list_ChattingLinkedWebSite, __ePAUIControl_StaticText, "StaticText_ChatLinkedWebSite", self._maxcount_ChattingLinkedWebSite)
    ChatUIPool:prepareControl(stylePanel, parentControl, self._list_Scroll, __ePAUIControl_Scroll, "Scroll_Chatting", self._maxcount_Scroll)
    ChatUIPool:prepareControl(stylePanel, parentControl, self._list_CloseButton, __ePAUIControl_Button, "Button_Close", self._maxcount_CloseButton)
    ChatUIPool:prepareControl(stylePanel, parentControl, self._list_ScrollReset, __ePAUIControl_Button, "Button_ScrollReset", self._maxcount_ScrollReset)
    ChatUIPool:prepareControl(stylePanel, parentControl, self._list_MoreList, __ePAUIControl_Static, "Static_MoreList", self._maxcount_MoreList)
    ChatUIPool:prepareControl(stylePanel, self._list_PanelBG[0], self._list_Emoticon, __ePAUIControl_Static, "Static_Emoticon", self._maxcount_Emoticon)
    ChatUIPool:prepareControl(stylePanel, self._list_PanelBG[0], self._list_ChatEmoticon, __ePAUIControl_Static, "Static_ChatEmoticon", self._maxcount_ChatEmoticon)
    ChatUIPool:prepareControl(stylePanel, self._list_PanelBG[0], self._list_At, __ePAUIControl_StaticText, "Static_At", self._maxcount_At)
    ChatUIPool:prepareControl(stylePanel, self._list_PanelBG[0], self._list_EmoticonAni, __ePAUIControl_Static, "Static_ChatEmoticon_SequenceAni", self._maxcount_EmoticonAni)
  ...
  function ChatUIPool:getCurrentEmoticonIndex()
    return self._count_ChatEmoticon - 1
  end
  ...
  function ChatUIPool:newChattingEmoticon()
    ...
    if self._maxcount_Emoticon < self._count_Emoticon then
      self._count_Emoticon = self._maxcount_Emoticon
    end
    return self._list_Emoticon[self._count_Emoticon - 1]
  end
  function ChatUIPool:newChattingNewEmoticon()
    self._count_ChatEmoticon = self._count_ChatEmoticon + 1
    if self._maxcount_ChatEmoticon < self._count_ChatEmoticon then
      self._count_ChatEmoticon = self._maxcount_ChatEmoticon
    end
    return self._list_ChatEmoticon[self._count_ChatEmoticon - 1]
  end
  ...
  function ChatUIPool:hideNotUse()
    ...
    for index = self._count_ChatEmoticon, self._maxcount_ChatEmoticon do
...
function ChattingViewManager:update(chatPanel, panelIndex, isShow)
  ...
  local chatting_content_PosY = poolCurrentUI._list_PanelBG[0]:GetSizeY() - 10
  ...
  local showEmoticonInfo = PaGlobalFunc_Chat_GetShowEmoticonInfo(panelIndex)
  if nil ~= showEmoticonInfo then
    ...
    PaGlobalFunc_Chat_ResetEmoticonInfo(panelIndex)
  end
  if chatPanel:isChattingPanelFreeze() and chatPanel:isFreezingMsgUpdatedValue() then
    self._srollPosition[panelIndex] = _scroll_Interval_AddPos[panelIndex] + addMessageCount
    _scroll_Interval_AddPos[panelIndex] = self._srollPosition[panelIndex]
...
function ChattingViewManager_UpdatePerFrame(deltaTime)
  ...
  if issmoothWheelscroll then
    ...
    if 0.2 < smoothWheelScorllTime then
...
function ChattingViewManager:UpdateSmoothChattingContent(chattingUpTime)
  ...
  if 0.2 < chattingUpTime and false == issmoothupMessage then
    ismsgin = false
    for panelIndex = 0, count - 1 do
      local chatPanel = ToClient_getChattingPanel(panelIndex)
...
function Chatting_OnResize()
  ...
  for poolIndex = 0, ChatUIPoolManager._poolCount - 1 do
    ...
    if 25 < panelSizeX and 50 < panelSizeY then
      panel:SetSize(panelSizeX, panelSizeY)
      chatUI._list_PanelBG[0]:SetSize(panelSizeX, panelSizeY)
      chatUI._list_Scroll[0]:SetSize(chatUI._list_Scroll[0]:GetSizeX(), panelSizeY - 67)
      for chattingContents_idx = 0, chatUI._maxcount_ChattingContents - 1 do
...
function HandleOn_ChattingEmoticon(poolIndex, emoticonIndex, EmoticonKey, isShow, isReset)
  ...
  if nil ~= EmoticonSS then
    ...
    if nil ~= emoticonAniUi then
      if true == isShow and (shownEmoticonIndex ~= emoticonIndex or true == isReset) then
        ...
        shownEmoticonIndex = emoticonIndex
        emoticonAniUi:setUpdateTextureAni(true)
        emoticonAniUi:SetShow(true)
      elseif false == isShow and emoticonIndex == shownEmoticonIndex then
        shownEmoticonIndex = nil
...
function HandleClicked_ChatSubMenu_InviteVolunteer()
  if nil ~= currentPoolIndex and nil ~= clickedMessageIndex then
    ...
    FGlobal_AgreementVolunteer_Master_Open(clickedName)
    ChatSubMenu:SetShow(false)
  end
end
...
function PaGlobal_Chat_UpdateChatPanel(panelIndex)
  local chatPanel = ToClient_getChattingPanel(panelIndex)
  if chatPanel:isOpen() then
    ChatUIPoolManager:getPool(panelIndex):clear()
    ChattingViewManager:update(chatPanel, panelIndex)
...
function PaGlobal_Chat_UpdatTopClipingPanel(panelIndex, addClipSizeY)
  ...
  local chatUI = ChatUIPoolManager._listChatUIPool[panelIndex]
