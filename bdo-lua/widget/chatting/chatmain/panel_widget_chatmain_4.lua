  ...
  if 0 ~= emoticonCount then
    ...
    while emoticonCount >= emoNum do
      ...
      if msgstartindex == emoticonindex then
        ...
        textStaticPosX = chatting_sender:GetTextSizeX() + chat_contentAddPosX
        while 1 < contentindex and nil ~= chatting_contents[contentindex - j] and chatting_contents[contentindex - j]:GetSizeX() < panelSizeX - chatting_Icon:GetSizeX() - chatting_sender:GetTextSizeX() do
          isWhile = true
  ...
  while msgstartindex < string.len(msg) do
    if false == isChattingAt and false == isLinkedItem and false == isLinkedGuild and false == isLinkedWebSite and false == isLinkedMentalCard then
      ...
      while 0 < msgDataLen do
        ...
        if "" == fontColor or nil == fontColor then
          if "" ~= checkmsg then
            fontColor = PaGlobal_ChatMain:getMessageFontColor(checkmsg)
          else
            fontColor = PaGlobal_ChatMain:getMessageFontColor(msg)
          end
        end
        if nil ~= fontColor and nil ~= checkmsg then
          local isContained, endIdx = PaGlobal_ChatMain:IsContainOldColor(checkmsg)
          if false == PaGlobal_ChatMain:IsContainPAColor(checkmsg) then
            chatting_contents[contentindex]:SetText(string.format("%s%s", fontColor, checkmsg))
          elseif 0 ~= endIdx then
            chatting_contents[contentindex]:SetText(string.format("%s%s", fontColor, checkmsg))
          else
            chatting_contents[contentindex]:SetText(checkmsg)
          end
          if true == isContained then
            local nextColorMsg = string.sub(checkmsg, endIdx, string.len(checkmsg))
            local anotherColor = PaGlobal_ChatMain:getMessageFontColor(nextColorMsg)
            if nil ~= anotherColor then
              fontColor = anotherColor
            end
          end
        else
          chatting_contents[contentindex]:SetText(checkmsg)
...
function PaGlobal_ChatMain:getChattingMessage(panelIndex, chattingMessage)
  ...
  if UI_CT.Channel == chatType then
    ...
    if true == ToClient_SelfPlayerIsGM() and 0 ~= chatRoomNo then
      local index = Uint64toUint32(chatRoomNo) - 1
      local chatRoomInfo = ToClient_getChannelChatingRoomIndex(index)
