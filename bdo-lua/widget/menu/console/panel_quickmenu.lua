...
function PaGlobal_ConsoleQuickMenu:SetUICurrentGroup(group)
  ...
  for position, info in ipairs(groupInfo) do
    ...
    control:setRenderTexture(control:getBaseTexture())
    self._ui._quickMenuPositionSlot[position - 1]:clearItem()
    self._ui._quickMenuPositionSlot[position - 1].icon:ReleaseTexture()
    self._ui._quickMenuPositionSlot[position - 1].icon:ChangeTextureInfoNameAsync("")
    if __eQuickMenuDataType_Item == info._type then
      ...
      local selfPlayer = getSelfPlayer():get()
      local inventory = selfPlayer:getInventoryByType(whereType)
      local slotNo = ToClient_GetItemNoByInventory(whereType, quickMenu._itemNo)
      local itemWrapper = getInventoryItemByType(whereType, slotNo)
      if nil ~= itemWrapper then
        self._ui._quickMenuPositionSlot[position - 1]:setItem(itemWrapper, slotNo, false, false)
        self._ui._quickMenuPositionSlot[position - 1].icon:SetShow(true)
      end
    end
  end
end
  ...
  for group = 0, __eQuickMenuDpadGroup_Count - 1 do
    ...
    if ToClient_isQuickMenuQuickMode(group) then
      ...
      if __eQuickMenuDataType_Item == quickSlotInfo._type then
        ...
        local itemWrapper = getInventoryItemByType(whereType, slotNo)
        if nil ~= itemWrapper then
          local widgetSlotNo = ToClient_GetItemNoByInventory(whereType, quickSlot._itemNo)
          widget._slot:setItem(itemWrapper, widgetSlotNo)
        end
