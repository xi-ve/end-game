...
function TooltipSimple_CommonShow(name, desc)
  ...
  if (nil == desc or "" == desc) and nil ~= name then
    local nameLength = string.len(name)
    _uiName:SetText(name)
    _uiName:SetSize(_uiName:GetTextSizeX(), _uiName:GetSizeY())
    Panel_Tooltip_SimpleText:SetSize(_uiName:GetTextSizeX() + 20, _uiName:GetTextSizeY() + _uiName:GetSpanSize().x * 2)
  ...
  if nil ~= desc and "" ~= desc then
    ...
    elseif 300 <= descLength and descLength < 500 then
...
function TooltipSimple_Hide()
  if nil == Panel_Lobby_ServerSelect_Remaster and not _ContentsGroup_RenewUI and nil ~= getCurrentCloseType and Defines.CloseType.eCloseType_Attacked == getCurrentCloseType() then
    return
  end
  Panel_Tooltip_SimpleText:SetShow(false)
