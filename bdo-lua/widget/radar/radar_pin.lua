...
function SendPingInfo_ToConsole(isForGuild)
  ...
  local float3Pos = float3(SelfPlayer:getPositionX(), 0, SelfPlayer:getPositionZ())
  ToClient_RequestSendPositionGuide(isForGuild, false, true, float3Pos)
end
function SendPingInfo_WorldMap_ToConsole(isForGuild)
  ...
  local float3Pos = float3(targetPos.x, 0, targetPos.z)
  ToClient_RequestSendPositionGuide(isForGuild, false, true, float3Pos)
end
