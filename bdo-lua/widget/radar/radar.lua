...
radar_GuildTeamBattleAlert:SetTextMode(__eTextMode_AutoWrap)
...
function RadarMap_UpdateIcon()
  ...
  if false == isGameTypeKR2() then
    ...
    radarTimeControl.VillageWar:SetShow(false)
  end
...
function RadarMap_UpdatePerFrame(deltaTime)
  ...
  if not ToClient_GetMessageFilter(10) and 10 <= playerLevel then
  ...
  if playerLevel < ToClient_getTutorialLimitLevel() then
    radar_ChangeBtn:SetShow(false)
...
function Fromclient_RadarChangePlayerIconColor()
  for Key, value in pairs(typeDepth) do
    ...
    local RadarIconList = FromClient_getRadarIconList(Key)
    for key2, actor in pairs(RadarIconActorList) do
      if true == actor:isPlayer() then
        local playerActorProxyWrapper = getPlayerActor(actor:getActorKeyRaw())
        if nil ~= playerActorProxyWrapper then
          local guildNo = playerActorProxyWrapper:getGuildNo_s64()
          local color = ToClient_GetColorStringByGuildNoForGM(guildNo)
          if nil ~= color then
            color = "0x" .. color
            if nil ~= RadarIconList[key2] then
              RadarIconList[key2]:SetColor(color)
            end
          end
        end
      end
...
function Radar_luaLoadComplete()
  ...
  if true == ToClient_isConsole() then
    RadarMap_UpdatePosition()
    local isShow = not ToClient_IsInstanceFieldPlayerbyContentsType(__eInstanceContentsType_SavageDefence)
    Panel_Radar:SetShow(isShow, false)
    Panel_TimeBar:SetShow(isShow, false)
    return
  end
