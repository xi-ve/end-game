...
local function createPartyControls()
  for index = 0, _maxPartyMemberCount - 1 do
    local partyMember = {}
    partyMember._base = UI.createControl(__ePAUIControl_Static, Panel_Party, "PartyMember_Back" .. index)
    partyMember._leader = UI.createControl(__ePAUIControl_Static, partyMember._base, "PartyMember_Leader" .. index)
    partyMember._name = UI.createControl(__ePAUIControl_StaticText, partyMember._base, "PartyMember_UserName" .. index)
    partyMember._class = UI.createControl(__ePAUIControl_Static, partyMember._base, "PartyMember_Class" .. index)
    partyMember._hpBG = UI.createControl(__ePAUIControl_Static, partyMember._base, "PartyMember_HpBG" .. index)
    partyMember._hp = UI.createControl(__ePAUIControl_Progress2, partyMember._base, "PartyMember_Hp" .. index)
    partyMember._mp = UI.createControl(__ePAUIControl_Progress2, partyMember._base, "PartyMember_Mp" .. index)
    partyMember._level = UI.createControl(__ePAUIControl_StaticText, partyMember._base, "PartyMember_UserLevel" .. index)
    partyMember._conditionBG = UI.createControl(__ePAUIControl_Static, partyMember._base, "PartyMember_ConditionBG" .. index)
    partyMember._stylePartyOptionBtn = UI.createControl(__ePAUIControl_Button, partyMember._base, "PartyMember_OptionBtn" .. index)
    partyMember._styleFollowBtn = UI.createControl(__ePAUIControl_Button, partyMember._base, "PartyMember_FollowBtn" .. index)
    partyMember._distance = UI.createControl(__ePAUIControl_Static, partyMember._base, "PartyMember_Distance" .. index)
    partyMember._conditionTxt = UI.createControl(__ePAUIControl_StaticText, partyMember._base, "PartyMember_ConditionTxt" .. index)
...
function ResponseParty_PartyMemberTextureSet(partyData, partyMemberCount, index)
  ...
  if index <= partyMemberCount - 1 and nil ~= _partyData[index] then
    ...
    elseif _partyData[index]._class == __eClassType_ShyWaman then
      ...
      _uiPartyMemberList[index]._class:getBaseTexture():setUV(x1, y1, x2, y2)
      _uiPartyMemberList[index]._class:setRenderTexture(_uiPartyMemberList[index]._class:getBaseTexture())
    elseif _partyData[index]._class == __eClassType_Guardian then
      classTypeTexture = "new_ui_common_forlua/widget/party/Party_01.dds"
      _uiPartyMemberList[index]._class:ChangeTextureInfoNameAsync(classTypeTexture)
      local x1, y1, x2, y2 = setTextureUV_Func(_uiPartyMemberList[index]._class, 121, 216, 151, 246)
      _uiPartyMemberList[index]._class:getBaseTexture():setUV(x1, y1, x2, y2)
...
function ResponseParty_updatePartyList()
  if Panel_Party:IsShow() and 0 == partyType then
    ...
    if partyMemberCount > firstCheck then
      ...
      registItem._bg:SetPosY(partyMemberCount * Panel_Party:GetSizeY() + 40)
    end
...
registerEvent("FromClient_ApplyUISettingPanelInfo", "partWidget_OnscreenEvent")
