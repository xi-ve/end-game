...
function PaGlobal_AlertArea:Update()
  ...
  Panel_AlertArea:SetShow(0 <= showCount)
end
...
function PaGlobal_AlertArea:OverEvent(index)
  ...
  elseif self._buttonType._enduranceShip == index then
    ...
    if nil ~= servantInfoWrapper then
      if true == PaGlobalFunc_Util_IsBigShip(servantInfoWrapper:getVehicleType()) then
        Panel_SailShipEndurance:SetShow(true)
        Panel_SailShipEndurance:SetPosX(Panel_AlertArea:GetPosX() + self._ui._button[index]:GetPosX() - Panel_SailShipEndurance:GetSizeX() / 3)
        Panel_SailShipEndurance:SetPosY(Panel_AlertArea:GetPosY() - Panel_SailShipEndurance:GetSizeY() - 10)
      else
...
function PaGlobal_AlertArea:OutEvent(index)
  ...
  Panel_SailShipEndurance:SetShow(false)
end
...
function PaGlobal_AlertArea:HuntingCountCheck()
  ...
  local isHuntingButtonShow = 0 < gargoyleCount + whaleCount
...
function FGlobal_AlertArea_EnduranceButtonShow(enduranceType, isShow)
  ...
  elseif CppEnums.EnduranceType.eEnduranceType_SailShip == enduranceType then
    self:ButtonShow(self._buttonType._enduranceShip, isShow)
  else
    return
  end
