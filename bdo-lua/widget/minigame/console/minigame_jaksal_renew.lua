...
local crosshairPosX = 0
local crosshairPosY = 0
local crosshairSpeed = 500
function Panel_MiniGame_Jaksal_Start()
  ...
  crosshairPosX = getOriginScreenSizeX() / 2
  crosshairPosY = getOriginScreenSizeY() / 2
end
...
local function updateCrossHair()
  local posX = crosshairPosX - Panel_MiniGame_Jaksal:GetPosX() - ui._crossHair:GetSizeX() / 2 + 5
  local posY = crosshairPosY - Panel_MiniGame_Jaksal:GetPosY() - ui._crossHair:GetSizeY() / 2 + 5
...
function Panel_MiniGame_PerFrameUpdate(deltaTime)
  ...
  if isPadPressed(__eJoyPadInputType_LeftStick_Up) then
    crosshairPosY = crosshairPosY - crosshairSpeed * deltaTime
  end
  if isPadPressed(__eJoyPadInputType_LeftStick_Down) then
    crosshairPosY = crosshairPosY + crosshairSpeed * deltaTime
  end
  if isPadPressed(__eJoyPadInputType_LeftStick_Left) then
    crosshairPosX = crosshairPosX - crosshairSpeed * deltaTime
  end
  if isPadPressed(__eJoyPadInputType_LeftStick_Right) then
    crosshairPosX = crosshairPosX + crosshairSpeed * deltaTime
  end
  if crosshairPosX < ui._water:GetPosX() + Panel_MiniGame_Jaksal:GetPosX() then
    crosshairPosX = ui._water:GetPosX() + Panel_MiniGame_Jaksal:GetPosX()
  elseif Panel_MiniGame_Jaksal:GetPosX() + ui._water:GetPosX() + ui._water:GetSizeX() < crosshairPosX then
    crosshairPosX = ui._water:GetPosX() + ui._water:GetSizeX() + Panel_MiniGame_Jaksal:GetPosX()
  end
  if crosshairPosY < ui._water:GetPosY() + Panel_MiniGame_Jaksal:GetPosY() then
    crosshairPosY = ui._water:GetPosY() + Panel_MiniGame_Jaksal:GetPosY()
  elseif ui._water:GetPosY() + ui._water:GetSizeY() + Panel_MiniGame_Jaksal:GetPosY() < crosshairPosY then
    crosshairPosY = ui._water:GetPosY() + ui._water:GetSizeY() + Panel_MiniGame_Jaksal:GetPosY()
  end
end
function Panel_MiniGame_FillGaugeUpdate(deltaTime, shotSuccess)
  ...
  ui._gaugeJaksalPower:SetProgressRate(sumTime * 100 / 2)
  if 1 <= sumTime then
...
function Panel_MiniGame_Jaksal_KeyPressCheck()
  local posX = crosshairPosX - Panel_MiniGame_Jaksal:GetPosX() - ui._jaksal:GetSizeX() / 2
  local posY = crosshairPosY - Panel_MiniGame_Jaksal:GetPosY() - ui._jaksal:GetSizeY() / 2 + 13
  if 2 <= sumTime and isStoppedGame == false then
...
function MiniGame_Jaksal_PadKeyPress(keyType)
  if keyCustom_IsDownOnce_Action(CppEnums.ActionInputType.ActionInputType_Jump) then
    Panel_MiniGame_Jaksal_KeyPressCheck()
  end
end
registerEvent("EventActionMiniGamePadDownOnce", "MiniGame_Jaksal_PadKeyPress")
