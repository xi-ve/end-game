...
function PaGlobal_MiniGame_Find:initialize()
  if nil == Panel_MiniGame_Find then
    return
  end
  PaGlobal_MiniGame_Find._slots = Array.new()
  PaGlobal_MiniGame_Find._rewardSlot = Array.new()
  PaGlobal_MiniGame_Find._damageSlot = Array.new()
  PaGlobal_MiniGame_Find._gameState = nil
  PaGlobal_MiniGame_Find._readyToEndGame = false
  PaGlobal_MiniGame_Find._readyToNextGame = false
  PaGlobal_MiniGame_Find._curSec = 0
  PaGlobal_MiniGame_Find._gameCurDepth = 0
  PaGlobal_MiniGame_Find._gameLastDepth = 0
  PaGlobal_MiniGame_Find._curRClickCount = 0
  PaGlobal_MiniGame_Find._curSlotSize = 0
  PaGlobal_MiniGame_Find._stateMsgKey = 0
  PaGlobal_MiniGame_Find._tutorialOpen = 0
  PaGlobal_MiniGame_Find._tutorialTime = 0
  PaGlobal_MiniGame_Find._tutorialIndex = -1
  PaGlobal_MiniGame_Find._tutorialEffectShow = false
  self._ui._baseOpenSlot = UI.getChildControl(Panel_MiniGame_Find, "Static_OpenSlot")
  self._ui._baseCloseSlot = UI.getChildControl(Panel_MiniGame_Find, "Static_CloseSlot")
  self._ui._closeButton = UI.getChildControl(Panel_MiniGame_Find, "Button_Win_Close")
  self._ui._rightBG = UI.getChildControl(Panel_MiniGame_Find, "Static_RightBg")
  self._ui._startMsg = UI.getChildControl(Panel_MiniGame_Find, "Static_Msg")
  self._ui._timerMsg = UI.getChildControl(Panel_MiniGame_MiniGameResult, "StaticText_Msg")
  self._ui._staticObjBg = UI.getChildControl(Panel_MiniGame_Find, "Static_Body")
  self._ui._descBg = UI.getChildControl(Panel_MiniGame_Find, "Static_BottomBg")
  self._ui._facePicture = UI.getChildControl(Panel_MiniGame_Find, "Static_Obsidian")
  self._ui._bubbleBg = UI.getChildControl(Panel_MiniGame_Find, "Static_Obsidian_B")
  self._ui._maskBg = UI.getChildControl(Panel_MiniGame_Find, "Static_MaskBg")
  self._ui._tutorialStep_1 = UI.getChildControl(Panel_MiniGame_Find, "Static_TutorialStep_1")
  self._ui._tutorialStep_2 = UI.getChildControl(Panel_MiniGame_Find, "Static_TutorialStep_2")
  self._ui._tutorialStep_3 = UI.getChildControl(Panel_MiniGame_Find, "Static_TutorialStep_3")
  ...
  registerCloseLuaEvent(Panel_MiniGame_Find, PAUIRenderModeBitSet({
    Defines.CloseType.eCloseType_Escape
  }), "PanelCloseFunc_MiniGameFind()")
...
function PaGlobal_MiniGame_Find:close()
  ...
  reqCloseUI(Panel_MiniGame_Find, false)
...
function PaGlobal_MiniGame_Find:createSlot()
  ...
  for col = 0, self._config._slotCols - 1 do
    ...
    for row = 0, self._config._slotRows - 1 do
      ...
      slot.close = UI.createControl(__ePAUIControl_Static, Panel_MiniGame_Find, "Slot_Close_" .. col .. "_" .. row)
      slot.open = UI.createControl(__ePAUIControl_Static, Panel_MiniGame_Find, "Slot_Open_" .. col .. "_" .. row)
      slot.damage = UI.createControl(__ePAUIControl_Static, Panel_MiniGame_Find, "Slot_Damage_" .. col .. "_" .. row)
  ...
  self._ui._mainObjBG = UI.createControl(__ePAUIControl_Static, Panel_MiniGame_Find, "MainObjBG")
...
function PaGlobal_MiniGame_Find:createRewardSlot()
  ...
  for ii = 0, self._config._rewardMaxCount - 1 do
    local slot = {}
    slot.background = UI.createControl(__ePAUIControl_Static, self._ui._rightBG, "RewardItemBG_" .. ii)
...
function PaGlobal_MiniGame_Find:itemTooltip_Show(index)
  ...
  if nil == self._rewardSlot[index] then
    return
  end
  if nil == self._rewardSlot[index].itemNo then
    return
  end
...
function FGlobal_MiniGameFind_Close()
  PaGlobal_MiniGame_Find:itemTooltip_Hide()
...
function FromClient_MiniGameFindSlotShowMain(col, row)
  PaGlobalFunc_MiniGameFind_CheckLoadUI()
  if nil == Panel_MiniGame_Find then
    return
  end
  local self = PaGlobal_MiniGame_Find
...
function FromClient_MiniGameFindSetState(serverState, msgKey)
  ...
  self:updateStateText(serverState)
  self._curSec = 0
  Panel_MiniGame_MiniGameResult:SetShow(true)
end
function PaGlobal_MiniGame_Find:getNonStateString(stringKey)
  if 1 == stringKey then
...
function PaGlobal_MiniGame_Find:getWaitStateString(stringKey)
  if 1 == stringKey then
...
function PaGlobal_MiniGame_Find:updateStateText(serverState)
  local strMsg = ""
  if serverState == self._state.None then
  ...
  local textSizeX = self._ui._timerMsg:GetTextSizeX()
  local textLimitSizeX = getScreenSizeX() * 0.8
  if panelSizeX < textSizeX then
    if textSizeX > textLimitSizeX then
      self._ui._timerMsg:SetSize(textSizeX * 0.5, self._ui._timerMsg:GetSizeY())
      self._ui._timerMsg:SetTextMode(__eTextMode_AutoWrap)
      self._ui._timerMsg:SetText(strMsg)
      Panel_MiniGame_MiniGameResult:SetSize(self._ui._timerMsg:GetTextSizeX(), Panel_MiniGame_MiniGameResult:GetSizeY())
    else
    ...
    self._ui._timerMsg:SetHorizonCenter()
  end
end
function PaGlobalFunc_MiniGameFind_CheckLoadUI()
  local rv = reqLoadUI("UI_Data/Widget/MiniGame/MiniGame_Find.xml", "Panel_MiniGame_Find", Defines.UIGroup.PAGameUIGroup_Windows, PAUIRenderModeBitSet({
    Defines.RenderMode.eRenderMode_Default
  }))
  if nil ~= rv and 0 ~= rv then
    Panel_MiniGame_Find = rv
    rv = nil
    PaGlobal_MiniGame_Find:initialize()
  end
end
function PaGloablFunc_MiniGameFind_GetShow()
  if nil == Panel_MiniGame_Find then
    return false
  end
  return Panel_MiniGame_Find:GetShow()
end
...
function FromClient_MiniGameFind_luaLoadComplete()
  PaGlobal_MiniGame_Find:registEventHandler()
end
registerEvent("FromClient_luaLoadComplete", "FromClient_MiniGameFind_luaLoadComplete")
