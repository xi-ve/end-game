...
local function init()
  ...
  for k = 1, 50 do
    local ball = UI.createControl(__ePAUIControl_Static, ui._myGauge, "ball_" .. tostring(k))
...
function BattleGauge_RePosition()
  ...
  Panel_BattleGauge:SetPosX(scrX / 2 - 135.5)
...
function Panel_Minigame_BattleGauge_Start()
  ...
  currentPercent = 50
  setProgress(true)
  ui._text_TitleText:SetText(PAGetString(Defines.StringSheet_RESOURCE, "MINIGAME_BATTLEGAUGE_TXT_PURPOSE"))
  ui._spaceBar:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_MINIGAME_SPACEBAR"))
