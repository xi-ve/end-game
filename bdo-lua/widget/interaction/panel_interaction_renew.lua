...
function PanelInteraction:show(actor)
  ...
  for ii = 0, #_interactionTargetUIList do
    ...
    if true == _ContentsGroup_NewUI_InstallMode_All then
      if true == PaGlobal_FarmInfo_All_GetShowPanel() then
        isShow = false
      end
    elseif true == PaGlobalFunc_InstallationMode_PlantInfo_GetShow() then
      isShow = false
    end
    if true == ToClient_isPS4() and CppEnums.InteractionType.InteractionType_RankerHouseList == ii then
      isShow = false
    end
...
function FromClient_PanelInteraction_Fail()
  ...
  if Defines.UIMode.eUIMode_NpcDialog == GetUIMode() and true == _ContentsGroup_NewUI_Dialog_All then
...
function FGlobal_Interaction_CheckAndGetPressedKeyCode_Xbox(deltaTime)
  ...
  elseif keyCustom_IsPressed_Action(CppEnums.ActionInputType.ActionInputType_Interaction) then
    ...
    if 0.5 < _currentInteractionKeyPressedTime then
